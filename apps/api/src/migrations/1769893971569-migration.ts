...
export class Migration1769893971569 implements MigrationInterface {
  name = 'Migration1769893971569'

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`ALTER TYPE "public"."sandbox_state_enum" RENAME TO "sandbox_state_enum_old"`)
    await queryRunner.query(
      `CREATE TYPE "public"."sandbox_state_enum" AS ENUM('creating', 'restoring', 'destroyed', 'destroying', 'started', 'stopped', 'starting', 'stopping', 'error', 'build_failed', 'pending_build', 'building_snapshot', 'unknown', 'pulling_snapshot', 'archived', 'archiving', 'snapshotting', 'forking', 'cloning')`,
    )
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."sandbox_state_enum_old" AS ENUM('archived', 'archiving', 'build_failed', 'building_snapshot', 'creating', 'destroyed', 'destroying', 'error', 'forking', 'pending_build', 'pulling_snapshot', 'restoring', 'snapshotting', 'started', 'starting', 'stopped', 'stopping', 'unknown')`,
    )
    await queryRunner.query(`ALTER TABLE "sandbox" ALTER COLUMN "state" DROP DEFAULT`)
    await queryRunner.query(
