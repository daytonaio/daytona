...
export class SandboxStartAction extends SandboxAction {
    ...
    if (snapshotRunner) {
      switch (snapshotRunner.state) {
        case SnapshotRunnerState.READY: {
          ...
          await this.sandboxService.updateSandboxState(sandbox.id, SandboxState.UNKNOWN, lockCode)
          return SYNC_AGAIN
        }
  ...
  private async handleUnassignedRunnerSandbox(
    ...
    try {
      ...
      if (runner) {
        await this.sandboxService.updateSandboxState(sandbox.id, SandboxState.UNKNOWN, lockCode, runner.id)
        return SYNC_AGAIN
      }
    } catch {
    ...
    for (const snapshotRunner of snapshotRunners) {
      ...
      if (runner.availabilityScore >= this.configService.getOrThrow('runnerUsage.declarativeBuildScoreThreshold')) {
        if (snapshotRunner.state === targetState) {
          await this.sandboxService.updateSandboxState(sandbox.id, targetSandboxState, lockCode, runner.id)
          return SYNC_AGAIN
    ...
    if (isBuild) {
      ...
      await this.sandboxService.updateSandboxState(sandbox.id, SandboxState.BUILDING_SNAPSHOT, lockCode, runner.id)
    } else {
      const snapshot = await this.snapshotService.getSnapshotByName(sandbox.snapshot, sandbox.organizationId)
      ...
      await this.sandboxService.updateSandboxState(sandbox.id, SandboxState.PULLING_SNAPSHOT, lockCode, runner.id)
    }
  ...
  private async handleRunnerSandboxUnknownStateOnDesiredStateStart(
    ...
    await this.sandboxService.updateSandboxState(sandbox.id, SandboxState.CREATING, lockCode)
  ...
  private async handleRunnerSandboxStoppedOrArchivedStateOnDesiredStateStart(
    ...
    if (sandbox.runnerId === null) {
      ...
      await this.sandboxService.updateSandboxState(sandbox.id, SandboxState.STARTING, lockCode)
      return SYNC_AGAIN
    }
  ...
  private async handleRunnerSandboxPullingSnapshotStateCheck(sandbox: Sandbox, lockCode: LockCode): Promise<SyncState> {
    ...
    if (sandboxInfo.state === SandboxState.PULLING_SNAPSHOT) {
      await this.sandboxService.updateSandboxState(sandbox.id, SandboxState.PULLING_SNAPSHOT, lockCode)
  ...
  private async restoreSandboxOnNewRunner(
    ...
    await this.sandboxService.updateSandboxState(sandbox.id, SandboxState.RESTORING, lockCode, runner.id)
