        ...
        await Promise.all(
          sandboxs.map(async (sandbox) => {
            ...
            try {
              ...
              this.logger.error(`Error processing auto-stop state for sandbox ${sandbox.id}: ${error.message}`)
            } finally {
        ...
        await Promise.all(
          sandboxs.map(async (sandbox) => {
            ...
            try {
              ...
              this.logger.error(`Error processing auto-archive state for sandbox ${sandbox.id}: ${error.message}`)
            } finally {
              await this.redisLockProvider.unlock(lockKey)
            }
          }),
