...
export class SnapshotManager {
  ...
  async processPullOnInitialRunner(snapshot: Snapshot) {
    ...
    try {
      ...
      if (err.code !== 'ECONNRESET') {
        await this.runnerService.createSnapshotRunner(runner.id, snapshot.id, SnapshotRunnerState.ERROR, err.message)
        throw err
      }
    }
    ...
    const snapshotRunner = await this.snapshotRunnerRepository.findOne({
      where: {
        runnerId: runner.id,
        snapshotRef: snapshot.imageName,
      },
    })

    if (!snapshotRunner) {
      await this.runnerService.createSnapshotRunner(runner.id, snapshot.id, SnapshotRunnerState.READY)
      return
    }
