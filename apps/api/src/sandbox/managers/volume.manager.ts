  ...
  implements OnModuleInit, TrackableJobExecutions, OnApplicationShutdown, OnApplicationBootstrap
{
  ...
  constructor(
    ...
    @InjectRepository(VolumeDisk)
    private readonly diskRepository: Repository<VolumeDisk>,
  ...
  //
  // When `subpath` is provided the disk is scoped to that prefix within
  // the bucket via `bucketPrefix`, so the mount token returned by Archil
  // can only see objects under that prefix — providing isolation between
  // sandboxes sharing the same volume with different subpaths.
  //
  ...
  // `S3_SECRET_KEY` at it.
  private buildArchilMount(volume: Volume, subpath?: string | null): ArchilDiskMount {
    ...
    const bucketPrefix = subpath ? (subpath.endsWith('/') ? subpath : `${subpath}/`) : undefined
  ...
  // Ensures an Archil disk exists for the given (volume, subpath) pair,
  // creating one with `bucketPrefix` if it doesn't already exist. Returns
  // the VolumeDisk record with disk credentials. Used at sandbox
  // start time so provisioning is lazy — we don't know which subpaths
  // will be needed until a sandbox references them.
  //
  // When subpath is null/undefined, the disk covers the entire bucket.
  // When subpath is set, `bucketPrefix` scopes the disk to that prefix
  // so the mount token only grants access to data under it.
  async ensureDisk(volume: Volume, subpath?: string | null): Promise<VolumeDisk> {
    this.assertArchilProvisioningPossible(volume)

    const normalizedSubpath = subpath || null

    // Check if a record already exists
    const existing = await this.diskRepository.findOne({
      where: {
        volumeId: volume.id,
        subpath: normalizedSubpath === null ? IsNull() : normalizedSubpath,
      },
    })
    if (existing) {
      return existing
    ...
    // Acquire a per-(volume, subpath) lock to prevent concurrent creation
    const lockKey = `volume-disk:${volume.id}:${normalizedSubpath || '__root__'}`
    if (!(await this.redisLockProvider.lock(lockKey, 30))) {
      // Another instance is creating the disk; wait briefly and re-check
      await setTimeout(2000)
      const retryExisting = await this.diskRepository.findOne({
        where: {
          volumeId: volume.id,
          subpath: normalizedSubpath === null ? IsNull() : normalizedSubpath,
        },
      ...
      throw new Error(
        `Could not acquire lock to provision Archil disk for volume ${volume.id} subpath=${normalizedSubpath}. Retry the sandbox start.`,
      )
    }

    try {
      const diskName = this.buildDiskName(volume.id, normalizedSubpath)
      const created = await this.archilClient.createDisk({
        name: diskName,
        region: this.archilClient.getDefaultRegion(),
        mount: this.buildArchilMount(volume, normalizedSubpath),
      })

      const encryptedToken = await this.encryptionService.encrypt(created.mountToken)

      const record = this.diskRepository.create({
        volumeId: volume.id,
      ...
      return await this.diskRepository.save(record)
    } finally {
      await this.redisLockProvider.unlock(lockKey)
    }
  }

  // Deletes all disks associated with a volume. Called during volume
  // deletion to clean up Archil resources.
  async deleteDisksFor(volumeId: string): Promise<void> {
    const disks = await this.diskRepository.find({ where: { volumeId } })
    if (!disks.length) {
      return
    }
    ...
    for (const disk of disks) {
      try {
        if (this.archilClient.isConfigured()) {
          await this.archilClient.deleteDisk(disk.archilDiskId, disk.archilRegion)
        }
      } catch (error) {
        this.logger.error(`Failed to delete Archil disk ${disk.archilDiskId} for volume ${volumeId}: ${error.message}`)
      }
    }

    await this.diskRepository.delete({ volumeId })
  }

  // Generates a stable, idempotent disk name for a (volume, subpath) pair.
  // Archil disk names allow [a-zA-Z0-9_-]+ up to 100 chars. We use a
  // short hash of the subpath to keep names deterministic and short.
  private buildDiskName(volumeId: string, subpath: string | null): string {
    if (!subpath) {
      return `daytona-vol-${volumeId}`
    }
    const hash = createHash('sha256').update(subpath).digest('hex').slice(0, 12)
    return `daytona-vol-${volumeId}-${hash}`
  }
  ...
  // the same data. Also cleans up any per-subpath disks from the
  // volume_disk table. Idempotent: calling this on a volume without an
  ...
  async detachArchilDiskFrom(volume: Volume): Promise<Volume> {
    ...
    // Also remove all disks created for this volume
    await this.deleteDisksFor(volume.id)
  ...
  private async handlePendingDelete(volume: Volume, lockKey: string): Promise<void> {
    try {
      ...
      // Delete per-(volume, subpath) Archil disks provisioned for this volume
      await this.deleteDisksFor(volume.id)
