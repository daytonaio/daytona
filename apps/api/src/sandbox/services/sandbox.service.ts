...
export class SandboxService {
  ...
  async getPortPreviewUrl(sandboxIdOrName: string, organizationId: string, port: number): Promise<PortPreviewUrlDto> {
    ...
    const region = await this.regionService.findOne(sandbox.region, undefined, true)
  ...
  async getToolboxProxyUrl(sandboxId: string): Promise<string> {
    ...
    return region.toolboxProxyUrl + '/toolbox' || this.configService.getOrThrow('proxy.toolboxUrl')
  }
  ...
  async createSshAccess(
    ...
    const region = await this.regionService.findOne(sandbox.region, organizationId, true)
