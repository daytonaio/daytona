...
export class VolumeService {
  ...
  async prepareRunnerVolumes(sandboxVolumes?: SandboxVolume[]): Promise<PreparedRunnerVolumes> {
    ...
    for (const ref of sandboxVolumes) {
      ...
      if (backend === VOLUME_BACKEND_EXPERIMENTAL) {
        // Lazily provision a per-(volume, subpath) Archil disk. Each disk
        // is created with `bucketPrefix` so its mount token only grants
        // access to the specified subpath's data — preventing cross-subpath
        // data leakage between sandboxes sharing the same volume.
        const disk = await this.volumeManager.ensureDisk(volume, ref.subpath)

        dto.archilDisk = disk.archilDiskId
        dto.archilRegion = disk.archilRegion
        dto.archilMountToken = await this.encryptionService.decrypt(disk.archilMountTokenEnc)
        // The disk is already scoped to the subpath via bucketPrefix;
        // clear the subpath from the DTO so the daemon mounts at the
        // disk root (which IS the subpath's data).
        dto.subpath = undefined
      }
