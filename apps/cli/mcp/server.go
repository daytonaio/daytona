...
func (s *DaytonaMCPServer) addTools() {
	createSandboxTool := mcp.NewTool("create_sandbox",
		...
		mcp.WithString("auto_delete_interval", mcp.DefaultString("-1"), mcp.Description("Auto-delete interval in minutes (negative value means disabled, 0 means delete immediately upon stopping) for the sandbox.")),
	)
