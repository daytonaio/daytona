...
func GetCreateSandboxTool() mcp.Tool {
	return mcp.NewTool("create_sandbox",
		...
		mcp.WithNumber("cpu", mcp.Description("CPU cores allocated to the sandbox.")),
		mcp.WithNumber("gpu", mcp.Description("GPU units allocated to the sandbox.")),
		mcp.WithNumber("memory", mcp.Description("Memory allocated to the sandbox in GB.")),
		mcp.WithNumber("disk", mcp.Description("Disk space allocated to the sandbox in GB.")),
