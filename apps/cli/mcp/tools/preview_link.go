...
func PreviewLink(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if checkServer {
		...
		result, _, err := apiClient.ToolboxAPI.ExecuteCommand(ctx, sandboxId).ExecuteRequest(*apiclient.NewExecuteRequest(checkCmd)).Execute()
		if err != nil {
		...
		if response == "error" || strings.HasPrefix(response, "0") {
			...
			psResult, _, err := apiClient.ToolboxAPI.ExecuteCommand(ctx, sandboxId).ExecuteRequest(*apiclient.NewExecuteRequest(psCmd)).Execute()
			if err != nil {
