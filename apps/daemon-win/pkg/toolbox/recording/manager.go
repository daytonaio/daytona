...
func NewRecordingManager(configDir string) *RecordingManager {
...
func (m *RecordingManager) StartRecording(label string) (*Recording, error) {
	...
	// -f gdigrab: Windows Desktop Duplication capture
	// -framerate 30: 30 FPS
	...
	log.Infof("Started recording %s to %s", id, filePath)
	...
	go func() {
		...
		if active, exists := m.activeRecordings[id]; exists {
			if err != nil {
				...
				active.recording.Status = StatusFailed
			}
...
func (m *RecordingManager) StopRecording(id string) (*Recording, error) {
	...
	active.recording.Status = StatusCompleted
