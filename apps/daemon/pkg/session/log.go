...
func (s *SessionService) GetSessionCommandLogs(sessionId, commandId string, isCombinedOutput, isWebsocketUpgrade bool, request *http.Request, responseWriter http.ResponseWriter, follow bool) ([]byte, error) {
	...
	if isWebsocketUpgrade {
		...
		defer logFile.Close()
		ReadLog(request, responseWriter, follow, logFile, util.ReadLogWithExitCode, exitCodeFilePath, func(conn *websocket.Conn, messages chan []byte, errors chan error) {
			var buffer []byte
			for {
				select {
...
func ReadLog[T any](request *http.Request, responseWriter http.ResponseWriter, follow bool, logReader io.Reader, readFunc func(context.Context, io.Reader, bool, string, chan T, chan error), exitCodeFilePath string, wsWriteFunc func(*websocket.Conn, chan T, chan error)) {
