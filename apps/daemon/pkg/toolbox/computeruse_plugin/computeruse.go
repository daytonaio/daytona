...
func NewComputerUse() *ComputerUse {
	return &ComputerUse{
		processes: make(map[string]*Process),
	}
}

func (c *ComputerUse) Start() error {
	...
	// Initialize processes based on supervisor config
...
func (c *ComputerUse) GetProcessStatus() map[string]interface{} {
	c.mu.RLock()
	defer c.mu.RUnlock()
	...
	for name, process := range c.processes {
		process.mu.Lock()
		processStatus := map[string]interface{}{
...
func (c *ComputerUse) IsProcessRunning(processName string) bool {
	c.mu.RLock()
	defer c.mu.RUnlock()

	process, exists := c.processes[processName]
	if !exists {
		return false
	}
...
func (c *ComputerUse) RestartProcess(processName string) error {
	c.mu.RLock()
	process, exists := c.processes[processName]
	c.mu.RUnlock()
...
func (c *ComputerUse) GetProcessLogs(processName string) (string, error) {
	c.mu.RLock()
	process, exists := c.processes[processName]
	c.mu.RUnlock()
	...
	if process.LogFile == "" {
		return "", fmt.Errorf("no log file configured for process %s", processName)
	}

	content, err := os.ReadFile(process.LogFile)
	if err != nil {
		return "", fmt.Errorf("failed to read log file for %s: %v", processName, err)
	}
...
func (c *ComputerUse) GetProcessErrors(processName string) (string, error) {
	c.mu.RLock()
	process, exists := c.processes[processName]
	c.mu.RUnlock()
	...
	if process.ErrFile == "" {
		return "", fmt.Errorf("no error file configured for process %s", processName)
	}

	content, err := os.ReadFile(process.ErrFile)
	if err != nil {
		return "", fmt.Errorf("failed to read error file for %s: %v", processName, err)
	}

	return string(content), nil
}

func (c *ComputerUse) GetStatus(ctx *gin.Context) {
