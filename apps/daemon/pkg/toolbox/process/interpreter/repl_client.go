...
func (c *Context) start() error {
	...
	slog.Debug("Started interpreter context", "contextId", c.info.ID, "pid", c.cmd.Process.Pid)
...
func (c *Context) workerReadLoop() {
	...
	for scanner.Scan() {
		...
		if err != nil {
			slog.Error("Failed to parse worker chunk", "error", err)
			continue
		}
		c.handleChunk(chunk)
	}
	...
	err := scanner.Err()
	if err != nil {
		slog.Error("Error reading from worker", "error", err)
	}
}
...
func (c *Context) monitorProcess() {
	...
	if err != nil {
		...
		c.commandMu.Unlock()
		slog.Error("Interpreter context process exited with error", "contextId", contextID, "error", err)
	} else {
		slog.Debug("Interpreter context process exited normally", "contextId", contextID)
	}
...
func (c *Context) shutdown() {
	...
	if done != nil {
		...
		case <-done:
			...
			slog.Debug("Interpreter context shut down gracefully", "contextId", contextID)
		case <-time.After(2 * time.Second):
			// Timeout - force kill
			slog.Debug("Interpreter context shutdown timeout, force killing", "contextId", contextID)
			if cancel != nil {
				cancel()
			}
			if cmd != nil && cmd.Process != nil {
