...
func (p *PTYController) CreatePTYSession(c *gin.Context) {
	...
	if !req.LazyStart {
		if err := session.start(); err != nil {
			...
			slog.Error("failed to start PTY at create", "error", err)
...
func (p *PTYController) DeletePTYSession(c *gin.Context) {
	...
	if s, ok := ptyManager.Delete(id); ok {
		s.kill()
		slog.Debug("Deleted PTY session", "sessionId", id)
...
func (p *PTYController) ConnectPTYSession(c *gin.Context) {
	...
	if err != nil {
		slog.Debug("failed to connect to PTY session", "sessionId", id, "error", err)
...
func (p *PTYController) ResizePTYSession(c *gin.Context) {
	...
	slog.Debug("Resized PTY session", "sessionId", id, "cols", req.Cols, "rows", req.Rows)
