...
func (s *PTYSession) start() error {
	...
	slog.Debug("Started PTY session", "sessionId", s.info.ID, "pid", s.cmd.Process.Pid)
	...
	go func() {
		...
		slog.Debug("PTY session process exited and cleaned up", "sessionId", sessionID, "exitCode", exitCode, "exitReason", exitReason)
	}()
