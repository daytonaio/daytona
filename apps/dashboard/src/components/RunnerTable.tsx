...
export function RunnerTable({
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, columnId, filterValue) => {
      ...
      const regionName = getRegionName(runner.regionId, regions)
      ...
      cell: ({ row }) => (
        <div className="w-full truncate flex items-center gap-2">
          ...
          <button
            onClick={(e) => {
              e.stopPropagation()
              copyToClipboard(getRegionName(row.original.regionId, regions))
            }}
            className="text-muted-foreground hover:text-foreground transition-colors"
...
const getRegionName = (regionId: string, regions: Region[]) => {
