  ...
  const columns: ColumnDef<Sandbox>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return (
          <div className=" w-full truncate">
            <span>{row.original.snapshot || 'None'}</span>
          </div>
        )
      },
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return (
          <span>
            {row.original.cpu} vCPU, {row.original.memory} GiB, {row.original.disk} GiB
          </span>
        )
      },
        ...
        return (
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger>
                {labelCount > 0 ? (
                  <div className="truncate max-w-md bg-blue-100 rounded-sm text-blue-800 dark:bg-blue-950 dark:text-blue-200 px-1">
                    {labelCount > 0 ? (labelCount === 1 ? '1 label' : `${labelCount} labels`) : '/'}
                  </div>
                ) : (
                  <div className="truncate max-w-md text-muted-foreground">No labels</div>
                )}
              </TooltipTrigger>
              {labels && (
                <TooltipContent>
