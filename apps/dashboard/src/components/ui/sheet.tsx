...
export const sheetVariants = cva(
  'fixed z-50 gap-4 bg-background p-6 shadow-lg outline-hidden transition ease-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:duration-250 data-[state=open]:duration-250',
  {
    variants: {
      side: {
...
function ResizableSheetContentHandle({
  ...
  return (
    <div
      ...
      className={cn(
        'group absolute top-0 z-20 hidden h-full cursor-col-resize touch-none select-none outline-hidden sm:block data-[disabled]:pointer-events-none',
        {
          'left-0 -translate-x-full': isRight,
          'right-0 translate-x-full': !isRight,
        },
        className,
      )}
      style={{ width: hitArea }}
    ...
    >
      ...
      <div
        className={cn(
          ...
          {
            'right-0 translate-x-1/2': isRight,
            'left-0 -translate-x-1/2': !isRight,
          },
        )}
      >
