...
const Snapshots: React.FC = () => {
  ...
  const [targetPropagations, setTargetPropagations] = useState<Array<{ target: string; userMinimum: number }>>([])
  ...
  const [editingTargetPropagations, setEditingTargetPropagations] = useState<
    Array<{ target: string; userMinimum: number }>
  >([])
  ...
  const addTargetPropagation = () => {
    ...
    setTargetPropagations([...targetPropagations, { target: newTarget.trim(), userMinimum: 1 }])
    setNewTarget('')
  }
  ...
  const updateTargetConcurrency = (target: string, value: number) => {
    setTargetPropagations(targetPropagations.map((tp) => (tp.target === target ? { ...tp, userMinimum: value } : tp)))
  }
  ...
  const addEditingTargetPropagation = () => {
    ...
    setEditingTargetPropagations([...editingTargetPropagations, { target: editingNewTarget.trim(), userMinimum: 1 }])
    setEditingNewTarget('')
  }
  ...
  const updateEditingTargetConcurrency = (target: string, value: number) => {
    setEditingTargetPropagations(
      editingTargetPropagations.map((tp) => (tp.target === target ? { ...tp, userMinimum: value } : tp)),
    )
  }
                      ...
                      <div key={tp.target} className="border rounded-md p-3 space-y-2">
                        ...
                        <div className="grid grid-cols-[auto_1fr] gap-4 items-center">
                          ...
                          <div className="flex flex-col">
                            <span className="text-right font-medium">{tp.userMinimum}</span>
                            <Slider
                              value={[tp.userMinimum]}
                              min={0}
                              max={100}
                              step={1}
