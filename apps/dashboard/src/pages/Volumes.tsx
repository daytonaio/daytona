...
import { OrganizationRolePermissionsEnum, Volume, VolumeState } from '@daytonaio/api-client'
...
const Volumes: React.FC = () => {
  ...
  useEffect(() => {
    const handleVolumeCreatedEvent = (volume: Volume) => {
      if (!volumes.some((v) => v.id === volume.id)) {
        setVolumes((prev) => [volume, ...prev])
      }
    }

    const handleVolumeStateUpdatedEvent = (data: { volume: Volume; oldState: VolumeState; newState: VolumeState }) => {
    ...
    const handleVolumeLastUsedAtUpdatedEvent = (volume: Volume) => {
      if (!volumes.some((v) => v.id === volume.id)) {
        setVolumes((prev) => [volume, ...prev])
      } else {
  ...
  const handleBulkDelete = async (volumes: Volume[]) => {
