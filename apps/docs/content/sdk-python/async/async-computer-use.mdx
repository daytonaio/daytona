...
async def get_process_status(process_name: str) -> None
```
...
async def restart_process(process_name: str) -> None
```
...
async def get_process_logs(process_name: str) -> None
```
...
async def get_process_errors(process_name: str) -> None
```
...
async def type(text: str, delay: Optional[int] = None) -> KeyboardTypeResponse
```
...
async def hotkey(keys: str) -> KeyboardHotkeyResponse
```
