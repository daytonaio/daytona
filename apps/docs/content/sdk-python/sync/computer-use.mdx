...
def get_process_status(process_name: str) -> None
```
...
def get_process_logs(process_name: str) -> None
```
...
def get_process_errors(process_name: str) -> None
```
...
def scroll(x: int,
           y: int,
           direction: str,
           amount: int = 1) -> MouseScrollResponse
```
...
def type(text: str, delay: Optional[int] = None) -> KeyboardTypeResponse
```
...
def press(key: str,
          modifiers: Optional[List[str]] = None) -> KeyboardPressResponse
```
...
```python
# Press Enter
enter = sandbox.computer_use.keyboard.press("Return")

# Press Ctrl+C
copy = sandbox.computer_use.keyboard.press("c", ["ctrl"])

# Press Ctrl+Shift+T
new_tab = sandbox.computer_use.keyboard.press("t", ["ctrl", "shift"])
```
...
def hotkey(keys: str) -> KeyboardHotkeyResponse
```
...
```python
# Copy
copy = sandbox.computer_use.keyboard.hotkey("ctrl+c")

# Paste
paste = sandbox.computer_use.keyboard.hotkey("ctrl+v")

# Alt+Tab
alt_tab = sandbox.computer_use.keyboard.hotkey("alt+tab")
```
