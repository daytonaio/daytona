...
```ts
hotkey(keys: string): Promise<boolean>
```
...
- `Promise<boolean>` - Whether the hotkey operation was successful
...
```ts
press(key: string, modifiers?: string[]): Promise<boolean>
```
...
- `Promise<boolean>` - Whether the press operation was successful
...
```ts
type(text: string, delay?: number): Promise<boolean>
```
...
- `Promise<boolean>` - Whether the type operation was successful
