...
await git.clone('https://github.com/user/repo.git', 'sandbox/repo')
```
...
await git.clone('https://github.com/user/repo.git', 'sandbox/repo-old', (commitId = 'abc123'))
```
...
await git.commit('sandbox/repo', 'Update documentation', 'John Doe', 'john@example.com')
```
...
await git.pull('sandbox/repo', 'user', 'token')
```
...
await git.push('sandbox/repo', 'user', 'token')
```
