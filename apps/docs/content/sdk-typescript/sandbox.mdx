...
```ts
static toWorkspaceInfo(instance: Sandbox): SandboxInfo
```
...
- `instance` _Sandbox_ - The API workspace instance to convert
...
- `backupCreatedAt` _null \| string_ - When the backup was created
- `backupState` _null \| string_ - Current state of Sandbox backup
- `class` _CreateRunnerClassEnum_ - Sandbox class
- `created` _string_ - The creation timestamp of the project
  - _Inherited from_: `SandboxInfo.created`
...
- `lastBackup` _null \| string_ - When the last backup was created
- `nodeDomain` _string_ - Domain name of the Sandbox node
- ~~`providerMetadata?`~~ _string_ - **_Deprecated_** - Use `state`, `nodeDomain`, `region`, `class`, `updatedAt`, `lastBackup`, `resources`, `autoStopInterval`, `autoArchiveInterval` instead.
- `public` _boolean_ - Whether the Sandbox is publicly accessible
- `region` _CreateRunnerRegionEnum_ - Region of the Sandbox node
- `resources` _Resources_ - Resource allocations for the Sandbox
...
- `autoArchiveInterval?` _number_ - Auto-archive interval in minutes
  - _Inherited from_: `Sandbox.autoArchiveInterval`
- `autoStopInterval?` _number_ - Auto-stop interval in minutes (0 means disabled)
  - _Inherited from_: `Sandbox.autoStopInterval`
- `backupCreatedAt?` _string_ - The creation timestamp of the last backup
  - _Inherited from_: `Sandbox.backupCreatedAt`
- `backupState?` _SandboxBackupStateEnum_ - The state of the backup
  - _Inherited from_: `Sandbox.backupState`
- `cpu?` _number_ - The CPU quota for the sandbox
  - _Inherited from_: `Sandbox.cpu`
- `disk?` _number_ - The disk quota for the sandbox
  - _Inherited from_: `Sandbox.disk`
- `env` _\[key: string\]: string_ - Environment variables for the sandbox
  - _Inherited from_: `Sandbox.env`
- `errorReason?` _string_ - The error reason of the sandbox
  - _Inherited from_: `Sandbox.errorReason`
- `gpu?` _number_ - The GPU quota for the sandbox
  - _Inherited from_: `Sandbox.gpu`
- `id` _string_ - The ID of the sandbox
  - _Inherited from_: `Sandbox.id`
- `info?` _SandboxInfo_
- `labels` _\[key: string\]: string_ - Labels for the sandbox
  - _Inherited from_: `Sandbox.labels`
- `memory?` _number_ - The memory quota for the sandbox
  - _Inherited from_: `Sandbox.memory`
- `~~name~~` _string_ - The name of the sandbox
  - _Inherited from_: `Omit.name`
- `organizationId` _string_ - The organization ID of the sandbox
  - _Inherited from_: `Sandbox.organizationId`
- `public` _boolean_ - Whether the sandbox http preview is public
  - _Inherited from_: `Sandbox.public`
- `snapshot?` _string_ - The snapshot used for the sandbox
  - _Inherited from_: `Sandbox.snapshot`
- `state?` _SandboxState_ - The state of the sandbox
  - _Inherited from_: `Sandbox.state`
- `target` _string_ - The target environment for the sandbox
  - _Inherited from_: `Sandbox.target`
- `user` _string_ - The user associated with the project
  - _Inherited from_: `Sandbox.user`
- `volumes?` _SandboxVolume\[\]_ - Array of volumes attached to the sandbox

  Sandbox

  - _Inherited from_: `Sandbox.volumes`
