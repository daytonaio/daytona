...
Daytona provides options to create API keys in [Daytona Dashboard ↗](https://app.daytona.io/dashboard/keys) or programmatically using the API.
...
Daytona provides options to delete an API key by name. The key is revoked immediately and cannot be recovered.
