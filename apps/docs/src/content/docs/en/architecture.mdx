...
Public sandboxes allow unauthenticated access to application ports, while internal ports always require authentication. For private sandboxes, the proxy supports multiple authentication methods: bearer tokens in the Authorization header, preview tokens in a custom header, query parameter tokens, signed preview URL tokens with expiration, and secure cookies scoped per sandbox. The proxy tracks sandbox activity by updating the last-activity timestamp on each request, which feeds into the automated lifecycle management ([auto-stop](/docs/en/sandboxes#auto-stop-interval)) timer.

#### Snapshot builder

The snapshot builder is part of the API process and orchestrates the creation of sandbox [snapshots](/docs/en/snapshots) from a Dockerfile or a pre-built image from a [container registry](#container-registry). It coordinates with runners to build or pull images, which are then pushed to an internal snapshot registry that implements the OCI distribution specification. All images are built for the `linux/amd64` platform. Build logs are stored and can be streamed in real time.
...
Resource isolation is enforced per sandbox:

- **CPU and memory**: cgroups quotas limit CPU time and memory usage per sandbox, swap is disabled, preventing sandboxes from consuming swap space
- **Disk**: XFS project quotas enforce per-sandbox disk limits on runners using XFS-backed storage
- **Network**: per-sandbox iptables chains control egress traffic: network policies support full egress blocking or allow-list rules by CIDR
