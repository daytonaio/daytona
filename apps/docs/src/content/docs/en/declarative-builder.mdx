...
```go
// Define a declarative image with python packages
declarativeImage := daytona.DebianSlim("3.12").
  PipInstall([]string{"requests", "pytest"}).
  Workdir("/home/daytona")

// Create a new sandbox with the declarative image and stream the build logs
sandbox, err := client.Create(&types.CreateSandboxFromImageParams{
  Image: declarativeImage,
}, &types.CreateOptions{
  Timeout: 0,
...
// Create the snapshot and stream the build logs
client.Snapshot.Create(&types.CreateSnapshotParams{
  Name:  snapshotName,
  Image: image,
}, &types.CreateSnapshotOptions{
...
// Create a new sandbox using the pre-built snapshot
sandbox, err := client.Create(&types.CreateSandboxFromSnapshotParams{
  Snapshot: snapshotName,
})
```
...
```go
// Add pip packages
image := daytona.DebianSlim("3.12").PipInstall([]string{"requests", "pandas"})
...
```go
// Add a local file
image := daytona.DebianSlim("3.12").AddLocalFile("package.json", "/home/daytona/package.json")

// Add a local directory
image := daytona.DebianSlim("3.12").AddLocalDir("src", "/home/daytona/src")
```
...
```go
// Set environment variables
image := daytona.DebianSlim("3.12").Env("PROJECT_ROOT", "/home/daytona")

// Set working directory
image := daytona.DebianSlim("3.12").Workdir("/home/daytona")
```
...
// Set entrypoint
image := daytona.DebianSlim("3.12").Entrypoint([]string{"/bin/bash"})

// Set default command
image := daytona.DebianSlim("3.12").Cmd([]string{"/bin/bash"})
```
...
```go
// Note: The Go SDK does not support arbitrary Dockerfile commands
// Use the Run() method for shell commands instead

// Use an existing Dockerfile
image := daytona.FromDockerfile("Dockerfile")

// Extend an existing Dockerfile with additional commands
image := daytona.FromDockerfile("app/Dockerfile").
