...
<Tabs>
  ...
  <TabItem label="TypeScript" icon="seti:typescript">
    ...
    // Create a Python image with common data science packages
    let image = Image.debianSlim('3.12')
    ...
    image = await image.dockerfileCommands(['USER daytona'])
    image = image
      .workdir('/home/daytona/workspace')
      .env({
          MY_ENV_VAR: 'My Environment Variable',
      })
    image = await image.addLocalFile(localFilePath, '/home/daytona/workspace/file_example.txt')
