...
# List sandboxes by label and pick one
result = daytona.list(labels={"project": "my-app"})
...
// List sandboxes by label and pick one
const result = await daytona.list({ project: 'my-app' });
...
func main() {
    ...
    // List sandboxes by label and pick one
    result, err := client.List(context.Background(), map[string]string{"project": "my-app"}, nil, nil)
    if err != nil {
        log.Fatal(err)
    }
    found := result.Items[0]
...
# List sandboxes by label and pick one
result = daytona.list({ 'project' => 'my-app' })
found = result.items[0]
