...
<Tabs syncKey="language">
  ...
  <TabItem label="Ruby" icon="seti:ruby">
    ...
    # Run code securely inside the Sandbox
    response = sandbox.process.code_run(code: 'print("Sum of 3 and 4 is " + str(3 + 4))')
    if response.exit_code != 0
...
<Tabs syncKey="language">
  ...
  <TabItem label="Ruby" icon="seti:ruby">
    ...
    sandbox.process.execute_session_command(
      exec_session_id,
      Daytona::SessionExecuteRequest.new(command: 'python app.py', var_async: true)
    )
...
<Tabs syncKey="language">
  ...
  <TabItem label="Ruby" icon="seti:ruby">
    ...
    def get_claude_response(api_key, prompt)
      ...
      return "Error #{response.code}: #{response.message}" unless response.is_a?(Net::HTTPSuccess)
    ...
    # Extract code from the response using regex
    code_match = result.match(/```python\n(.*?)```/m)

    code = code_match ? code_match[1] : result
    code = code.gsub('\\', '\\\\')
