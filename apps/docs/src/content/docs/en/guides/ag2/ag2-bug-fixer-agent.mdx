    ...
    with DaytonaCodeExecutor(timeout=60) as executor:
        bug_fixer = ConversableAgent(
            ...
            is_termination_msg=lambda x: (
                "TERMINATE" in x.get("content", "") or not x.get("content", "").strip()
            ),
        )
...
Assign the return value of `initiate_chat` to a variable to access more details about the session:

```python
result = code_executor.initiate_chat(recipient=bug_fixer, message=message, max_turns=8)

result.chat_history  # full message exchange between agents
result.cost          # token usage and cost breakdown per model
result.summary       # conversation summary (requires summary_method to be set)
```
:::
...
`recipient=bug_fixer` in `initiate_chat` is what connects the two agents. AG2 sets up a managed back-and-forth loop between them — after each reply, the message is automatically forwarded to the other agent. The agents have no direct reference to each other outside of that call.

Tracing the session above step by step:

1. `code_executor.initiate_chat(recipient=bug_fixer, ...)` — AG2 starts the loop and `code_executor` sends the broken code as plain text to `bug_fixer`. Nothing is executed yet.
