...
The agent has 11 tools and a sandbox. On the first prompt it calls `createSandbox`, then chains shell commands, file writes, and filesystem queries until the user's request is done. When the user asks for something they should see in a browser, the agent starts a dev server in the background and then calls `getPreviewUrl` to surface the running app as a live iframe inside the chat.
...
`runCommand` is the most heavily-used tool. The `background` flag flips between a synchronous `executeCommand` (blocks until exit, returns stdout) and an asynchronous `executeSessionCommand({ runAsync: true })` (returns immediately, leaves the process running). Dev servers go through the background path; everything else uses the synchronous one:

```typescript
const runCommand = defineTool({
  name: 'runCommand',
  description:
    'Execute a shell command in the sandbox. Set background:true for dev servers and other long-lived processes. Use plain commands (rm, mv, mkdir, chmod, ...) for filesystem ops that do not need structured output.',
  parameters: z.object({
    sandboxId: z.string(),
...
`getPreviewUrl` is the bridge to the iframe-in-chat experience. It returns the public proxy URL for the given port; the frontend renderer then embeds that URL as an `<iframe>` in the chat card:
...
Each render hook references a named Zod schema and a result type declared at the top of `app/page.tsx`. The `getPreviewUrl` renderer is the one users notice most: while the tool is `inProgress`/`executing` it shows a shimmering skeleton, and once `complete` it flips to a live `<iframe>` pointing at the Daytona preview URL:
