...
This installs the DSPy framework and the Daytona SDK. To also run the included demo (which plots results with matplotlib), use `pip install -e ".[demo]"` instead.
...
You can also pass host-side Python functions into the sandbox via the `tools` dict. These are bridged through the same broker mechanism as `llm_query()`:
...
Inside the sandbox, the LLM calls `search_knowledge_base(...)` as if it were a local function. The call is routed to the host through the broker, executed, and the result is returned to the sandbox.
...
State persists across iterations — variables, imports, and function definitions all carry over. You can also pass host-side Python functions into the sandbox as custom tools via the `tools` dict, bridged through the same broker mechanism as `llm_query()`.
...
These execute on the host (they need LLM API access) and are bridged into the sandbox transparently. From the generated code's perspective they are ordinary Python functions that take strings and return strings. This is what makes the pattern powerful: the LLM can write a `for` loop over 100 chapters, call `llm_query_batched()` to extract structured data from each one in parallel, then aggregate the results with pandas — mixing arbitrary computation with natural-language understanding in a single program.
...
- **Tool bridging** — Host-side functions are available inside the sandbox via the same broker mechanism
