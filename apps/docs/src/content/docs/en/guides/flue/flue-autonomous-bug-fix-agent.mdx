...
This guide builds an autonomous bug-fix agent using [Flue](https://flueframework.com/) (a runtime-agnostic agent harness framework) together with [Daytona](https://www.daytona.io/) sandboxes. Given a GitHub issue URL, the agent reproduces the bug with a failing test, implements the minimal fix, runs the full test suite, and opens a real pull request.

A sandbox is non-negotiable for this workflow. The agent clones unknown code, installs unknown dependencies, and executes the project's test suite. Doing any of that on your laptop is a security and stability risk. Daytona runs every iteration in a clean Linux container and tears it down on completion, so an untrusted repo can never affect your host.
...
The first terminal streams the orchestrator's setup logs as the agent runs. When it finishes, the response body contains the structured result.
...
That maps to `flue run bug-fix --target node --id run-1 --env .env --payload '{}'`. Unlike Options A and B, this **builds and spawns its own ephemeral server**, POSTs with `Accept: text/event-stream`, and decorates every agent event (`tool:start`, `tool:done`, the LLM's reasoning text) into a readable line on stderr. The final structured result lands on stdout. See the [Example Walkthrough](#4-example-walkthrough) below for a real `npm run run` trace.
...
The user owns the Daytona client lifecycle (you decide how the sandbox is created, reused, or cleaned up); Flue just adapts it for agent use. The `cleanup: true` option deletes the sandbox automatically when the session ends.

#### The Orchestrator

The agent file (`.flue/agents/bug-fix.ts`) is small on purpose. It provisions the sandbox, prepares the environment, and hands off to a skill that does the real work. Here's the shape (see the full source for env validation, error checks, and package-manager detection):
...
export default async function ({ init, payload, env }: FlueContext) {
  ...
  const client = new Daytona({ apiKey: requireEnv(env, 'DAYTONA_API_KEY') })
  // Set GH_TOKEN as a sandbox-level env var so every gh/git invocation inside
  // the sandbox inherits it without scope-validation surprises.
  const sandbox = await client.create({
    envVars: { GH_TOKEN: requireEnv(env, 'GITHUB_TOKEN') },
  })

  const setupAgent = await init({
    sandbox: daytona(sandbox, { cleanup: true }),
    model: env.MODEL ?? 'anthropic/claude-sonnet-4-6',
  })
  const setup = await setupAgent.session()

  // Configure git's credential helper. We avoid `gh auth login --with-token`
  // because it requires the `read:org` scope, which classic `repo`-only PATs
  // don't include. `gh auth setup-git` only edits git config and works as
  // long as GH_TOKEN is in the env.
  await setup.shell('gh auth setup-git')
  await setup.shell(`gh repo clone ${repo} /home/daytona/project`)
  const packageManager = await detectPackageManager(setup)
  await setup.shell(`${packageManager} install`, { cwd: '/home/daytona/project' })

  // Issues live on the upstream parent (forks have issues disabled by default).
  const issueRepo =
    parsed.issueRepo ?? env.ISSUE_REPO ?? (await detectUpstream(setup, repo)) ?? repo
  const issueData = await setup.shell(
    `gh issue view ${issueNumber} --repo ${issueRepo} --json title,body,number,url`,
  )
  ...
  return await session.skill('bug-fix', {
    args: { issueNumber, issueData: issueData.stdout, repo, issueRepo, packageManager },
...
- **Two agents, one sandbox.** The setup agent installs `gh`, configures auth, clones the repo, and installs dependencies. A second agent (given a different `id` and `cwd`) operates inside the cloned repo and discovers `AGENTS.md` and skills from there. Both share the same Daytona sandbox.
...
- **Package-manager auto-detection.** The setup phase reads the project's lockfile (`pnpm-lock.yaml`, `yarn.lock`, `bun.lock(b)`, or falls back to `npm`) and uses the matching tool throughout. The detected name is also passed into the skill so the LLM uses the right test command.
...
Or use Flue's one-shot CLI invoker, which handles SSE for you and prints the result to stdout (this is what `npm run run` calls):
...
| Runtime target | Storage | Survives restarts? |
|---|---|---|
| `flue dev --target node` (this guide) | In-memory, default `SessionStore` | No: stop the dev server and all sessions are gone |
| `flue dev --target cloudflare` | Cloudflare Durable Objects | Yes: across requests, deploys, weeks/months |
| Your own `init({ persist: customStore })` | Whatever you back it with (Redis, Postgres, etc.) | Up to you |

For local development against `flue dev` on Node, sessions are wiped every time you Ctrl+C the dev server, so reusing an `<id>` only matters within one server lifetime.
...
That runs `flue run bug-fix --target node --id run-1 --env .env --payload '{}'`, which builds, spawns an ephemeral server, POSTs with `Accept: text/event-stream`, and decorates each agent event into a readable line on stderr. Below is a trimmed real run against a fork of `vercel/ms`. The orchestrator's `[bug-fix] ...` lines come from our setup phase; the `[flue] tool:start/done ...` lines and inline italicized text are the LLM's tool calls and reasoning streamed back via SSE:
...
[flue] tool:start  read  /home/daytona/project/AGENTS.md
[flue] tool:done   read  (1477 chars)
[flue] tool:done   read  (626 chars)
...
The Daytona sandbox auto-deletes when the agent's session ends, thanks to the `cleanup: true` option passed to the connector. Confirm in your [Daytona Dashboard](https://app.daytona.io/dashboard); there should be no leftover sandboxes after a successful run.
