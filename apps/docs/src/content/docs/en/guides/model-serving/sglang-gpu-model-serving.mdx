...
That `max_tokens: 4096` on a haiku request is not a typo. gpt-oss reasons before it answers, and `max_tokens` covers reasoning plus answer combined. If thinking exhausts the budget, the request returns `finish_reason: "length"` with the truncated trace in `reasoning_content` and `content: null`, which looks like the model returned nothing. The thinking length also varies between identical runs: the same haiku prompt measured anywhere from 400 to over 1000 reasoning tokens. Give requests generous budgets, or turn `reasoning_effort` down for simple tasks.
:::

The subsections below drive that same client through SGLang's features, each a runnable example in `query_openai.py`: reasoning effort, structured output, tool calling, and prefix caching.
...
The dial matters in both directions: `"high"` buys more careful answers on hard problems, while `"low"` makes simple tasks faster, cheaper, and less variable, which is why the classification workload below runs at low effort.
...
#### Scaling up: gpt-oss-120b on one GPU
...
### 8. Going Further: One Endpoint, Many Sandboxes
