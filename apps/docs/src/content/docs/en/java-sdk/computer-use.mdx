...
- `format` _String_ - output image format (for example: {@code png}, {@code jpeg}, {@code webp})
- `quality` _int_ - compression quality (typically 1-100, format dependent)
- `scale` _double_ - screenshot scale factor (for example: {@code 0.5} for 50%)
...
- `x` _int_ - target X coordinate
- `y` _int_ - target Y coordinate
- `button` _String_ - button type ({@code left}, {@code right}, {@code middle})
...
- `deltaX` _int_ - horizontal delta (used only when {@code deltaY == 0})
...
- `key` _String_ - key to press (for example: {@code Enter}, {@code Escape}, {@code a})
