...
screenshots, mouse and keyboard automation, display/window inspection, screen recording,
and AT-SPI accessibility.
...
#### findAccessibilityNodes()
```java
public AccessibilityNodesResponse findAccessibilityNodes(String scope, Integer pid, String role, String name, String nameMatch, List<String> states, Integer limit)
```
...
- `scope` _String_ - optional scope (`focused`, `pid`, or `all`)
- `pid` _Integer_ - process ID when `scope` is `pid`
- `role` _String_ - optional node role filter
- `name` _String_ - optional accessible name filter
- `nameMatch` _String_ - optional name match mode (`exact`, `substring`, or `regex`)
- `states` `List<String>` - optional state filters
- `limit` _Integer_ - optional result limit
