...
- `mode` _String_ - POSIX mode (for example {@code 755}); defaults to {@code 755} when {@code null}
...
- `content` _byte[]_ - file bytes; {@code null} uploads an empty file
...
- `Map\<String, Object\>` - result map containing {@code files}
