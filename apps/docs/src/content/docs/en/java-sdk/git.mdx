...
- `url` _String_ - repository URL
- `path` _String_ - destination path in the Sandbox
- `branch` _String_ - branch to clone; {@code null} uses default branch
- `commitId` _String_ - commit SHA to checkout after clone; {@code null} skips detached checkout
...
- `Map\<String, Object\>` - map containing {@code branches} list
