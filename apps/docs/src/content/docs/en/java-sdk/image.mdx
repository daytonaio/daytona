...
- `baseImage` _String_ - base image reference (for example {@code python:3.12-slim-bookworm})

**Returns**:

- `Image` - new {@link Image} initialized with a {@code FROM} instruction
...
- `pythonVersion` _String_ - Python version to use; defaults to {@code 3.11} when {@code null} or empty

**Returns**:

- `Image` - new {@link Image} using a Python slim base image
...
- `Image` - this {@link Image} for method chaining
