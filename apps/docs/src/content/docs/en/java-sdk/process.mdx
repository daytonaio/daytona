...
- `command` _String_ - command to execute
- `cwd` _String_ - working directory, or {@code null} to use sandbox default
...
Uploads code to a temporary file in the Sandbox and executes it with the language runtime
selected by the `code-toolbox-language` label.
...
- `DaytonaException` - if upload or execution fails
...
- `options` _PtyCreateOptions_ - PTY options, or {@code null} to use defaults
