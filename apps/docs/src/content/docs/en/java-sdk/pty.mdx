...
- `id` _String_ - custom PTY session identifier; if {@code null}, the server generates one
...
- `String` - requested PTY session identifier, or {@code null} to auto-generate
...
- `Consumer\<byte[]\>` - PTY output callback, or {@code null} when not configured
...
- `String` - error message, or {@code null} when the session ended successfully
