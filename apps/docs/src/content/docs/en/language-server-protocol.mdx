...
Daytona SDK provides an option to create LSP servers using Python and TypeScript. The `path_to_project` starting point defaults to the current Sandbox working directory. Uses the WORKDIR specified in
the Dockerfile if present, or falling back to the user's home directory if not - e.g. `workspace/project` implies `/my-work-dir/workspace/project`, but by starting with `/`, you may define an absolute path as well.
