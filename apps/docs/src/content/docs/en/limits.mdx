...
All errors include `headers` and `statusCode` properties, allowing you to access rate limit headers directly from the error object:
...
try {
  ...
  if (error instanceof DaytonaRateLimitError) {
    console.log(error.headers['x-ratelimit-remaining-sandbox-create'])
    console.log(error.headers['retry-after-sandbox-create'])
  }
}
```
...
try:
    ...
    print(e.headers.get('x-ratelimit-remaining-sandbox-create'))
  ...
  while (retries < maxRetries) {
    try {
      ...
      if (error instanceof DaytonaRateLimitError && retries < maxRetries - 1) {
        ...
        const retryAfter = error.headers['retry-after-sandbox-create']
