...
The `networkAllowList` parameter is a **comma-separated list of IPv4 CIDR blocks**.

- **IPv4 only**: hostnames, domains, and IPv6 are not supported
- **CIDR required**: every entry must include a `/` prefix length (for example `/32`)
...
For IPv4, the CIDR prefix length must be an integer in the range `0` to `32` (inclusive).

- `0.0.0.0/0` allows access to all IPv4 addresses

### Common validation errors

- **Missing prefix length**: `208.80.154.232` is invalid. Use `208.80.154.232/32`
- **Invalid IPv4 address**: `example.com/32` and `999.1.2.3/32` are invalid
- **Invalid prefix length**: `10.0.0.0/33` is invalid (must be 0 to 32)
- **Too many entries**: more than 10 items will be rejected
- **Trailing commas**: avoid a trailing comma like `10.0.0.0/8,`. Empty items are skipped during validation, but they still count toward the 10-item limit because the limit is based on the number of comma-separated items
