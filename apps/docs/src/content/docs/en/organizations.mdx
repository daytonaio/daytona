...
Daytona provides options to create organizations in [Daytona Dashboard ↗](https://app.daytona.io/dashboard/organizations) or programmatically using the [API](/docs/en/tools/api/#daytona/tag/organizations).
...
Daytona provides a method to get the organization by sandbox ID.

<Tabs syncKey="language">
<TabItem label="API" icon="seti:json">

```bash
curl 'https://app.daytona.io/api/organizations/by-sandbox-id/SANDBOX_ID' \
  --header 'Authorization: Bearer YOUR_API_KEY'
```
...
> [**Get organization by sandbox ID (API)**](/docs/en/tools/api/#daytona/tag/organizations/GET/organizations/by-sandbox-id/{sandboxId})
...
4. [Select a role](#organization-roles) for the new user. If you select the **`Member`** role, define their [assignments](#role-assignments)
...
1. Navigate to [Daytona Dashboard ↗](https://app.daytona.io/dashboard/invitations)
...
Daytona provides options to set the default region for an organization in [Daytona Dashboard ↗](https://app.daytona.io/dashboard/settings) or programmatically using the [API](/docs/en/tools/api/#daytona/tag/organizations).

1. Navigate to [Daytona Dashboard ↗](https://app.daytona.io/dashboard/)
2. Click **Settings** in the sidebar
3. In the **Organization Details** card, locate the **Default Region** row
4. Click the **Set Region** button
5. Select a region and click **Save**
...
Daytona provides a method to get the region quota for a sandbox by ID.

<Tabs syncKey="language">
<TabItem label="API" icon="seti:json">

```bash
curl 'https://app.daytona.io/api/organizations/region-quota/by-sandbox-id/SANDBOX_ID' \
  --header 'Authorization: Bearer YOUR_API_KEY'
```
...
> [**Get region quota by sandbox ID (API)**](/docs/en/tools/api/#daytona/tag/organizations/GET/organizations/region-quota/by-sandbox-id/{sandboxId})

### Update region quota

Daytona provides a method to update the region quota for an organization.
...
> [**Update organization region quota (API)**](/docs/en/tools/api/#daytona/tag/organizations/PATCH/organizations/{organizationId}/quota/{regionId})
...
Daytona provides a method to suspend an organization.
...
> [**Suspend organization (API)**](/docs/en/tools/api/#daytona/tag/organizations/POST/organizations/{organizationId}/suspend)

### Unsuspend organization

Daytona provides a method to unsuspend an organization.
...
> [**Unsuspend organization (API)**](/docs/en/tools/api/#daytona/tag/organizations/POST/organizations/{organizationId}/unsuspend)
...
Daytona provides a method to update the quota for an organization.
...
> [**Update organization quota (API)**](/docs/en/tools/api/#daytona/tag/organizations/PATCH/organizations/{organizationId}/quota)
...
Daytona provides a method to get the OTEL configuration by sandbox authentication token.

<Tabs syncKey="language">
<TabItem label="API" icon="seti:json">

```bash
curl 'https://app.daytona.io/api/organizations/otel-config/by-sandbox-auth-token/AUTH_TOKEN' \
  --header 'Authorization: Bearer YOUR_API_KEY'
```
...
> [**Get organization OTEL config by sandbox auth token (API)**](/docs/en/tools/api/#daytona/tag/organizations/GET/organizations/otel-config/by-sandbox-auth-token/{authToken})