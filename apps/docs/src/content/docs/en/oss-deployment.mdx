...
| `BUILD_INFO_MAX_SANDBOXES_PER_RUNNER`      | number  | `30`                                                 | Max active sandboxes per runner for the same declarative build (`buildInfoSnapshotRef`); runners at or above this cap are excluded when picking a runner. Set to `0` to disable the limit. |
