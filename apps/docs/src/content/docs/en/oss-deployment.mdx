...
| `LAYERED_API_KEY`                          | string  | (empty)                                              | API key for the layered (in-container) volume backend control plane. Blank disables the layered backend (volumes fall back to s3fuse).                    |
| `LAYERED_DEFAULT_REGION`                   | string  | `aws-us-east-1`                                      | Default region for newly created layered volumes (e.g. `aws-us-east-1`, `aws-eu-west-1`, `aws-us-west-2`, `gcp-us-central1`)                              |
| `LAYERED_DEFAULT_STORAGE_REGION`           | string  | `$LAYERED_DEFAULT_REGION`                            | Fallback storage region used when a layered volume's region has no storage region set                                                                     |
| `LAYERED_CONTROL_URL_<REGION>`             | string  | (per-region default)                                 | Per-region control-plane base URL override, e.g. `LAYERED_CONTROL_URL_AWS_US_EAST_1`. Falls back to the built-in default for that region                  |
...
| `LAYERED_BINARY_PATH`                   | string  | (empty)                           | Host path to layered mount CLI; empty disables it     |
