...
Daytona provides methods to execute shell commands in sandboxes by providing the command string and optional parameters for working directory, timeout, and environment variables. Default timeout is 6 minutes. You can also use the `daytona exec` CLI command for quick command execution.
