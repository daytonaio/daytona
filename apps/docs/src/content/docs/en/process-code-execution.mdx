...
Daytona provides methods to manage the entrypoint session in sandboxes. Each sandbox has an internal entrypoint session created to run the configured entrypoint command. You can retrieve the entrypoint session details and read its logs.
...
session.commands.forEach((cmd) => console.log(`Command: ${cmd.command}`));
