...
@with_instrumentation()
async def get_tree(scope: str | None = None, pid: int | None = None, max_depth: int | None = None) -> AccessibilityTreeResponse
```
...
- `scope` _str | None_ - Optional scope (`focused`, `pid`, or `all`).
- `pid` _int | None_ - Process ID when `scope` is `pid`.
- `max_depth` _int | None_ - Optional max tree depth. Use `0` for the root only.
...
```python
tree = await sandbox.computer_use.accessibility.get_tree(scope="focused", max_depth=2)
```
...
@with_instrumentation()
async def find_nodes(scope: str | None = None, pid: int | None = None, role: str | None = None, name: str | None = None, name_match: str | None = None, states: list[str] | None = None, limit: int | None = None) -> AccessibilityNodesResponse
```
...
- `scope` _str | None_ - Optional scope (`focused`, `pid`, or `all`).
- `pid` _int | None_ - Process ID when `scope` is `pid`.
- `role` _str | None_ - Optional node role filter.
- `name` _str | None_ - Optional accessible name filter.
- `name_match` _str | None_ - Optional name match mode (`exact`, `substring`, or `regex`).
- `states` _list[str] | None_ - Optional state filters.
- `limit` _int | None_ - Optional result limit.
