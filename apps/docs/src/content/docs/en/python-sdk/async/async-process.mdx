...
async def get_entrypoint_session() -> SessionDTO
```
...
async def get_session_command(session_id: str, command_id: str) -> CommandDTO
```
...
```python
@intercept_errors(message_prefix="Failed to get entrypoint logs: ")
async def get_entrypoint_logs_async(on_stdout: Callable[[str], None],
                                    on_stderr: Callable[[str], None]) -> None
```
