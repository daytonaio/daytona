...
- `resizing` _bool_ - Whether a resize operation is in progress.
...
async def resize(resources: Resources,
                 hot: bool = False,
                 timeout: float | None = 60) -> None
```
...
- `hot` _bool_ - If True, performs hot resize on a running sandbox (only CPU/memory increase allowed).
