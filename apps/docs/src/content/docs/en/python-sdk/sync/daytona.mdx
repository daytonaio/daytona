...
@deprecated(
    reason=
    "Method is deprecated. Use `get` instead. Filtering by labels will be removed in a future version."
)
...
def list(labels: Annotated[
    Optional[Dict[str, str]],
    Field(
        default=None,
        deprecated=
        "`labels` is deprecated and will be removed in a future version. Use `name` instead.",
    ),
] = None,
         page: Optional[int] = None,
         limit: Optional[int] = None,
         name: Optional[str] = None) -> PaginatedSandboxes
```
...
- `name` _Optional[str]_ - Filter by partial name match
...
```python
result = daytona.list(page=2, limit=10, name="my-value")
