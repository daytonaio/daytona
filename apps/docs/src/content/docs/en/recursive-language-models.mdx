...
This guide demonstrates how to build a recursive language model (RLM) agent system on Daytona sandboxes, based on the approach pioneered in [Recursive Language Models](https://arxiv.org/abs/2512.24601) (Zhang, Kraska, Khattab) and further explored by [Prime Intellect](https://www.primeintellect.ai/blog/rlm).
...
4. **Aggregate**: Sub-agents return results; parent synthesizes findings
5. **Complete**: Root agent produces a git patch; all sandboxes are cleaned up
...
Each agent runs in its own isolated Daytona sandbox with a fresh repository clone, enabling true parallel exploration.
...
This spawns a root agent that explores the codebase, delegates to sub-agents for parallel investigation, and produces a git patch fixing the easiest TODO it finds. We'll walk through the results and trace the execution in detail later — first, let's look at how the code works.
...
Let's trace what happens when we run an agent on scikit-learn:
...
Some depth-1 agents decide their module is too large and spawn their own sub-agents. For example, the `sklearn/metrics/` agent spawned 5 depth-2 agents:
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    tasks = [
        ...
        "Identify and assess TODOs in 'sklearn/metrics/_pairwise_distances_reduction/'.",
        "Identify and assess TODOs in remaining files in 'sklearn/metrics/' (e.g., _classification.py, _scorer.py)."
    ]
...
| `rlm.max_sandboxes` | 50 | Maximum concurrent sandboxes (global budget) |
...
- The sandbox budget is shared across the entire agent tree
- Sub-agent sandboxes are deleted immediately after completion to free budget
:::
