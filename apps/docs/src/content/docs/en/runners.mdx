...
Customer managed compute enables you to run sandbox workloads on your own infrastructure while using Daytona's control plane to manage them. Runners are machines that power Daytona's compute plane and provide the underlying infrastructure.

Runners are grouped into regions, and each sandbox is assigned to a single region. Daytona provides two [built-in regions](/docs/en/regions) (**US** and **EU**) for shared compute resources and handling provisioning, maintenance, monitoring, and scaling on your behalf.

With customer managed compute, you can define one or more custom regions and assign your own runner nodes: instances running the sandbox workloads. Custom regions can also include a region-specific sandbox proxy service and snapshot manager service.
...
Daytona provides official [Terraform modules](#terraform) and [Helm charts](#helm-charts) for automated runner deployment.
