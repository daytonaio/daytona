...
- Daytona SDK API calls (toolbox actions connecting to the sandbox)
...
**Recommendation**: If your workflow relies on long-running background tasks, ensure there is a form of external interaction (such as polling logs via the SDK) to prevent the sandbox from auto-stopping.
