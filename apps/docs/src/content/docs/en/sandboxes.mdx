...
4. Select a [snapshot](/docs/en/snapshots): if not specified, the [default snapshot](/docs/en/snapshots#default-snapshots) will be used
5. Select a [region](/docs/en/regions): if not specified, your organization's default region will be used
6. Define [resources](/docs/en/sandboxes#resources): if not specified, default values will be used: 1 vCPU, 1GB RAM, 3GiB disk
7. Define sandbox lifecycle management options: [auto-stop](#auto-stop-interval), [auto-archive](#auto-archive-interval), [auto-delete](#auto-delete-interval)
8. Add environment variables in key-value pairs
9. Add labels in key-value pairs
10. Select network settings:
...
11. Click the **Create Sandbox** button to create the sandbox
