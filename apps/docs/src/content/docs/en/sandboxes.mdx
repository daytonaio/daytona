...
## Auto-resume Sandboxes

:::caution[Experimental]
This feature is experimental. To request access, contact [support@daytona.io](mailto:support@daytona.io).
:::

Daytona provides methods to auto-resume sandboxes from a stopped state. Once a sandbox is stopped, it remains in this state until it is [manually started](#start-sandboxes) again or auto-resumed. Auto-resuming starts the sandbox and resumes it from the last known state.

To configure auto-resume, set the `wakeOnRequest` field when [creating a sandbox](#create-sandboxes):

- `none`: the sandbox remains stopped until it is manually started again
- `http`: auto-resumes the sandbox on [preview URL](/docs/en/preview) requests
- `ssh`: auto-resumes the sandbox on [SSH access](/docs/en/ssh-access) requests
- `http_and_ssh`: auto-resumes the sandbox on both [preview URL](/docs/en/preview) and [SSH access](/docs/en/ssh-access) requests
...
Daytona provides methods to fork sandboxes using [Daytona Dashboard ↗](https://app.daytona.io/dashboard/). Forking creates a duplicate of your sandbox's filesystem and copies it into a new sandbox. The new sandbox is fully independent: it can be started, stopped, and deleted without affecting the original. However, Daytona tracks the parent-child relationship in a fork tree, so you can always trace a fork's lineage back to the sandbox it was created from. You can fork a fork, building out branches as needed. If the parent sandbox is deleted, all its forks continue to run unaffected.

Forking and [cloning](#clone-sandboxes) both duplicate the sandbox's **filesystem state only**. The in-memory and runtime state (running processes, open connections, environment variables set at runtime) of the original sandbox is not carried over. The new sandbox starts in a stopped state with a fresh runtime. To capture a reusable environment including installed packages and file contents, consider [creating a snapshot](#create-snapshot-from-sandbox) instead.
...
Daytona provides methods to clone sandboxes using [Daytona Dashboard ↗](https://app.daytona.io/dashboard/). Cloning creates a one-off duplicate of your sandbox's filesystem. Unlike [forking](#fork-sandboxes), there is no parent-child relationship or tree structure — the result is a completely standalone copy with no tracked connection to the original.
