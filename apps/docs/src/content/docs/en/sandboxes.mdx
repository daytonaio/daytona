...
- **Networking**
  ...
  TCP connections establish from the parent sandbox to the child sandbox using the child sandbox's ID as the host. The traffic stays on the internal link network. For example: `telnet DEVICE_SANDBOX_ID 5555` reaches port `5555` on the linked child sandbox.
...
Android sandboxes are used to run Android emulators alongside a standard Linux sandbox.
...
- **Region**
  ...
  Android workloads require the `android` target region on every sandbox in the setup (base and devices). Set `DAYTONA_TARGET=android` in the environment, or pass the `android` target parameter
- **Snapshots**

  Base: `android-base`. 
  
  Devices (x86_64, AOSP, the numbers correspond to Android SDK levels / OS versions):

  `android-device-12-aosp`, `android-device-13-aosp`, `android-device-14-aosp`, `android-device-15-aosp`, `android-device-16-aosp`
- **adb from the base sandbox**

  Connect using the device sandbox ID and port `5555`: 
