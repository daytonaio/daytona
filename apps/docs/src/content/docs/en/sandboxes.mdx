...
{/* When Sandboxes are not actively used, it is recommended that they be stopped. This can be done manually [using the stop command](/docs/en/sandboxes#stop-and-start-sandbox) or automatically by [setting the auto-stop interval](/docs/en/sandboxes#auto-stop-and-auto-archive). */}
