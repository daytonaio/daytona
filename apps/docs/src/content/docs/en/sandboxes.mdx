...
Daytona provides methods to resize sandbox resources (CPU, memory, and disk) after creation. The resize operation can be performed on a running sandbox to increase CPU and memory, or on a stopped sandbox to change CPU, memory, and disk.
