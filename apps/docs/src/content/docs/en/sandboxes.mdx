...
Sandboxes are built from OCI-compliant container images. A [snapshot](/docs/en/snapshots) captures a fully configured environment: base OS, installed packages, dependencies, and configuration to create new sandboxes. Snapshots can be built from any image in a public or private container registry, or defined programmatically using the [declarative image builder](/docs/en/declarative-builder). The sandbox filesystem uses a layered overlay model where the snapshot provides the read-only base and all runtime changes are written to a dedicated layer, preserving the original snapshot for reuse.

Each sandbox has its own network stack with per-sandbox firewall rules. By default, sandboxes follow standard network policies, but you can restrict egress to a specific set of allowed destinations or block all outbound traffic entirely. Egress rate limiting is also supported. For details on configuring network access, see [Network settings](#network-settings-firewall).

For a high-level overview of the Daytona platform components, see [architecture](/docs/en/architecture).
...
Compute resources are allocated per sandbox and enforced at the kernel level. Each sandbox receives a dedicated CPU quota, a hard memory limit with swap disabled, and a disk allocation on an XFS-backed filesystem. CPU and memory can be increased on a running sandbox without interruption. Disk resizing and resource decreases require the sandbox to be stopped first.
