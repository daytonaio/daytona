...
- **Unknown**: the sandbox state is unknown and may require further attention
...
- **Block all network access**: block all outbound network access (default)
...
Daytona provides methods to resize [sandbox resources](#resources) after creation. The resize operation can be performed on a started and running sandbox to increase CPU and memory, or on a stopped sandbox to change CPU, memory, and disk.
...
For more information, see the [Python SDK](/docs/en/python-sdk/sync/sandbox#sandboxresize), [TypeScript SDK](/docs/en/typescript-sdk/sandbox#resize), [Go SDK](/docs/en/go-sdk/daytona/#Sandbox.Resize), and [Ruby SDK](/docs/en/ruby-sdk/sandbox#resize) and [API](/docs/en/tools/api/#daytona/tag/sandbox/POST/sandbox/{sandboxIdOrName}/resize) references:
