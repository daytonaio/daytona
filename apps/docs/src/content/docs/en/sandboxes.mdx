...
Daytona provides pre-built Android snapshots for creating Android sandboxes.

1. Navigate to [Daytona Dashboard ↗](https://app.daytona.io/dashboard/)
2. Click **Create Sandbox**
3. Select an Android snapshot (the numbers correspond to Android SDK levels / OS versions): `android-12`, `android-13`, `android-14`, `android-15`, `android-16`

4. Select **`android`** region
5. Click **Create** to create an Android sandbox
...
# Device sandbox: an Android 14 emulator linked to the base
...
# Drive the device from the base over the link network
base.process.exec(command: "adb connect #{device.name}:5555")
response = base.process.exec(
  command: "adb -s #{device.name}:5555 shell getprop ro.build.version.release"
)
```
...
```bash
# Create the base sandbox (a Linux machine with adb installed)
curl 'https://app.daytona.io/api/sandbox' \
  --request POST \
  --header 'Content-Type: application/json' \
  --header 'Authorization: Bearer YOUR_API_KEY' \
...
# Create the device sandbox (replace BASE_SANDBOX_ID with the id from the first response)
curl 'https://app.daytona.io/api/sandbox' \
  --request POST \
  --header 'Content-Type: application/json' \
  --header 'Authorization: Bearer YOUR_API_KEY' \
