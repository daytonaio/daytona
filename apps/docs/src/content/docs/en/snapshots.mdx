...
Optionally, set the **`snapshot`** field in your sandbox creation parameters to use the custom snapshot.
...
```go
snapshot, err := client.Snapshot.Get(ctx, "my-awesome-snapshot")
if err != nil {
    return err
}
...
services:
  web:
    ...
    ports:
      - "8080:80"
`
	if err := sandbox.FileSystem.UploadFile(ctx, []byte(composeContent), "docker-compose.yml"); err != nil {
		log.Fatal(err)
	}

	// Start Docker Compose services
	result, err := sandbox.Process.ExecuteCommand(ctx, "docker compose -p demo up -d")
	if err != nil {
		log.Fatal(err)
	}
	...
	// Check running services
	result, err = sandbox.Process.ExecuteCommand(ctx, "docker compose -p demo ps")
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println(result.Result)
	...
	// Clean up
	if _, err := sandbox.Process.ExecuteCommand(ctx, "docker compose -p demo down"); err != nil {
		log.Fatal(err)
	}
}
```
