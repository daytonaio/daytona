...
<a id="cold-snapshots"></a>
### Cold snapshots

By default, an active snapshot is distributed to runners ahead of time so that sandboxes start instantly. A **cold** snapshot is not distributed ahead of time — it is pulled on demand the first time you create a sandbox from it, so that first sandbox takes a little longer to start (it briefly shows the `pulling_snapshot` state).

Set `cold` when creating the snapshot:
...
daytona = Daytona()
snapshot = daytona.snapshot.create(
    CreateSnapshotParams(name="my-cold-snapshot", image="python:3.12", cold=True),
)
```
...
```bash
daytona snapshot create my-cold-snapshot --image python:3.12 --cold
```
...
Warm snapshots automatically become inactive after 2 weeks of not being used ([cold snapshots](#cold-snapshots) are never auto-deactivated). To activate an inactive snapshot:
...
Daytona provides an option to deactivate snapshots. Deactivating a snapshot removes its pre-distributed copies from runners to free resources. You can still create a sandbox from it afterwards via an on-demand pull, which re-activates the snapshot in the background.
...
:::note
You can still create a sandbox from an inactive snapshot: Daytona pulls it onto a runner on demand (the sandbox briefly enters the `pulling_snapshot` state), and warm snapshots are re-activated in the background so later sandboxes start instantly. This is the same on-demand flow used by [cold snapshots](#cold-snapshots). You can also [activate](#activate-snapshots) a snapshot manually to pre-distribute it ahead of time.
:::
