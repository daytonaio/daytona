...
Daytona provides options to create snapshots using [public images](#using-a-public-image), [private images](<(#using-private-registries)>), [local images](#using-a-local-image), and the [declarative builder](#using-the-declarative-builder).
...
Daytona supports creating snapshots from private images from Docker Hub or other private container registries.
...
### Using private Docker Hub image

Daytona supports creating snapshots from private Docker Hub images.
...
- **Username**: Docker Hub username (the account with access to the private image)
...
4. After the container registry is successfully created, navigate to [Daytona Snapshots ↗](https://app.daytona.io/dashboard/snapshots)
5. Click the **Create Snapshot** button
6. Enter the snapshot name and image (tag or digest) of the private image. When creating the snapshot, make sure to input the entire private image name, including the registry location and project name (e.g. **`docker.io/<username>/<image>:<tag>`**)
...
4. After the container registry is successfully created, navigate to [Daytona Snapshots ↗](https://app.daytona.io/dashboard/snapshots)
5. Click the **Create Snapshot** button
6. Enter the snapshot name and image (tag or digest) of the private image. When creating the snapshot, make sure to input the entire private image name, including the registry location and project name (e.g. **`us-central1-docker.pkg.dev/<project>/<repo>/<image>:<tag>`**)
