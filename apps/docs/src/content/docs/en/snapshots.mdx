...
- **Image**: Base image for the snapshot. Must include either a tag or a digest (e.g., **`ubuntu:22.04`**). The **`latest`** tag is not allowed. Since images tagged `latest` get frequent updates, use immutable version tags or digests to prevent unexpected behavior.
...
...
 ...
 Snapshot my-awesome-snapshot successfully created
 ✓ Run 'daytona sandbox create --snapshot my-awesome-snapshot' to create a new sandbox using this snapshot
```
