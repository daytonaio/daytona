...
Push local Docker image as a snapshot

```shell
daytona snapshot push [SNAPSHOT] [flags]
```

`[SNAPSHOT]` is the local Docker image reference to push (for example, `custom-alpine:3.21`).
...
| Long | Short | Description |
| :--- | :---- | :---------- |
| `--size` | `-s` | Reserved flag. The current API creates volumes by name and ignores this value |
| `--help` |  | help for daytona |
