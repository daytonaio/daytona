...
description: Use 500 parallel Daytona sandboxes to train code-generating LLMs with TRL's GRPO trainer.
---
...
This guide demonstrates how to use Daytona sandboxes to safely execute hundreds of code completions in parallel during reinforcement learning training. We'll use [TRL's](https://huggingface.co/docs/trl/) GRPOTrainer together with **500 Daytona sandboxes evaluating completions concurrently**, in order to train the `Qwen3-1.7B-Base` model on code-writing tasks.
...
The evaluation step happens in parallel across all 500 sandboxes.

### 2. Setup

The code in this guide is written to run on a single 80GB VRAM GPU. If you want to run it on a GPU with less VRAM, you can decrease `per_device_train_batch_size` parameter, possibly increasing `gradient_accumulation_steps` proportionally if you wish to keep effective batch size at 500.
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    async def evaluate_single_completion_async(
        ...
        # Step 1: Extract just the function body
        body = sanitize_completion(raw_completion)

        # Step 2: Reject empty completions
        if not body.strip():
            return _fail_result(num_task_tests)

        # Step 3: Reject completions with banned patterns
        if has_banned_pattern(body, task):
            return _fail_result(num_task_tests)

        # Step 4: Build executable test code
        code = build_test_harness(task, body)

        # Step 5: Run in sandbox with timeout
        try:
        ...
        # Step 6: Parse results from stdout
    ...
    async def _evaluate_batch_async(
        ...
        async def run_one(
            ...
            try:
                ...
                return _fail_result(len(PROMPT_TO_TASK[prompt]["tests"]))
...
```
This equals `SANDBOX_POOL_SIZE`. Each training step generates exactly 500 completions, and we have exactly 500 sandboxes - so every completion evaluates in parallel with no waiting. If we had fewer sandboxes, some completions would queue up. If we had more, sandboxes would sit idle.
...
**Generation settings:**
- `num_generations=250`: Generate 250 completions per prompt. With 2 prompts (sorting and max_subarray), that's 500 total per step
...
|-----------|---------|-------------|
| `SANDBOX_POOL_SIZE` | 500 | Number of parallel sandboxes |
| `MAX_TIMEOUT_SECONDS` | 1 | Timeout per code execution |
| `MODEL_NAME` | `Qwen/Qwen3-1.7B-Base` | Base model to train |

:::tip[Scaling Tips]
- Match `SANDBOX_POOL_SIZE` to your total batch size for optimal parallelism
