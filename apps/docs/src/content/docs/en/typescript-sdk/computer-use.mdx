...
- `options` _AccessibilityTreeOptions_ - Optional `scope`, `pid`, and `maxDepth` filters.
...
```typescript
const tree = await sandbox.computerUse.accessibility.getTree({ scope: 'focused', maxDepth: 2 });
```
...
- `options` _AccessibilityFindOptions_ - Optional `scope`, `pid`, `role`, `name`, `nameMatch`, `states`, and `limit` filters.
...
- `id` _string_ - Accessibility node ID
- `action` _string_ - Optional action name
