...
Use `get` instead. Filtering by labels will be removed in a future version.
...
list(
   ...
   page?: number, 
   limit?: number, 
name?: string): Promise<PaginatedSandboxes>
```
...
- `name?` _string_ - Filter by partial name match
...
Use `name` instead. Filtering by labels will be removed in a future version.
