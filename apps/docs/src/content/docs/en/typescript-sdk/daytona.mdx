...
Use listV2 instead. This method uses offset-based pagination against a
deprecated API endpoint that will be removed on April 1, 2026. After that date, this method
will be removed and listV2 will be renamed to `list`.
...
listV2(
   cursor?: string, 
   limit?: number, 
states?: SandboxState[]): Promise<PaginatedSandboxesV2>
```
...
- `limit?` _number_ - Maximum number of items per page.
- `states?` _SandboxState\[\]_ - Filter by Sandbox states.


**Returns**:

- `Promise<PaginatedSandboxesV2>` - Cursor-paginated list of Sandboxes.
