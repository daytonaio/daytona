...
codeRun(
   ...
   timeout?: number, 
signal?: AbortSignal): Promise<ExecuteResponse>
```
...
- `signal?` _AbortSignal_ - AbortSignal to cancel the command. When aborted, the server kills the running process.
...
executeSessionCommand(
   ...
   timeout?: number, 
signal?: AbortSignal): Promise<SessionExecuteResponse>
```
...
```ts
terminateSessionCommand(sessionId: string, commandId: string): Promise<void>
```
