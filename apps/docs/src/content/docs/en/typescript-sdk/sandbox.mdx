...
- `recoverable?` _boolean_ - Whether the Sandbox error is recoverable.
    ...
    ```
- `resizing?` _boolean_ - Whether a resize operation is in progress.
...
- `hot?` _boolean = false_ - If true, performs hot resize on a running sandbox (only CPU/memory increase allowed).
...
- If GPU is specified, hot resize constraints are violated, disk size decrease is attempted,
