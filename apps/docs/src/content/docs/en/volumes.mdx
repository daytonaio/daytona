...
Daytona Volumes are included in your organization's compute pool at no additional cost beyond standard disk usage.

Volume quotas are applied per organization and vary by tier. You can view your current volume usage and limits in the [Daytona Dashboard ↗](https://app.daytona.io/dashboard/limits). Volume data counts against your organization's storage quota, but there are no separate charges for volume storage.

For more information about tier limits and how to upgrade, see [limits](/docs/en/limits).
