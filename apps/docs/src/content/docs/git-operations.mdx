...
the Dockerfile if present, or falling back to the user's home directory if not - e.g. `workspace/repo` implies `/my-workdir/workspace/repo`, but you are free to provide an absolute workdir path as well (by starting the path with `/`).
