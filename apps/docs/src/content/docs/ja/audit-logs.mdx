---
title: 監査ログ
description: Daytona の組織全体でのすべてのユーザー操作を表示・監視します。
---

Daytona の監査ログは、組織全体におけるユーザーおよびシステムの活動を詳細に記録します。サンドボックスのライフサイクルイベント、ユーザーアクセス、システム変更などの追跡に利用できます。

* **セキュリティ監査**: 不正アクセスやサンドボックスの不正利用を監視
* **デバッグ**: サンドボックスのライフサイクルに関する問題（例: 起動失敗）を把握
* **コンプライアンス向けエクスポート**: 社内外の監査向けにログをエクスポート（近日対応予定）

監査ログは、フルアクセス権を持つ[管理者](/docs/ja/organizations#organization-roles)と、監査ログ権限を持つ[メンバー](/docs/ja/organizations#organization-roles)が利用できます。監査ログにアクセスできない場合は、組織管理者にお問い合わせください。

## ダッシュボードからのアクセス \{#access-from-dashboard\}

[Daytona Dashboard ↗](https://app.daytona.io/dashboard/audit-logs) から監査ログページに直接アクセスできます。監査ログページでは、組織のすべての監査ログが一覧表示され、次の列が含まれます:

* **Time**: アクションが行われた時刻
* **User**: アクションを実行したユーザー
* [Actions](#actions): 実行されたアクション
* [Targets](#targets): アクションの影響を受けたリソース
* [Outcomes](#outcomes): アクションの結果

監査ログを時間範囲でフィルタリングするには、ページ左上にある日付範囲ピッカーを使用します。

## リアルタイム更新 \{#real-time-updates\}

Daytona では監査ログのリアルタイム更新が利用できます。[Daytona Audit Logs ↗](https://app.daytona.io/dashboard/audit-logs) ページ右上の**Auto Refresh**トグルをオンにすると、新規イベントの発生にあわせてログが自動更新されます。

## プログラムによる管理 \{#programmatic-management\}

Daytona は、監査ログにプログラムからアクセスするための API エンドポイントを提供しています。

### すべての監査ログを取得する \{#get-all-audit-logs\}

すべての監査ログを取得するには、以下の API エンドポイントを使用します。

```bash
curl https://app.daytona.io/api/audit \
  --header 'Authorization: Bearer YOUR_API_KEY'
```

詳細については、[API](/docs/ja/tools/api/#daytona/tag/audit) リファレンスを参照してください。

> [**すべての監査ログを取得 (API)**](/docs/ja/tools/api/#daytona/tag/audit/GET/audit-logs)

### 組織の監査ログを取得する \{#get-audit-logs-for-organization\}

特定の組織の監査ログを取得するには、以下の API エンドポイントを使用します。

```bash
curl https://app.daytona.io/api/audit/organizations/{organizationId} \
  --header 'Authorization: Bearer YOUR_API_KEY'
```

詳細については、[API](/docs/ja/tools/api/#daytona/tag/audit) リファレンスを参照してください。

> [**組織の監査ログを取得 (API)**](/docs/ja/tools/api/#daytona/tag/audit/GET/audit/organizations/\{organizationId})

## ログ構造 \{#log-structure\}

各監査ログエントリには、次のフィールドが含まれます。

| Field                | Type   | Description                                                   |
| -------------------- | ------ | ------------------------------------------------------------- |
| **`id`**             | string | 一意のログエントリ識別子                                     |
| **`actorId`**        | string | アクションを実行したユーザーの ID                            |
| **`actorEmail`**     | string | アクションを実行したユーザーのメールアドレス                |
| **`organizationId`** | string | 組織 ID                                                       |
| **`action`**         | string | 実行された操作（例: `create`、`start`、`stop`）              |
| **`targetType`**     | string | 影響を受けたリソースの種類（例: `サンドボックス`、`スナップショット`） |
| **`targetId`**       | string | 影響を受けたリソースの ID                                    |
| **`statusCode`**     | number | 結果の HTTP ステータスコード                                 |
| **`errorMessage`**   | string | アクションが失敗した場合のエラーメッセージ                  |
| **`ipAddress`**      | string | リクエスト送信元の IP アドレス                               |
| **`userAgent`**      | string | リクエスト送信元のユーザーエージェント                      |
| **`source`**         | string | アクションの発生元                                           |
| **`metadata`**       | object | アクションに関する追加の情報                                 |
| **`createdAt`**      | string | アクションが発生した時刻の ISO 8601 タイムスタンプ          |

## アクション \{#actions\}

以下は、Daytona によってログに記録されるアクションの一覧です。

```text
create, read, update, delete, login,
set_default, update_access, update_quota, update_region_quota,
suspend, unsuspend, accept, decline,
link_account, unlink_account, leave_organization,
regenerate_key_pair, update_scheduling,
start, stop, replace_labels, create_backup,
update_public_status, set_auto_stop_interval,
set_auto_archive_interval, set_auto_delete_interval, archive,
get_port_preview_url, set_general_status, activate, deactivate,
update_network_settings, get_webhook_app_portal_access,
send_webhook_message, initialize_webhooks,
update_sandbox_default_limited_network_egress,
create_ssh_access, revoke_ssh_access,
regenerate_proxy_api_key,regenerate_ssh_gateway_api_key,regenerate_snapshot_manager_credentials,
toolbox_delete_file, toolbox_download_file, toolbox_create_folder,
toolbox_move_file, toolbox_set_file_permissions, toolbox_replace_in_files,
toolbox_upload_file, toolbox_bulk_upload_files,
toolbox_git_add_files, toolbox_git_create_branch, toolbox_git_delete_branch,
toolbox_git_clone_repository, toolbox_git_commit_changes,
toolbox_git_pull_changes, toolbox_git_push_changes,
toolbox_git_checkout_branch, toolbox_execute_command,
toolbox_create_session, toolbox_session_execute_command,
toolbox_delete_session, toolbox_computer_use_start,
toolbox_computer_use_stop, toolbox_computer_use_restart_process
```

## 対象 \{#targets\}

各アクションは特定のリソースタイプを対象とします。想定される対象には次のものが含まれます：

```text
api_key, 組織, organization_invitation,
organization_role, organization_user, docker_registry,
ランナー, サンドボックス, スナップショット, user, ボリューム
```

## 結果 \{#outcomes\}

outcome フィールドはアクションの結果を示します。ステータスは標準的な HTTP セマンティクスに従います。

| **結果** | **説明** |
| ----------- | ----------------------------- |
| Info        | 情報 (1xx コード) |
| Success     | 成功 (2xx コード) |
| Redirect    | リダイレクト (3xx コード) |
| Error       | クライアント/サーバーエラー (4xx/5xx) |