---
title: AI でデータを分析する
description: Daytona を使って AI 生成コードを実行し、データの分析と可視化を行います。
---
...
Daytona サンドボックスを使用して、AI が生成したコードを実行し、データを分析できます。一般的な AI データ分析ワークフローは次のとおりです。

1. ユーザーは CSV 形式またはその他の形式のデータセットを用意します。
2. ユーザーのデータに基づいてコード（通常は Python）を生成するよう、LLM にプロンプトを送ります。
3. サンドボックスが AI 生成コードを実行し、その結果を返します。
4. LLM は実行結果からフィードバックを受け取り、必要に応じてコードを改善するために何度でも繰り返し実行できます。
5. 最終的な結果をユーザーに表示します。
...
この例では、Daytona の安全なサンドボックス環境を使って、CSV データから自動的に洞察と可視化を生成する AI を活用したデータアナリストを構築する方法を紹介します。

**ここで構築するもの:** 車両評価データセットを分析し、製造年と価格の関係を明らかにし、プロフェッショナルな可視化を生成するシステムです。これらはすべて、Claude への自然言語プロンプトだけで行います。このシステムは、Claude が実行結果に基づいてコードを反復的に改善できるようにするエージェントループを利用します。
...
次に、[Daytona サンドボックス](/docs/ja/sandbox-management/#basic-sandbox-creation) を作成し、データセットをアップロードします。
...
まず、コードの実行とチャートの抽出を行う関数を作成します。この関数は、AI モデルにフィードバックできる実行結果を返します。
...
次に、データセットの内容と実行したい分析内容を Claude に伝えるプロンプトを作成します。このプロンプトには次の内容が含まれます：

* データセットのスキーマとカラムの説明
* 具体的な分析内容（製造年ごとの車両価格の変動）
* コード生成に関する指示
...
Claude がサンドボックス環境で Python コードを実行し、その結果を取得できるようにするツールを定義します。
...
#### 3.4 エージェンティックループの実装 \{#34-agentic-loop-implementation\}

ここでは、Claude が実行フィードバックに基づいてコードを反復的に改善できるようにする、エージェンティックループを実装します。これにより、Claude はエラーの修正やエッジケースの処理を行い、複数回の反復を通じて分析を高度化できるようになります。
...
<Tabs syncKey="language">
  <TabItem label="Python" icon="seti:python">
    ...
    print("エージェントループを開始しています...\n")

    while continue_loop and iteration_count < max_iterations:
        iteration_count += 1
        print(f"\n=== イテレーション {iteration_count} ===")
        print("モデルの応答を待機しています...")
    ...
    if iteration_count >= max_iterations:
        print("\n⚠️  最大イテレーション数に達しました。タスクが完了していない可能性があります。")
...
1. **初回リクエスト**: ツール定義とともに最初のプロンプトを Claude に送信する
2. **反復ループ**: 各イテレーション（最大 10 回）ごとに:
   * Claude が、任意のツール呼び出しを含むレスポンスを生成する
   * ツール呼び出しがある場合、サンドボックス内で Python コードを実行する
   * 実行結果を Claude に返す（エラーや成功メッセージを含む）
   * Claude はフィードバックに基づいてコードを改善できる
3. **完了**: Claude がこれ以上ツール呼び出しが不要であると示すか、最大イテレーション数に達した時点でループが終了する

このアプローチにより、Claude は次のことが可能になります。

* 初期コードが失敗した場合にエラーを修正する
* 結果に満足できない場合に分析を繰り返し改善する
* 実行中に発見されたエッジケースを処理する
* 実際のデータに基づいて可視化を改善する

**このアプローチの主な利点:**

* **安全な実行:** コードは分離された Daytona サンドボックス内で実行される
* **アーティファクトの自動取得:** チャート、テーブル、各種出力が自動的に抽出される
* **エラー処理:** エラー検出とロギングが組み込みで提供される
* **言語非依存:** ここでは Python を使用しているが、Daytona は複数の言語をサポートしている
...
以下に、エージェントループを備えた、すぐに実行できる完全なサンプルを示します。
...
<Tabs syncKey="language">
  <TabItem label="Python" icon="seti:python">
    ...
    def main():
        ...
        initial_prompt = """
    /home/daytona/dataset.csv のサンドボックス内に車両評価額のCSVファイルがあります。
    ...
    私の要求に基づいてデータセットを分析し、matplotlibグラフを生成するPythonコードを記述してください。
    必ず最後にplt.show()を使用してグラフを表示してください。"""
                ...
                if tool_use.name == 'run_python_code':
                    ...
                    if execution_result['exit_code'] == 0:
                        ...
                        if execution_result['stdout']:
                            result_content += f"出力:\n{execution_result['stdout']}\n"
                        if execution_result['charts'] and len(execution_result['charts']) > 0:
                            result_content += f"\n{len(execution_result['charts'])}個のグラフを生成しました。"
                        else:
  ...
  <TabItem label="TypeScript" icon="seti:typescript">
    ...
    async function main() {
      ...
      const initialPrompt = `
    /home/daytona/dataset.csv に車両評価データを含むCSVファイルがサンドボックス内に保存されています。

    関連する列:
    - 'year': 整数型、車両の製造年
    - 'price_in_euro': 浮動小数点型、車両のユーロ建て表示価格

    製造年ごとの価格変動を分析してください。
    'year' または 'price_in_euro' が欠損値、非数値、または外れ値である行を削除してください。
    年ごとの平均価格を示す折れ線グラフを作成してください。
    私の要求に基づいてデータセットを分析し、matplotlibでグラフを生成するPythonコードを記述してください。
    グラフを表示するため、必ず最後に plt.show() を実行してください。`;
      ...
      const tools: Tool[] = [
        {
          name: "run_python_code",
          description: "サンドボックス環境でPythonコードを実行し、実行結果を取得します",
          input_schema: {
            type: "object",
            properties: {
              code: {
      ...
      console.log("エージェントループを開始しています...\n");

      while (continueLoop && iterationCount < maxIterations) {
        iterationCount++;
        console.log(`\n=== 反復処理 ${iterationCount} ===`);
        console.log("モデルの応答を待機しています...");
          ...
          if (toolUse.name === "run_python_code") {
            ...
            if (executionResult.exitCode === 0) {
              ...
              if (executionResult.charts && executionResult.charts.length > 0) {
                ...
                resultContent += `\n注意: グラフは生成されませんでした。グラフを表示するには plt.show() を使用してください。`;
              }
            } else {
              resultContent += `終了コード ${executionResult.exitCode} で実行が失敗しました\n\n`;
              if (executionResult.stdout) {
                resultContent += `出力:\n${executionResult.stdout}\n`;
              }
            }
