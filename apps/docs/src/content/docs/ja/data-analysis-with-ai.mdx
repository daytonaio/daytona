...
**構築するもの:** 車両評価データセットを分析し、製造年と価格の関係を特定し、Claudeへの自然言語プロンプトを通じてプロフェッショナルな可視化を生成するシステム。

### 1. プロジェクトセットアップ

#### 1.1 依存関係のインストール

Daytona SDKとAnthropic SDKをプロジェクトにインストールします：
...
ClaudeとDaytonaを接続してデータを分析し、可視化を生成するコア機能を作成します。
...
print('実行アーティファクトにチャートが見つかりませんでした')
return
...
async function runAIGeneratedCode(sandbox, aiGeneratedCode: string) {
  ...
  if (!execution.artifacts || !execution.artifacts.charts) {
    console.log('実行アーティファクトにチャートが見つかりませんでした')
    return
  }
...
次に、データセットについてClaudeに説明し、どのような分析を行いたいかを伝えるプロンプトを作成します。このプロンプトには以下が含まれます：

- データセットのスキーマと列の説明
- 具体的な分析リクエスト（時間経過による投票平均のトレンド）
- コード生成の指示
...
prompt = f"""
サンドボックスの /home/daytona/dataset.csv に保存された車両評価のCSVファイルがあります。
...
'year' または 'price_in_euro' が欠損、非数値、または外れ値である行を削除してください。
...
最後に、Claudeのレスポンスを解析し、生成されたコードをDaytonaサンドボックスで実行します：
...
以上です！作成した`run_ai_generated_code`関数は、チャートの保存を自動的に処理します。ClaudeがPythonで`plt.show()`を使用して可視化を生成すると、Daytonaがそれをチャートアーティファクトとしてキャプチャし、PNGファイルとして保存します。
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    /home/daytona/dataset.csvにサンドボックス内に保存された車両評価のCSVファイルがあります。
    ...
    return
        ...
        for result in execution.artifacts.charts:
            if result.png:
                ...
                with open(f'chart-{result_idx}.png', 'wb') as f:
                    f.write(base64.b64decode(result.png))
                print(f'グラフがchart-{result_idx}.pngに保存されました')
                result_idx += 1
  ...
  <TabItem label="TypeScript" icon="seti:typescript">
    ...
    async function runAIGeneratedCode(sandbox: Sandbox, aiGeneratedCode: string) {
      ...
      // すべての結果を反復処理し、チャートを表すpngファイルを特に確認する
      if (!execution.artifacts || !execution.artifacts.charts) {
