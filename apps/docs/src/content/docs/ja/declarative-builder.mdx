...
サンドボックス（Daytonaが管理する隔離された一時的な実行環境）を作成する際に、その場でイメージを生成できます。既存のイメージに含まれていない特定の依存関係を持つ新しいサンドボックスを作成したい場合に有用です。

完全に新しいイメージを定義することも、既存のイメージに特定の依存関係を追加することもできます（例：`pip`パッケージや`apt-get install`コマンド）。
これによりビルド処理のために自前の計算リソースを使う必要がなくなり、代わりにDaytona（サンドボックスの作成・管理プラットフォーム）のインフラにオフロードできます。
各バージョンごとに別個のスナップショット（サンドボックス作成に使う事前設定済みの再利用可能なイメージ／テンプレート）を登録・検証する必要はありません。代わりに、依存関係リストを素早く反復しやすくなり、数十から数百の類似ユースケース／セットアップに対してわずかに異なるバージョンを用意できます。
...
:::tip
動的イメージからのサンドボックス作成を一度でも実行すると、そのイメージは以後24時間キャッシュされ、同じランナー（サンドボックスがスケジュールされ実行される基盤の計算ノード／ホスト）上での後続のサンドボックス作成はほぼ即時になります。
...
特定の依存関係を含む新しいDaytona（サンドボックスの作成・管理プラットフォーム）のスナップショット（サンドボックス作成に使う事前設定済みの再利用可能なイメージ／テンプレート）を用意し、必要なときに複数のサンドボックス（Daytonaが管理する隔離された一時的な実行環境）で即座に使いたい場合は、事前ビルド済みスナップショットを作成できます。

このスナップショットはDaytonaダッシュボードに表示され続け、恒久的にキャッシュされるため、再ビルドは不要です。
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    # イメージに追加するためのローカルファイルを作成
    with open("file_example.txt", "w") as f:
        f.write("Hello, World!")

    # 代表的なデータサイエンス用パッケージを含むPythonイメージを作成
    image = (
        Image.debian_slim("3.12")
...
イメージのベースとして既存のDockerfileを使いたい場合は、次のようにインポートできます。
