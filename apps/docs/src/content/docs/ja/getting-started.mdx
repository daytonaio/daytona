...
## Daytona SDK のインストール \{#install-the-daytona-sdk\}

Daytona は、Daytona プラットフォームと連携するための公式 Python および TypeScript 向け SDK を提供しています。以下のいずれかの方法で SDK をインストールしてください。
...
[ローカルデバイスのイメージ](/docs/ja/snapshots#using-a-local-image)を使用する場合や、コマンドラインインターフェイスでサンドボックスを管理したい場合は、次のコマンドを実行して Daytona CLI をインストールしてください。
