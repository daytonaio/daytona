...
## Vite プロジェクトでの Daytona \{#daytona-in-vite-projects\}
...
Next.js プロジェクトで Daytona SDK を使用する場合、利用するバンドラー（Webpack または Turbopack）との互換性を確保するため、Node のポリフィルを設定する必要があります。`next.config.ts` に次の設定を追加してください。
