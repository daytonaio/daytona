...
GetPreviewLink は、サンドボックス上のポートにアクセスするためのプレビューリンクを返します。

返される PreviewLink には、URL と認証トークンの両方が含まれます。プライベートサンドボックスの場合、このトークンは &quot;x-daytona-preview-token&quot; リクエストヘッダーに設定して送信する必要があります。
