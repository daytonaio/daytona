...
* [type CodeInterpreterService](#CodeInterpreterService)
  * [func NewCodeInterpreterService(toolboxClient *toolbox.APIClient) *CodeInterpreterService](#NewCodeInterpreterService)
...
* [type ComputerUseService](#ComputerUseService)
  * [func NewComputerUseService(toolboxClient *toolbox.APIClient) *ComputerUseService](#NewComputerUseService)
...
* [DisplayService 型](#DisplayService)
  * [func NewDisplayService(toolboxClient *toolbox.APIClient) *DisplayService](#NewDisplayService)
...
* [FileSystemService 型](#FileSystemService)
  * [func NewFileSystemService(toolboxClient *toolbox.APIClient) *FileSystemService](#NewFileSystemService)
...
* [GitService 型](#GitService)
  * [func NewGitService(toolboxClient *toolbox.APIClient) *GitService](#NewGitService)
...
* [type KeyboardService](#KeyboardService)
  * [func NewKeyboardService(toolboxClient *toolbox.APIClient) *KeyboardService](#NewKeyboardService)
...
* [type LspServerService](#LspServerService)
  * [func NewLspServerService(toolboxClient *toolbox.APIClient, languageID types.LspLanguageID, projectPath string) *LspServerService](#NewLspServerService)
...
* [type MouseService](#MouseService)
  * [func NewMouseService(toolboxClient *toolbox.APIClient) *MouseService](#NewMouseService)
...
* [ProcessService 型](#ProcessService)
  * [func NewProcessService(toolboxClient *toolbox.APIClient) *ProcessService](#NewProcessService)
...
* [ScreenshotService 型](#ScreenshotService)
  * [func NewScreenshotService(toolboxClient *toolbox.APIClient) *ScreenshotService](#NewScreenshotService)
...
type Client struct {
    ...
    // Volume は永続ボリュームを管理するためのメソッドを提供します。
...
ComputerUseService により、マウス制御、キーボード入力、スクリーンショット取得、ディスプレイ管理を含む GUI 自動化が可能になります。これらの機能を使用する前に、デスクトップ環境を起動しておく必要があります。[Sandbox.ComputerUse] を通じてアクセスできます。
...
DeleteWithTimeout は、指定したタイムアウトでサンドボックスを削除します。
...
このメソッドは、サンドボックスが「started」状態に到達するか、タイムアウトを超えるまでブロックされます。
...
このメソッドは、サンドボックスが &quot;stopped&quot; 状態に到達するか、タイムアウトが発生するまで処理をブロックします。
...
このメソッドは、サンドボックスが started 状態になるか、エラー状態に遷移するか、またはタイムアウトに達するまで、その状態をポーリングし続けます。
...
このメソッドは、サンドボックスが停止状態になるか、タイムアウトに達するまで状態をポーリングします。
