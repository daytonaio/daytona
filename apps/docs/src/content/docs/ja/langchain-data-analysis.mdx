...
#### 2.1 依存関係のインストール \{#21-install-dependencies\}
...
#### 2.2 環境の設定 \{#22-configure-environment\}
...
ファイルをダウンロードし、プロジェクトディレクトリに `vehicle-dataset.csv` の名前で保存してください。
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    * `max_tokens_to_sample`: 1 回の生成で予測するトークン数
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    **ここで行われること:**
    ...
    7. `finally` ブロックでサンドボックスのリソースがクリーンアップされる
  </TabItem>
</Tabs>
...
**ステップ 3: コードは Daytona のサンドボックスで実行されます**
...
初期のデータセット情報に基づき、エージェントは主要カラムを検証するための、より具体的なコードを生成します。
