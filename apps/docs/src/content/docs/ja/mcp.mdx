...
### 1. Daytona CLI をインストール \{#1-install-daytona-cli\}
...
* **サンドボックスの作成**
  ...
  * パラメータ:
    * `name`: サンドボックス名（任意）
    * `target` (default: &quot;us&quot;): 対象リージョン
    * `snapshot`: サンドボックス用のスナップショット（任意）
    * `auto_stop_interval` (default: &quot;15&quot;): 自動停止までの分数（0 は無効）
    * `auto_archive_interval` (default: &quot;10080&quot;): 自動アーカイブまでの分数（0 は最大間隔を使用）
    * `auto_delete_interval` (default: &quot;-1&quot;): 自動削除までの分数（負の値は無効、0 は停止時に即時削除）
    * `volumes`: サンドボックスにアタッチするボリューム（任意）
    * `network_block_all`: サンドボックスの全ネットワークアクセスをブロックするかどうか（任意）
    * `network_allow_list`: サンドボックスで許可する CIDR ネットワークアドレスのカンマ区切りリスト（任意）
    * `public`: サンドボックスの HTTP プレビューを公開アクセス可能にするかどうか（任意）
