...
Daytona サービスでアウトバウンド HTTP プロキシを設定するには、プロキシ経由のアクセスが必要な各サービスの `docker-compose.yaml` に以下の環境変数を設定します（イメージの取得のために外向きアクセスが必要なのは API サービスのみです）:

* `HTTP_PROXY`: HTTP プロキシサーバーの URL
* `HTTPS_PROXY`: HTTPS プロキシサーバーの URL
* `NO_PROXY`: プロキシを経由しないホスト名または IP アドレスのカンマ区切りリスト

API サービスの基本構成は次のとおりです:
