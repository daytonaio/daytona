...
| `POSTHOG_ENVIRONMENT`                      | string | `local`                                              | PostHog の環境 ID                                                 |
...
| `SMTP_SECURE`                              | ブール値   | (空)                                                  | SMTP のセキュア接続を有効にする                                             |
| `SMTP_EMAIL_FROM`                          | 文字列    | `"Daytona Team <no-reply@daytona.io>"`               | SMTP 送信元メールアドレス                                                |
| `S3_ENDPOINT`                              | string | `http://minio:9000`                                  | S3 互換ストレージのエンドポイント                                             |
...
| `PROXY_DOMAIN`                             | 文字列    | `proxy.localhost:4000`                               | プロキシのドメイン名                                                     |
| `PROXY_PROTOCOL`                           | 文字列    | `http`                                               | プロキシプロトコル                                                      |
| `PROXY_API_KEY`                            | 文字列    | `super_secret_key`                                   | プロキシ API キー                                                    |
| `PROXY_TEMPLATE_URL`                       | 文字列    | `http://{{PORT}}-{{sandboxId}}.proxy.localhost:4000` | プロキシ用テンプレート URL のパターン                                          |
| `PROXY_TOOLBOX_BASE_URL`                   | 文字列    | `{PROXY_PROTOCOL}://{PROXY_DOMAIN}`                  | ツールボックスからのリクエスト用プロキシのベース URL                                   |
...
| `DEFAULT_RUNNER_GPU_TYPE`                  | 文字列    | `none`                                               | ランナーのデフォルト GPU タイプ                                             |
| `DEFAULT_RUNNER_CAPACITY`                  | 数値     | `100`                                                | デフォルトのランナー容量                                                   |
| `DEFAULT_RUNNER_CLASS`                     | 文字列    | `small`                                              | デフォルトのランナークラス                                                  |
| `DEFAULT_ORG_QUOTA_TOTAL_CPU_QUOTA`        | 数値     | `10000`                                              | 組織のデフォルト合計 CPU クォータ                                            |
...
| `DEFAULT_ORG_QUOTA_MAX_DISK_PER_SANDBOX`   | 数値     | `1000`                                               | 組織におけるサンドボックスあたりのデフォルト最大ディスク容量                                 |
...
| `ADMIN_API_KEY`                            | 文字列    | (空)                                                  | 管理者用 API キー。値が空の場合は自動生成され、初期セットアップ時にのみ使用されます。本番環境での利用は推奨されません。 |
...
| `RATE_LIMIT_FAILED_AUTH_TTL`               | 数値     | (empty)                                              | 認証失敗時のレート制限の有効期間（秒。空の場合はレート制限を無効化）                             |
...
| `DEFAULT_REGION_ENFORCE_QUOTAS`            | ブール値   | `false`                                              | デフォルトリージョンにリージョン単位のリソースクォータを適用する                               |
