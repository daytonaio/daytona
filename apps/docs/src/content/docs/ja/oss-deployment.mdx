...
| `DB_TLS_REJECT_UNAUTHORIZED`               | ブール値    | `true`                                               | 認証されていない TLS 証明書を拒否する                                          |
...
| `PUBLIC_OIDC_DOMAIN`                       | string  | `http://localhost:5556/dex`                          | パブリック OIDC ドメイン                                                |
| `OIDC_AUDIENCE`                            | 文字列     | `daytona`                                            | OIDC オーディエンス ID                                                |
| `OIDC_MANAGEMENT_API_ENABLED`              | boolean | （空）                                                  | OIDC 管理 API を有効にする                                             |
...
| `OIDC_MANAGEMENT_API_AUDIENCE`             | 文字列     | (empty)                                              | OIDC 管理 API のオーディエンス ID                                        |
...
| `DASHBOARD_BASE_API_URL`                   | 文字列     | `http://localhost:3000`                              | ダッシュボードのベース API URL                                            |
...
| `TRANSIENT_REGISTRY_URL`                   | string  | `http://registry:6000`                               | 一時レジストリ URL                                                    |
| `TRANSIENT_REGISTRY_ADMIN`                 | 文字列     | `admin`                                              | 一時レジストリ管理者のユーザー名                                               |
| `TRANSIENT_REGISTRY_PASSWORD`              | string  | `password`                                           | 一時レジストリ管理者のパスワード                                               |
...
| `PROXY_DOMAIN`                             | 文字列     | `proxy.localhost:4000`                               | プロキシ用ドメイン                                                      |
| `PROXY_PROTOCOL`                           | 文字列     | `http`                                               | プロキシプロトコル                                                      |
| `PROXY_API_KEY`                            | 文字列     | `super_secret_key`                                   | プロキシ API キー                                                    |
| `PROXY_TEMPLATE_URL`                       | 文字列     | `http://{{PORT}}-{{sandboxId}}.proxy.localhost:4000` | プロキシ用テンプレートURLのパターン                                            |
| `PROXY_TOOLBOX_BASE_URL`                   | 文字列     | `{PROXY_PROTOCOL}://{PROXY_DOMAIN}`                  | ツールボックスへのリクエスト用プロキシのベース URL                                    |
| `DEFAULT_RUNNER_DOMAIN`                    | 文字列     | `runner:3003`                                        | デフォルトランナーのドメイン                                                 |
| `DEFAULT_RUNNER_API_URL`                   | 文字列     | `http://runner:3003`                                 | デフォルトのランナー API URL                                             |
| `DEFAULT_RUNNER_PROXY_URL`                 | 文字列     | `http://runner:3003`                                 | デフォルトのランナー プロキシ URL                                            |
...
| `DEFAULT_RUNNER_GPU_TYPE`                  | 文字列     | `none`                                               | デフォルトのランナー GPU タイプ                                             |
| `DEFAULT_RUNNER_CAPACITY`                  | 数値      | `100`                                                | デフォルトのランナー容量                                                   |
| `DEFAULT_RUNNER_CLASS`                     | 文字列     | `small`                                              | デフォルトのランナークラス                                                  |
...
| `DEFAULT_ORG_QUOTA_TOTAL_DISK_QUOTA`       | 数値      | `100000`                                             | 組織のデフォルトの合計ディスククォータ                                            |
...
| `DEFAULT_ORG_QUOTA_MAX_SNAPSHOT_SIZE`      | 数値      | `1000`                                               | 組織のデフォルト最大スナップショットサイズ                                          |
...
| `SSH_GATEWAY_COMMAND`                      | 文字列     | `ssh -p 2222 {{TOKEN}}@localhost`                    | SSH ゲートウェイ コマンドのテンプレート                                         |
...
| `RUNNER_HEALTH_TIMEOUT_SECONDS`            | 数値      | `3`                                                  | ランナーのヘルスチェックのタイムアウト（秒）                                         |
...
| `ADMIN_TOTAL_MEMORY_QUOTA`                 | 数値      | `0`                                                  | 管理者向けの合計メモリクォータ。初回セットアップ時にのみ使用されます                             |
...
| `ADMIN_MAX_MEMORY_PER_SANDBOX`             | 数値      | `0`                                                  | サンドボックスごとの管理者用最大メモリ（初期セットアップ時のみ使用）                             |
| `ADMIN_MAX_DISK_PER_SANDBOX`               | 数値      | `0`                                                  | 初期セットアップ時にのみ使用される、サンドボックスごとの管理者用最大ディスク容量                       |
...
| `SKIP_USER_EMAIL_VERIFICATION`             | ブール値    | `true`                                               | ユーザーのメールアドレス確認プロセスをスキップする                                      |
...
| `RATE_LIMIT_SANDBOX_LIFECYCLE_TTL`         | 数値      | (empty)                                              | サンドボックスライフサイクルのレート制限の有効期間（秒。空の場合はレート制限を無効化）                    |
| `RATE_LIMIT_SANDBOX_LIFECYCLE_LIMIT`       | 数値      | (空)                                                  | サンドボックスのライフサイクルに対するレート制限（TTL あたりのリクエスト数。空欄の場合はレート制限は無効）        |
| `DEFAULT_REGION_ID`                        | 文字列     | `us`                                                 | デフォルトリージョンの ID                                                 |
| `DEFAULT_REGION_NAME`                      | string  | `us`                                                 | デフォルトリージョン名                                                    |
| `DEFAULT_REGION_ENFORCE_QUOTAS`            | ブール値    | `false`                                              | デフォルトリージョンに対してリージョン単位のリソースクォータを適用する                            |
