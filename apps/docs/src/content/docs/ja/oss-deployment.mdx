...
| `DB_TLS_ENABLED`                           | ブール値    | `false`                                              | データベース接続の TLS を有効にする                                           |
...
| `TRANSIENT_REGISTRY_PASSWORD`              | 文字列     | `password`                                           | 一時レジストリ管理者のパスワード                                               |
...
| `S3_ENDPOINT`                              | string  | `http://minio:9000`                                  | S3 互換ストレージエンドポイント                                              |
...
| `PROXY_TEMPLATE_URL`                       | 文字列     | `http://{{PORT}}-{{sandboxId}}.proxy.localhost:4000` | プロキシ用テンプレート URL パターン                                           |
| `PROXY_TOOLBOX_BASE_URL`                   | 文字列     | `{PROXY_PROTOCOL}://{PROXY_DOMAIN}`                  | ツールボックス リクエスト用プロキシのベース URL                                     |
...
| `DEFAULT_RUNNER_CLASS`                     | 文字列     | `small`                                              | デフォルトのランナー クラス                                                 |
...
| `DEFAULT_ORG_QUOTA_MAX_DISK_PER_SANDBOX`   | 数値      | `1000`                                               | 組織のサンドボックスごとのデフォルト最大ディスク                                       |
| `DEFAULT_ORG_QUOTA_SNAPSHOT_QUOTA`         | 数値      | `1000`                                               | 組織のデフォルトのスナップショットクォータ                                          |
...
| `SSH_GATEWAY_COMMAND`                      | 文字列     | `ssh -p 2222 {{TOKEN}}@localhost`                    | SSH ゲートウェイのコマンド テンプレート                                         |
| `RUNNER_DECLARATIVE_BUILD_SCORE_THRESHOLD` | 数値      | `10`                                                 | ランナーの宣言的ビルドにおけるスコアしきい値                                         |
...
| `RATE_LIMIT_ANONYMOUS_TTL`                 | 数値      | (empty)                                              | 匿名ユーザー向けレート制限の有効期間（秒。空欄の場合はレート制限は無効になる）                        |
...
| `RATE_LIMIT_SANDBOX_CREATE_TTL`            | 数値      | (空)                                                  | サンドボックス作成レート制限の有効期間（秒。空の場合はレート制限を無効化）                          |
...
| `DEFAULT_REGION_NAME`                      | 文字列     | `us`                                                 | デフォルトリージョンの名前                                                  |
| `DEFAULT_REGION_ENFORCE_QUOTAS`            | ブール値    | `false`                                              | デフォルトリージョンに対してリージョン単位のリソースクォータを有効にする                           |
