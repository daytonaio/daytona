...
| `OTEL_LOGGING_ENABLED`                  | boolean | `false`                           | ランナーの OpenTelemetry ログ出力を有効にするかどうか            |
| `OTEL_TRACING_ENABLED`                  | boolean | `false`                           | ランナーの OpenTelemetry トレースを有効にするかどうか            |
| `OTEL_EXPORTER_OTLP_ENDPOINT`           | string  | (empty)                           | ランナーの OpenTelemetry OTLP エクスポーターのエンドポイント      |
| `OTEL_EXPORTER_OTLP_HEADERS`            | string  | (empty)                           | ランナーの OpenTelemetry OTLP エクスポーターのヘッダー           |
...
| `VOLUME_CLEANUP_INTERVAL`               | number  | `30s`                             | ボリュームのクリーンアップ間隔（秒）（最小: 10s）                |
...
| `SNAPSHOT_ERROR_CACHE_RETENTION`        | string  | `10m`                             | スナップショットエラーキャッシュの保持期間（最小: 5m）           |
