...
## アウトバウンド HTTP プロキシの設定 \{#configuring-outbound-http-proxy\}

Daytona サービスでアウトバウンド HTTP プロキシを利用するには、プロキシが必要な各サービスの `docker-compose.yaml` に以下の環境変数を設定します（イメージ取得のためにアウトバウンドアクセスが必要なのは API サービスのみです）:

* `HTTP_PROXY`: HTTP プロキシサーバーのURL
* `HTTPS_PROXY`: HTTPS プロキシサーバーのURL
* `NO_PROXY`: プロキシを経由しないホスト名またはIPアドレスのカンマ区切りリスト
...
| `OIDC_MANAGEMENT_API_ENABLED`              | boolean | (empty)                                              | OIDC管理APIの有効化                          |
...
| `SMTP_SECURE`                              | boolean | (empty)                                              | SMTP安全な接続の有効化                       |
...
| `OTEL_ENABLED`                             | boolean | `true`                                               | OpenTelemetryトレースの有効化                |
| `OTEL_COLLECTOR_URL`                       | string  | `http://jaeger:4318/v1/traces`                       | OpenTelemetryコレクターURL                  |
| `MAINTENANCE_MODE`                         | boolean | `false`                                              | メンテナンスモードの有効化                   |
...
| `DEFAULT_ORG_QUOTA_VOLUME_QUOTA`           | number  | `10000`                                              | デフォルト組織ボリュームクォータ             |
...
| `RUNNER_DECLARATIVE_BUILD_SCORE_THRESHOLD` | number  | `10`                                                 | ランナー宣言的ビルドスコアの閾値    |
| `RUNNER_AVAILABILITY_SCORE_THRESHOLD`      | number  | `10`                                                 | ランナー可用性スコアの閾値         |
...
| 変数                  | 型     | 既定値                       | 説明                              |
| -------------------- | ------ | --------------------------- | --------------------------------- |
| `DAYTONA_API_URL`    | string | `http://api:3000/api`       | Daytona の API URL                |
