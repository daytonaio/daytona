...
### 対話的セッションコマンド \{#interactive-session-commands\}

セッションコマンドは対話的に動作させることができ、確認や対話的なツール（例: データベース CLI やパッケージマネージャーなど）のようにユーザーからの入力を想定している実行中のコマンドに対して入力を送信できます。
...
参照: [send&#95;session&#95;command&#95;input (Python SDK)](/docs/python-sdk/sync/process/#processsend_session_command_input), [sendSessionCommandInput (TypeScript SDK)](/docs/typescript-sdk/process/#sendsessioncommandinput)
