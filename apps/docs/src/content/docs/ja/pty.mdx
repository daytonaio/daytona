...
PTY セッションは、ユーザー入力を伴う対話型コマンドの実行や、実行中のターミナルサイズ変更に適しています。
...
## PTY を使った長時間実行プロセス \{#long-running-processes-with-pty\}
...
リソースリークを防ぐため、常に PTY セッションをクリーンアップしてください:
