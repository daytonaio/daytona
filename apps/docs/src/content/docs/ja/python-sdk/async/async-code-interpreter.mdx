...
* `code` *str* - 実行するコード。
* `context` *Optional[InterpreterContext]* - コードを実行するコンテキスト。指定しない場合はデフォルトコンテキストが使用されます。
* `on_stdout` *Optional[OutputHandler[OutputMessage]]* - stdout メッセージ用のコールバック。
* `on_stderr` *Optional[OutputHandler[OutputMessage]]* - stderr メッセージ用のコールバック。
* `on_error` *Optional[OutputHandler[ExecutionError]]* - 実行エラー
  （例: 構文エラー、実行時エラー）のコールバック。
* `envs` *Optional[Dict[str, str]]* - この実行用の環境変数。
* `timeout` *Optional[int]* - タイムアウト時間（秒単位）。0 はタイムアウトなしを意味します。デフォルトは 10 分です。
...
* `cwd` *Optional[str]* - コンテキストの作業ディレクトリ。指定されていない場合は、サンドボックスの作業ディレクトリが使用されます。
