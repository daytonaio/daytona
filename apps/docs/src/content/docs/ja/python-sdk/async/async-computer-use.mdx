...
* `MousePosition` - x および y 座標を含む現在のマウスカーソル位置。
...
* `RegionScreenshotResponse` - 画像を base64 エンコードしたスクリーンショットデータ。
...
* `CompressedScreenshotResponse` - 圧縮スクリーンショットデータ。
...
* `CompressedScreenshotResponse` - 圧縮されたスクリーンショットのデータ。
