...
#### AsyncProcess.__init__ \{#asyncprocess__init__\}
...
* `cwd` *Optional[str]* - コマンドを実行する作業ディレクトリ。指定しない場合は、
  サンドボックスの作業ディレクトリが使用されます。
* `env` *Optional[Dict[str, str]]* - コマンドに設定する環境変数。
* `timeout` *Optional[int]* - コマンドの完了を待機する最大時間（秒）。0 の場合は、
  完了するまで無期限に待機します。
...
* `code` *str* - 実行するコード。
* `params` *Optional[CodeRunParams]* - コード実行のパラメーター。
* `timeout` *Optional[int]* - コードの完了を待機する最大時間（秒）。0 は無期限に待機することを意味します。
...
print(f"タイプ: {chart.type}")
print(f"タイトル: {chart.title}")
if chart.type == ChartType.LINE and isinstance(chart, LineChart):
    print(f"Xラベル: {chart.x_label}")
    print(f"Yラベル: {chart.y_label}")
    print(f"X目盛り: {chart.x_ticks}")
    print(f"X目盛りラベル: {chart.x_tick_labels}")
    print(f"Xスケール: {chart.x_scale}")
    print(f"Y目盛り: {chart.y_ticks}")
    print(f"Y目盛りラベル: {chart.y_tick_labels}")
    print(f"Yスケール: {chart.y_scale}")
    print("要素:")
    for element in chart.elements:
        print(f"ラベル: {element.label}")
        print(f"ポイント: {element.points}")
```
...
* `on_stdout` *Callable[[str], None]* - 受信した stdout ログチャンクを処理するためのコールバック関数。
* `on_stderr` *Callable[[str], None]* - 受信した stderr ログチャンクを処理するためのコールバック関数。
...
async def create_pty_session(
        id: str,
        on_data: Union[Callable[[bytes], None],
                       Callable[[bytes], Awaitable[None]]] = None,
        cwd: Optional[str] = None,
        envs: Optional[Dict[str, str]] = None,
        pty_size: Optional[PtySize] = None) -> AsyncPtyHandle
```
...
async def connect_pty_session(
    session_id: str, on_data: Union[Callable[[bytes], None],
...
* `var_async` *Optional[bool]* - 非推奨です。代わりに `run_async` を使用してください。
...
* `charts` *Optional[List[Chart]]* - matplotlib によるチャートメタデータのリスト
