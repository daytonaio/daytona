...
@with_timeout(error_message=lambda self, timeout: (
    f"サンドボックス {self.id} が {timeout} 秒のタイムアウト期間内に開始できませんでした"
))
...
@with_timeout(error_message=lambda self, timeout: (
    f"サンドボックス {self.id} が {timeout} 秒のタイムアウト期間内に復旧に失敗しました"
))
...
* `expires_in_seconds` *Optional[int]* - 署名付きプレビューリンクが有効となる秒数。デフォルトは 60 秒。
