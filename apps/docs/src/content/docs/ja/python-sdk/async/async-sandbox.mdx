...
「stopped」状態になるか、エラーが発生するか、タイムアウトするまで待機します。サンドボックスが停止するまで最大60秒待機します。

**引数**:

- `timeout` _Optional[float]_ - 待機する最大時間（秒）。0はタイムアウトなしを意味します。デフォルトは60秒。

**送出**:

- `DaytonaError` - timeoutが負の場合。サンドボックスの停止に失敗した場合、またはタイムアウトした場合。
...
イベントには、SDKを通じたサンドボックスとのやり取りや、あらゆる状態変化が含まれます。
サンドボックスのプレビューを用いたやり取りは含まれません。

**引数**:

- `interval` _int_ - 自動停止までの非アクティブ時間（分）。
  0に設定すると自動停止を無効化します。デフォルトは15。

**送出**:

- `DaytonaError` - intervalが負の場合
...
指定した間隔のあいだ停止状態が連続して続いた場合、サンドボックスは自動的にアーカイブされます。

**引数**:

- `interval` _int_ - 停止状態が連続してから自動アーカイブされるまでの分数。
  0に設定すると最大の間隔になります。デフォルトは7日。
...
指定した間隔のあいだ停止状態が連続して続いた場合、サンドボックスは自動的に削除されます。

**引数**:

- `interval` _int_ - 停止状態が連続してから自動削除されるまでの分数。
  負の値を設定すると自動削除を無効化します。0に設定すると停止直後に削除します。
  デフォルトでは自動削除は無効です。
...
自動的に開かれます。プライベートなサンドボックスでは、URLへのアクセスを許可するトークンが含まれます。
...
- `PortPreviewUrl` - プレビューリンクのレスポンスオブジェクト。`url`
  および`token`（プライベートなサンドボックスへアクセスするため）を含みます。
...
アーカイブされると、ファイルシステム全体の状態がコスト効率の高いオブジェクトストレージへ移動され、
長期間にわたってサンドボックスを利用可能な状態に保てます。アーカイブ状態と
停止状態のトレードオフは、アーカイブされたサンドボックスの起動に、サイズに応じてより時間がかかる点です。
