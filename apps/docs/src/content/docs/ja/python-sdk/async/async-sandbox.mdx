...
## AsyncSandbox（非同期サンドボックス） \{#asyncsandbox\}
...
* `user` *str* - サンドボックスで実行される OS ユーザー。
...
* `cpu` *int* - サンドボックスに割り当てられる CPU 数。
* `gpu` *int* - サンドボックスに割り当てられる GPU 数。
* `memory` *int* - サンドボックスに割り当てられるメモリ（GiB）。
* `disk` *int* - サンドボックスに割り当てられるディスク容量（GiB）。
...
* `backup_created_at` *str* - バックアップの作成日時。
...
* `created_at` *str* - サンドボックスの作成日時。
* `updated_at` *str* - サンドボックスの最終更新日時。
