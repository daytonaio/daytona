...
'started'状態に達するまで、エラーが発生するまで、またはタイムアウトするまでポーリングします。
...
'stopped'状態に達するまで、エラーが発生するまで、またはタイムアウトするまでポーリングします。Sandboxが停止するまで最大60秒間待機します。
...
sandboxを長期間利用可能な状態で保持することができます。アーカイブ状態と
