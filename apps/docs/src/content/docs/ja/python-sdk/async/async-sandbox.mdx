...
- `interval` _int_ - 連続して停止したサンドボックスが自動アーカイブされるまでの時間（分）。
...
- `interval` _int_ - 連続して停止したサンドボックスが自動削除されるまでの時間（分）。
...
- `expires_in_minutes` _Optional[int]_ - トークンの有効期間（分）。
