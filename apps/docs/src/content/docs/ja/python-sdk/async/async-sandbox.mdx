...
@with_timeout(error_message=lambda self, timeout: (
    f"サンドボックス {cast('AsyncSandbox', self).id} が {timeout} 秒のタイムアウト期間内に開始できませんでした"
))
...
@with_timeout(error_message=lambda self, timeout: (
    f"サンドボックス {cast('AsyncSandbox', self).id} が {timeout} 秒のタイムアウト期間内に復旧に失敗しました"
))
...
@with_timeout(error_message=lambda self, timeout: (
    f"サンドボックス {cast('AsyncSandbox', self).id} は {timeout} 秒のタイムアウト期間内に停止できませんでした"
))
...
@with_timeout(error_message=lambda self, timeout:
              (f"サンドボックス {cast('AsyncSandbox', self).id} は {timeout} 秒のタイムアウト期間内に準備完了状態になりませんでした"))
