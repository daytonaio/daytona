...
```python
@intercept_errors(message_prefix="スナップショット一覧の取得に失敗しました: ")
...
## CreateSnapshotParams（スナップショット作成パラメーター） \{#createsnapshotparams\}
...
* `image` *Union[str, Image]* - スナップショットのイメージ。文字列が指定された場合は、
  いずれかのレジストリで参照可能である必要があります。Image インスタンスが指定された場合は、
