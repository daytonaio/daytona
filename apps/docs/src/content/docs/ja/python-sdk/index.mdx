...
分離された環境でコードを安全に実行するために、Daytona サンドボックスを作成します。次のスニペットは、Daytona サンドボックス内で安全に実行される「Hello, World」プログラムの例です。
...
Daytona SDK は環境変数を使うか、コンストラクタにオプションを渡して設定できます:
...
Daytona SDK の構成に関する詳細は、[configuration](/docs/ja/configuration) を参照してください。