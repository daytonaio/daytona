...
#### CodeInterpreter.&#95;&#95;init&#95;&#95; \{#codeinterpreter&#95;&#95;init&#95;&#95;\}
...
* `ensure_toolbox_url` - toolbox API の URL が初期化済みであることを保証します。
...
* `code` *str* - 実行するコード。
* `context` *Optional[InterpreterContext]* - コードを実行するコンテキスト。指定しない場合は、デフォルトのコンテキストが使用されます。
* `on_stdout` *Optional[OutputHandler[OutputMessage]]* - 標準出力メッセージのコールバック。
* `on_stderr` *Optional[OutputHandler[OutputMessage]]* - 標準エラーメッセージのコールバック。
* `on_error` *Optional[OutputHandler[ExecutionError]]* - 実行エラーのコールバック
  (例: 構文エラー、実行時エラー)。
* `envs` *Optional[Dict[str, str]]* - この実行で使用する環境変数。
* `timeout` *Optional[int]* - タイムアウト(秒単位)。0は無制限を意味します。デフォルトは10分です。
...
* `cwd` *Optional[str]* - コンテキストの作業ディレクトリ。指定しない場合は、サンドボックスの作業ディレクトリが使用されます。
