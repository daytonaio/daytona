...
- `auto_stop_interval` _Optional[int]_ - サンドボックスでイベントが発生しない場合に
  自動停止するまでの間隔（分）。デフォルトは15分。
  0は自動停止なしを意味します。
- `auto_archive_interval` _Optional[int]_ - 継続して停止状態のサンドボックスが
  自動アーカイブされるまでの間隔（分）。デフォルトは7日。
  0は最大の間隔が使用されることを意味します。
- `auto_delete_interval` _Optional[int]_ - 継続して停止状態のサンドボックスが
  自動削除されるまでの間隔（分）。デフォルトでは自動削除は無効です。
  負の値は無効、0は停止後ただちに削除を意味します。
...
- `ephemeral` _Optional[bool]_ - サンドボックスをエフェメラル（短命）にするかどうか。True の場合、auto_delete_interval は 0 に設定されます。
