...
コードインテリジェンス機能を提供するLanguage Server Protocol機能を提供し、コード補完、シンボル検索などのIDE風の機能を実現します。
...
このメソッドは、LSPサーバーが不要になった際にシステムリソースを
解放するために呼び出す必要があります。
...
このメソッドは、エディタでファイルが開かれた際に呼び出して、そのファイルに対する
...
このメソッドは、エディタでファイルが閉じられた際に呼び出して、
...
Sandbox内のすべてのファイルから、クエリ文字列に一致するシンボルを
検索します。
