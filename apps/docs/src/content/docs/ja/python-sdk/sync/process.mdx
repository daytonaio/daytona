...
* `code_toolbox` *SandboxPythonCodeToolbox* - 言語別のコード実行用ツールボックス。
...
* `cwd` *Optional[str]* - コマンド実行時の作業ディレクトリ。未指定の場合はサンドボックスのルートディレクトリを使用します。デフォルトはユーザーのルートディレクトリです。
* `env` *Optional[Dict[str, str]]* - コマンドに設定する環境変数。
* `timeout` *Optional[int]* - コマンドの完了を待機する最大時間（秒）。0 の場合は無期限に待機します。
