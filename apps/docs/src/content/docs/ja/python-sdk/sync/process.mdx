...
#### Process.__init__ {#process__init__}
...
- `sandbox_id` _str_ - サンドボックスのID。
- `code_toolbox` _SandboxPythonCodeToolbox_ - 言語別のコード実行用ツールボックス。
...
- `cwd` _Optional[str]_ - コマンド実行時の作業ディレクトリ。未指定の場合はサンドボックスのルートディレクトリを使用します。デフォルトはユーザーのルートディレクトリです。
- `env` _Optional[Dict[str, str]]_ - コマンドに設定する環境変数。
- `timeout` _Optional[int]_ - コマンドの完了を待機する最大時間（秒）。0 の場合は無期限に待機します。

**戻り値**:

- `ExecuteResponse` - コマンド実行結果。以下を含みます:
  - exit_code: コマンドの終了ステータス
  - result: コマンドの標準出力
  - artifacts: `stdout`（result と同一）と `charts`（matplotlib のチャートメタデータ）を含む ExecutionArtifacts オブジェクト
...
- `timeout` _Optional[int]_ - コードの完了を待つ最大時間（秒）。0 は無期限に待機します。
...
- `session_id` _str_ - 新規セッションの一意の識別子。
...
- `session_id` _str_ - 取得対象のセッションの一意識別子。
...
- `session_id` _str_ - 使用するセッションの一意の識別子。
...
- `session_id` _str_ - セッションの一意の識別子。
- `command_id` _str_ - コマンドの一意の識別子。
- `on_stdout` _Callable[[str], None]_ - 受信した標準出力のログチャンクを処理するコールバック関数。
- `on_stderr` _Callable[[str], None]_ - 受信した標準エラー出力のログチャンクを処理するコールバック関数。
...
- `session_id` _str_ - 削除対象のセッションの一意識別子。
...
- `artifacts` _Optional[ExecutionArtifacts]_ - コマンド実行の成果物
...
- `output` _str_ - コマンドの出力を結合したもの
- `stdout` _str_ - コマンドの標準出力
- `stderr` _str_ - コマンドの標準エラー出力
