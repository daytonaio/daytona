...
- `labels` _Dict[str, str]_ - サンドボックスのラベルを表すキーと値のペアの辞書。
...
- `interval` _int_ - 連続して停止したサンドボックスが自動アーカイブされるまでの分数。
...
- `interval` _int_ - 連続して停止したサンドボックスが自動削除されるまでの分数。
...
- `expires_in_minutes` _Optional[int]_ - SSH アクセストークンの有効期間（分）。
