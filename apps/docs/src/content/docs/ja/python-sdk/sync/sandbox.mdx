...
@with_timeout(error_message=lambda self, timeout: (
    f"サンドボックス {self.id} は {timeout} 秒のタイムアウト期間内に復旧できませんでした"
))
...
def create_signed_preview_url(
        port: int,
        expires_in_seconds: Optional[int] = None) -> SignedPortPreviewUrl
```
...
* `expires_in_seconds` *Optional[int]* - 署名付きプレビューリンクが有効となる秒数。既定値は 60 秒です。
