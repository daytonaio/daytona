...
ラベルはSandboxを整理し、識別するために使用できるキーと値のペアです。
...
Sandboxが「開始済み」状態に達するまで待機します。Sandboxが「開始済み」状態に達するか、エラーが発生するか、タイムアウトするまでSandboxのステータスをポーリングします。
...
Sandboxが「停止済み」状態に達するまで待機します。Sandboxが「停止済み」状態に達するか、エラーが発生するか、タイムアウトするまでSandboxのステータスをポーリングします。Sandboxが停止するまで最大60秒間待機します。
