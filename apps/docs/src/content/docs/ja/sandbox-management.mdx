...
### language パラメータ \{#language-parameter\}

`language` パラメータは、サンドボックス内でコードを直接実行する際に利用可能なプログラミング言語ランタイムを指定します。

* **`python`** – Python ランタイム（`language` が指定されていない場合、Python SDK と TypeScript SDK の両方でのデフォルト）。
* **`typescript`** – TypeScript ランタイム。
* **`javascript`** – JavaScript ランタイム。

`language` が省略された場合、SDK はデフォルトで `python` を使用します。サンドボックスを作成するときに `language` の値を明示的に設定することで、いつでもこのデフォルトを上書きできます。
...
サンドボックスがエラー状態になった場合でも、根本的なエラー原因によっては `recover` メソッドを使用して復旧できる場合があります。`recoverable` フラグは、そのエラー状態が自動復旧手順によって解決可能かどうかを示します。

:::note
ストレージ容量の解放など、**さらなるユーザーの介入**が必要となるエラーに対処するため、復旧処理は自動的には実行されません。
:::
