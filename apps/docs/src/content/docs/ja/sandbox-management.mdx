...
Daytona SDK では、デフォルトまたはカスタム構成でサンドボックスを作成できます。言語、[スナップショット](/docs/ja/snapshots)、リソース、環境変数、ボリュームをサンドボックスに指定可能です。
実行中のサンドボックスは CPU、メモリ、ディスクストレージを消費します。各リソースは使用した秒数に応じて課金されます。

:::tip
自動停止までの間隔を延長したい場合は、サンドボックス作成時に[自動停止間隔パラメータを設定](/docs/ja/sandbox-management#auto-stop-interval)できます。
:::

### 基本的なサンドボックス作成 {#basic-sandbox-creation}

Daytona SDK は、Python と TypeScript で、デフォルト構成・特定言語・カスタムラベルによるサンドボックス作成メソッドを提供します。
...
サンドボックスが積極的に使用されていない場合は、停止することを推奨します。これは[stop コマンドを使用](/docs/ja/sandbox-management#stop-and-start-sandbox)して手動で、または[自動停止間隔を設定](/docs/ja/sandbox-management#auto-stop-and-auto-archive)して自動で行えます。

:::note
Daytona はデフォルトのスナップショットを用いたウォームサンドボックスのプールを維持しています。\
利用可能な場合は、コールドブートせずミリ秒でサンドボックスが起動します。
:::

### サンドボックスのリソース {#sandbox-resources}

Daytona のサンドボックスは、デフォルトで **1 vCPU**、**1GB RAM**、**3GiB ディスク**が付与されます。

より高い性能が必要ですか？`Resources` クラスで必要な内容を正確に定義できます。CPU、メモリ、ディスク容量はすべてカスタマイズ可能です。
...
Daytona のサンドボックスは、セキュリティ強化と接続性管理のためのネットワークファイアウォール設定を提供します。デフォルトではネットワークアクセスは標準のセキュリティポリシーに従いますが、サンドボックス作成時にネットワーク設定をカスタマイズできます。
...
## サンドボックスの停止と起動 {#stop-and-start-sandbox}
...
### 自動アーカイブの間隔 {#auto-archive-interval}
