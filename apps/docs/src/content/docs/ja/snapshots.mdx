...
プライベートコンテナレジストリを手動で設定してイメージをプッシュする手間を避けるために、[Daytona CLI](/docs/en/getting-started#setting-up-the-daytona-cli)では、ローカルイメージまたはローカルのDockerfileからSnapshotを作成し、Sandboxで使用することができます。
...
詳細については、[CLIドキュメント](/docs/en/tools/cli#daytona-snapshot-push)を参照してください。
:::
