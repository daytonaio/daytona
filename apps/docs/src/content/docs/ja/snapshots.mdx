...
スナップショットには Daytona サンドボックスのリソース要件が含まれます。既定では、Daytona サンドボックスには **1 vCPU**、**1GB RAM**、**3GiB ディスク**が割り当てられます。

より高い性能が必要ですか？`Resources` クラスを使えば、必要な CPU、メモリ、ディスク容量を正確に指定できます。
...
:::note
すべてのリソースパラメータは任意です。指定しない場合、Daytona は既定値を使用します。
:::
