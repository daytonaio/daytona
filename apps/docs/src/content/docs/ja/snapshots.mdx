...
#### Google Artifact Registry を使用する \{#using-google-artifact-registry\}

Google Artifact Registry のイメージを使用するには、JSON 形式の[サービス アカウント キー](https://cloud.google.com/iam/docs/keys-create-delete)を使ってレジストリを設定する必要があります。

* **Registry URL**: 利用するリージョンのベース URL（例: `https://us-central1-docker.pkg.dev` または `https://us-central1-docker.pkg.dev/your-org`）。
* **Username**: `_json_key` を指定します。
* **Password**: サービス アカウントの JSON キー ファイルの内容をすべて貼り付けます。
* **Project**: 使用する Google Cloud プロジェクトの Project ID。
* **Create the Snapshot**: 上記を設定したら、次の完全なイメージ パスを使ってスナップショットを作成します: `us-central1-docker.pkg.dev/<project>/<repo>/<image>:<tag>`。
...
スナップショットは、特定のリソース要件でカスタマイズできます。デフォルトでは、Daytona サンドボックスには **1 vCPU**、**1GB RAM**、**3GiB ディスク**が割り当てられます。
