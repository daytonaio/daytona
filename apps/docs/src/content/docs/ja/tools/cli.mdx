...
<Aside type="note">
  bash シェルを使用する場合、完全な自動補完機能を利用するには bash-completion がインストールされていることを確認してください。Linux でのインストール: `sudo
    apt-get install bash-completion` macOS でのインストール: `brew install
    bash-completion`
</Aside>
...
| Long | Short | 説明 |
| :--- | :---- | :---------- |
| `--auto-archive` | | 自動アーカイブの間隔（分）。0 は最大間隔を使用 |
| `--auto-delete` | | 自動削除の間隔（分）。負の値で無効、0 は停止直後に削除 |
