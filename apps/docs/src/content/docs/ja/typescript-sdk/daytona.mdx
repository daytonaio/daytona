...
console.log(`サンドボックス ID: ${sandbox.id}, 状態: ${sandbox.state}`);
```
...
* `sandboxId` *string* - 取得するサンドボックスのID
...
ラベルでフィルタリングされたサンドボックスのページ分割一覧を返します。
...
* `Promise<PaginatedSandboxes>` - 指定したラベルに一致するサンドボックスのページ分割一覧。
...
* `ephemeral?` *boolean* - サンドボックスをエフェメラル（一時的）にするかどうか。true の場合、autoDeleteInterval は 0 に設定されます。
...
* `networkBlockAll?` *boolean* - サンドボックスで全てのネットワークアクセスをブロックするかどうか
* `public?` *boolean* - サンドボックスのポートプレビューを公開するかどうか
* `user?` *string* - サンドボックスで使用する任意の OS ユーザー
* `volumes?` *VolumeMount[]* - サンドボックスにマウントする任意のボリュームの配列
...
* `ephemeral?` *boolean*
* `image` *string | Image* - サンドボックスで使用するカスタム Docker イメージ。Image オブジェクトが指定された場合、
  イメージは動的にビルドされます。
...
* `public?` *boolean*
* `resources?` *Resources* - サンドボックスのリソース割り当て。未指定の場合、サンドボックスには
  既定のリソースが適用されます。
* `user?` *string*
* `volumes?` *VolumeMount[]*
...
* `networkBlockAll?` *boolean*
* `public?` *boolean*
* `snapshot?` *string* - サンドボックスに使用するスナップショットの名前。
* `user?` *string*
* `volumes?` *VolumeMount[]*
...
サンドボックスにおけるボリュームのマウントを表します。

**プロパティ**:

* `mountPath` *string* - サンドボックス内でボリュームをマウントするパス
