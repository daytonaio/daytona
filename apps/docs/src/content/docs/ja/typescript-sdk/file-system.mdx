...
Sandbox に指定した権限で新しいディレクトリを作成します。
...
Sandbox からファイルをダウンロードします。このメソッドはファイル全体をメモリに読み込むため、大きなファイルのダウンロードには推奨されません。

**Parameters**:

- `remotePath` _string_ - ダウンロードするファイルのパス。相対パスはユーザーの
    ルートディレクトリを基準に解決されます。
- `timeout?` _number_ - ダウンロード処理のタイムアウト（秒）。0 はタイムアウトなしを意味します。
    既定は 30 分です。

**Returns**:

- `Promise<Buffer<ArrayBufferLike>>` - Buffer としてのファイル内容。
...
Sandbox からファイルをダウンロードしてローカルファイルに保存します。このメソッドはストリーミングでダウンロードするため、大きなファイルの取得に適しています。

**Parameters**:

- `remotePath` _string_ - Sandbox 内でダウンロードするファイルのパス。相対パスはユーザーの
    ルートディレクトリを基準に解決されます。
...
- `Promise<FileInfo>` - サイズ、権限、更新時刻を含む詳細なファイル情報
...
Sandbox でファイル権限を設定するためのパラメータ。

**プロパティ**:

- `group?` _string_ - ファイルのグループ所有者
- `mode?` _string_ - 8進数形式のファイルモード/パーミッション（例: "644"）
- `owner?` _string_ - ファイルの所有ユーザー
...
- `destination` _string_ - Sandbox 内の絶対パス。相対パスはユーザーの
    ルートディレクトリを基準に解決されます。
- `source` _string_ | Buffer\<ArrayBufferLike\>_ - アップロードするファイル。Buffer の場合は、メモリに読み込まれるファイル内容として解釈されます。
    メモリに収まることを確認してください。収まらない場合はローカルファイルパスを使用し、内容を Sandbox にストリーミングしてください。