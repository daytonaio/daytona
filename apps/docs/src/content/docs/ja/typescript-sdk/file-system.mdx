...
ファイルのダウンロード処理に関するメタデータを表します。

**プロパティ**:

* `destination?` *string* - ファイル内容がストリーミングされるローカルファイルシステム上の保存先パス。
* `error?` *string* - ダウンロードに失敗した場合のエラーメッセージ。成功時は undefined。
* `result?` *string | Uint8Array&lt;ArrayBufferLike&gt; | Buffer&lt;ArrayBufferLike&gt;* - ダウンロード結果。リクエストで保存先が指定されている場合はファイルパス、
  指定がない場合はバイト列。失敗した場合やデータ未受信の場合は undefined。
