...
適切な言語ランタイムを用いて、Sandbox 内でコードを実行します。

**パラメータ**:

- `code` _string_ - 実行するコード
- `params?` _CodeRunParams_ - 実行パラメータ
...
Sandbox に新しい常駐型のバックグラウンドセッションを作成します。

セッションはコマンド間で状態を保持するバックグラウンドプロセスで、関連する複数コマンドの実行や永続的な環境準備が必要なケースに最適です。長時間実行のコマンドを走らせ、プロセス状態を監視できます。
