...
* `name` *string* - サンドボックスの名前
  ...
  SandboxDto.name
  ```
* `networkAllowList?` *string* - サンドボックスで許可されている CIDR ネットワークアドレスのカンマ区切りリスト
...
* `public` *boolean* - サンドボックスが公開アクセス可能かどうか
  ...
  SandboxDto.public
  ```
* `recoverable?` *boolean* - サンドボックスのエラーが復旧可能かどうか。
...
* `target` *string* - サンドボックスが実行されるランナーのターゲットロケーション
  ...
  SandboxDto.target
  ```
* `toolboxProxyUrl` *string* - サンドボックスのツールボックスプロキシ URL
