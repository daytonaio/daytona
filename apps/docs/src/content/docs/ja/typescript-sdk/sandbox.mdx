...
* `disk` *number* - サンドボックスに割り当てられたディスク容量（GiB）
  ...
  SandboxDto.disk
  ```
* `env` `_Record&lt;string, string&gt;_` - サンドボックスに設定された環境変数
...
* `networkAllowList?` *string* - サンドボックスで許可される CIDR ネットワークアドレスのカンマ区切りリスト
  ...
  ```
* `networkBlockAll` *boolean* - サンドボックスのすべてのネットワークアクセスをブロックするかどうか
...
* `state?` *SandboxState* - サンドボックスの現在の状態（例: &quot;started&quot;, &quot;stopped&quot;）
  ...
  SandboxDto.state
  ```
* `target` *string* - サンドボックスを実行するランナーのターゲット
...
* `updatedAt?` *string* - サンドボックスの最終更新時刻
  ...
  ```
* `user` *string* - サンドボックス内で稼働する OS ユーザー
  ...
  SandboxDto.user
  ```
* `volumes?` `_SandboxVolume[]_` - サンドボックスにアタッチされたボリューム
