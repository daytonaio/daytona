...
* `autoArchiveInterval?` *number* - 自動アーカイブ間隔（分）
  ...
  ```
* `autoDeleteInterval?` *number* - 自動削除間隔（分）
  ...
  ```
* `autoStopInterval?` *number* - 自動停止間隔（分）
  ...
  ```
* `backupCreatedAt?` *string* - バックアップの作成日時
...
* `disk` *number* - サンドボックスに割り当てられたディスク容量（GiB）
  ...
  SandboxDto.disk
  ```
* `env` *Record&lt;string, string&gt;* - サンドボックスで設定された環境変数
...
* `state?` *SandboxState* - サンドボックスの現在の状態（例：「started」「stopped」）
  ...
  SandboxDto.state
  ```
* `target` *string* - サンドボックスが実行されるランナーのターゲットロケーション
  ...
  SandboxDto.target
  ```
* `updatedAt?` *string* - サンドボックスの最終更新日時
  ...
  ```
* `user` *string* - サンドボックス内で実行中の OS ユーザー
...
新しいサンドボックス・インスタンスを作成します

**パラメーター**:

* `sandboxDto` *Sandbox* - API のサンドボックス・インスタンス
* `clientConfig` *Configuration*
* `sandboxApi` *SandboxApi* - サンドボックス操作用 API クライアント
* `toolboxApi` *ToolboxApi* - ツールボックス操作用 API クライアント
* `codeToolbox` *SandboxCodeToolbox* - 言語別ツールボックス実装
