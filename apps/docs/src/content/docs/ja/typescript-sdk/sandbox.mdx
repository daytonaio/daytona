...
Sandboxは、指定された間隔の間継続的に停止された後、自動的に削除されます。

**パラメータ**:

- `interval` _number_ - 継続的に停止されたSandboxが自動削除されるまでの分数。
    自動削除を無効にするには負の値を設定します。停止時に即座に削除するには0を設定します。
...
Sandboxは、指定された間隔の間アイドル状態（新しいイベントなし）が続いた後、自動的に停止します。
...
- `interval` _number_ - 自動停止までの非アクティブ時間（分）。
    自動停止を無効にするには0を設定します。デフォルトは15分です。
...
このメソッドは、Sandboxが'started'状態に達するか、エラーが発生するまでSandboxのステータスをポーリングします。
...
このメソッドは、Sandboxが'stopped'状態に達するか、エラーが発生するまでSandboxのステータスをポーリングします。
