...
- `gpu` _number_ - SandboxにGiB単位で割り当てられたGPU数
...
指定されたポートでサンドボックスのプレビューリンクを取得します。ポートが閉じている場合は、
自動的に開かれます。プライベートサンドボックスの場合、URLへのアクセスを許可するトークンが含まれます。
...
- `Promise<PortPreviewUrl>` - プレビューリンクのレスポンスオブジェクト。`url`と
    `token`（プライベートサンドボックスにアクセスするため）が含まれます。
...
サンドボックス内でログインしているユーザーのルートディレクトリパスを取得します。

**戻り値**:

- `Promise<undefined | string>` - ログインしているユーザーのサンドボックスルートディレクトリへの絶対パス
...
サンドボックスは、指定された間隔の間継続的に停止された後、自動的にアーカイブされます。

**パラメータ**:

- `interval` _number_ - 継続的に停止されたサンドボックスが自動アーカイブされるまでの分数。
    最大間隔にするには0に設定します。デフォルトは7日間です。
...
サンドボックスは、指定された間隔の間継続的に停止された後、自動的に削除されます。

**パラメータ**:

- `interval` _number_ - 継続的に停止されたサンドボックスが自動削除されるまでの分数。
