...
ボリュームを作成したら、`CreateSandboxFromSnapshotParams` オブジェクトで指定することでサンドボックスにマウントできます。ボリュームのマウントパスは次の要件を満たす必要があります：

* **絶対パスであること**: マウントパスは `/` で始まる必要があります（例：`/home/daytona/volume`）
* **ルートディレクトリは不可**: `/` や `//` にはマウントできません
* **相対パス要素を含まないこと**: `/../`、`/./` を含んだり、`/..` または `/.` で終わってはいけません
* **連続するスラッシュを使用しないこと**: 先頭を除き、`//` のような連続するスラッシュを含められません
* **システムディレクトリへのマウント禁止**: 次のシステムディレクトリは使用できません：
