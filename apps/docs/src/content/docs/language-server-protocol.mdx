...
the Dockerfile if present, or falling back to the user's home directory if not - e.g. `workspace/project` implies `/my-workdir/workspace/project`, but by starting with `/`, you may define an absolute path as well.
