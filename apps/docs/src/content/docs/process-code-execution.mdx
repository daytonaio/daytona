...
the Dockerfile if present, or falling back to the user's home directory if not - e.g. `workspace/repo` implies `/my-workdir/workspace/repo`, but you can override it with an absolute path (by starting the path with `/`).
