...
params = CreateSandboxFromSnapshotParams(
    ...
    network_allow_list="192.168.1.0/24,10.0.0.0/24"
)
sandbox = daytona.create(params)
...
:::note
Enabling `networkAllowAll` allows unrestricted network access, which may pose security risks. 
It is recommended to whitelist specific network addresses using `networkAllowList` instead. 
This setting accepts `/24` CIDR blocks in a comma-separated format.
:::
