...
| `--network-allow-all` |  | Whether to allow all network access for the sandbox |
| `--network-allow-list` |  | Comma-separated list of allowed network addresses for the sandbox (only /24 CIDR blocks allowed) |
