...
- `path` _string_ - Path to the Git repository root. Relative paths are resolved based on the sandbox workdir.
...
- `path` _string_ - Path where the repository should be cloned. Relative paths are resolved based on the sandbox workdir.
