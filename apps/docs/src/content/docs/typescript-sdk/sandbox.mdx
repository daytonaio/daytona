...
- `memory` _number_ - Amount of memory allocated to the Sandbox in GiB
    ...
    SandboxDto.memory
    ```
- `networkAllowAll` _boolean_ - Whether to allow all network access for the Sandbox
    
    ##### Implementation of
    
    ```ts
    SandboxDto.networkAllowAll
    ```
- `networkAllowList?` _string_ - Comma-separated list of allowed network addresses for the Sandbox (only /24 CIDR blocks allowed)
