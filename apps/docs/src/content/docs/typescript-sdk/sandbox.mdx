...
- `pathToProject` _string_ - Path to the project root directory. Relative paths are resolved based on the sandbox workdir.
...
```ts
getUserHomeDir(): Promise<undefined | string>
```
...
- `Promise<undefined | string>` - The absolute path to the Sandbox user's home directory for the logged in user
...
```ts
getWorkdir(): Promise<undefined | string>
```
...
- `Promise<undefined | string>` - The absolute path to the Sandbox workdir. Uses the WORKDIR specified
