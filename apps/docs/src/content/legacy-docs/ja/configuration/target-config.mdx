...
Target Configurationとは、さまざまな[Providers](/docs/en/configuration/providers)によって促進される開発セットアップがデプロイされ、管理される特定の宛先または環境を指します。Providersは環境を作成するために使用される方法と技術を定義し、Target Configurationsはこれらの環境が存在する正確な場所またはプラットフォームを指定します。
...
このコマンドを実行すると、DaytonaはTarget Configurationの一覧とその詳細を表示します。Target Configuration名、接続されているProvider、および設定されたRunnerを確認できます。
...
:::note
以前に[Runner](/docs/en/usage/runners)をインストールしていない場合、DaytonaはProviderを管理するRunnerを選択するよう求めます。選択すると、DaytonaはターゲットシステムにRunnerをインストールし、Daytona Serverに接続します。
:::
...
DaytonaはWorkspaceのデプロイと管理に使用するAWSターゲット設定を作成するオプションを提供します。

1. 以下のコマンドを実行してターゲット設定を作成します：

```shell
daytona target-config create
```

2. **`AWS`**を選択してAWSプロバイダーを使用します。
...
:::note
以前に[Runner](/docs/en/usage/runners)をインストールしていない場合、Daytonaはプロバイダーを管理するRunnerを選択するよう促します。選択すると、DaytonaはターゲットシステムにRunnerをインストールし、Daytonaサーバーに接続します。
:::
...
5. **`Enter`**をクリックしてターゲット設定の作成を確認します。
...
:::note
以前に[Runner](/docs/en/usage/runners)をインストールしていない場合、DaytonaはProviderを管理するRunnerの選択を求めます。選択すると、DaytonaはターゲットシステムにRunnerをインストールし、Daytona Serverに接続します。
:::
...
https://docs.microsoft.com/azure/virtual-machines/linux/disks-types場所ごとの利用可能なディスクタイプのリストは次のコマンドで取得できます：
...
```text
Auth Token
空の場合、トークンはFLY_ACCESS_TOKEN環境変数から取得されます。
>
...
Region
flyマシンが存在するリージョン。指定されていない場合、近くのリージョンが使用されます。
>

Size
flyマシンのサイズ。デフォルトはshared-cpu-4xです。
利用可能なサイズのリスト https://fly.io/docs/about/pricing/#started-fly-machines
>
```
