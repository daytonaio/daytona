...
- **ビルド設定**
   ...
   [Automatic](/docs/en/usage/builders#automatic)は、あなたのWorkspaceに最も適切なBuilderを自動検出します。

   [Devcontainer](/docs/en/usage/builders#dev-container)は、`devcontainer.json`ファイルで指定された事前定義された開発コンテナを利用します。

   [Custom image](/docs/en/usage/builders#custom-image)は、カスタムベースコンテナを指定してWorkspaceイメージをビルドします。

   [None](/docs/en/usage/builders#none)は、デフォルトのベースイメージ（`daytonaio/workspace-project`）を使用してWorkspaceをビルドします。
...
DaytonaはWorkspace Templateをインポートするオプションを提供しており、既存のWorkspaceプロパティを複数のWorkspace間で再利用することができます。
