...
Buildersは、あなたの[Project](/docs/en/usage/projects)に必要なツールと設定を含むコンテナイメージを作成する責任があります。
Daytonaは、結果として生成されるProject環境の作成方法を制御し、Projectイメージをビルドするための好みの方法を選択できるようにします。

[Workspaceの作成](/docs/en/usage/workspaces#create-a-workspace)時に、以下のBuildersから選択できます：
...
自動Builderを使用するには、[ワークスペース作成](/docs/en/usage/workspaces#create-a-workspace)時に`--builder`フラグの値を`auto`に設定します。このフラグは単一のプロジェクトでワークスペースを作成する場合にのみ適用されます。
...
Development Containers（Dev Containers）は、あなたの[Projects](/docs/en/usage/projects)に対して一貫性があり、完全に機能する開発環境を提供します。Dev Containerは、Projectsをビルドして実行するために必要なすべてのツール、ランタイム、および環境設定を定義します。
...
Dev Container Builderを使用するには、[Workspace creation](/docs/en/usage/workspaces#create-a-workspace)中に`--devcontainer-path`フラグをリポジトリ内のDev Container設定パスに設定してください。このフラグは、単一のProjectでWorkspaceを作成する場合にのみ適用されます。
...
Custom Image Builderを使用するには、[Workspaceの作成](/docs/en/usage/workspaces#create-a-workspace)時に`--custom-image`と`--custom-image-user`の両方のフラグを設定します。これらのフラグは、単一のProjectでWorkspaceを作成する場合にのみ適用されます。
