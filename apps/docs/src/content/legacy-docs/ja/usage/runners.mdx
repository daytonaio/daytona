...
単一のRunnerは、デプロイ時または後でジョブを介して設定される複数の[Providers](/docs/ja/configuration/providers)を使用でき、さまざまな環境に対する柔軟性を可能にします。
...
Daytonaは、Runnerをフォアグラウンドで提供するか、デーモン/バックグラウンドプロセスとしてRunnerを開始するオプションを提供します。要件に最も適した方法を使用してください。
...
RunnerはDaytona Serverに接続し、そのステータスは`Running`としてマークされます。
