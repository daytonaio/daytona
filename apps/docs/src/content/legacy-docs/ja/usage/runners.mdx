...
デプロイされると、Runnerが起動し、[Daytona Server](/docs/en/usage/server)に接続します。この時点で、そのステータスは`Running`としてマークされます。
...
単一のRunnerは、デプロイ時または後でジョブを介して設定される複数の[Providers](/docs/en/configuration/providers)を使用でき、さまざまな環境に対する柔軟性を可能にします。
