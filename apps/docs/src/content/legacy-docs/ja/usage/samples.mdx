...
サンプルは、Workspaceを作成するために使用される、事前定義された開発設定、環境、および依存関係を持つクイックスタートリポジトリです。

## カスタムサンプル

Daytonaは、カスタムサンプルを使用してワークスペースを作成するオプションを提供します。カスタムサンプルの作成は、以下を含む2段階のプロセスです：
...
- [**サンプルインデックス**](#samples-index)
  ...
  ワークスペース作成用の利用可能なサンプルのレジストリとして機能するサンプルインデックス（`index.json`）ファイル。

### サンプルリポジトリ

カスタムサンプルリポジトリは、サンプルコードと開発環境を定義する`devcontainer.json`ファイルを含むGitリポジトリです。このリポジトリはサンプルのコアコンテンツとして機能し、ワークスペースの作成に使用されます。
...
サンプルは、リポジトリをクローンし、`devcontainer.json`ファイルを使用して開発環境を定義することで、ワークスペースの作成に使用されます。

### サンプルインデックス

サンプルには、ワークスペース作成用の利用可能なサンプルのレジストリとして機能するサンプルインデックス（`index.json`）ファイルが必要です。サンプルインデックスは、ワークスペース作成時に利用可能なサンプルを表示するために使用されます。
...
カスタムサンプルは、ワークスペース作成時に利用可能になります。選択されたサンプルがクローンされ、ワークスペースの作成に使用されます。
