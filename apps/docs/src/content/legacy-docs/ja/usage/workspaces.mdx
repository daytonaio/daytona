...
Workspacesは、クラウドベースやオンプレミス環境などの適切な[Target](/docs/en/usage/targets)インフラストラクチャを使用してホストされます。これにより、あるプロジェクトでの変更が他のプロジェクトに意図せず影響を与えることを防ぎ、より安定で信頼性の高いワークフローを促進します。これは特に共同作業環境において有益で、すべてのチームメンバーが同じ設定で作業することを保証し、環境関連の問題が発生する可能性を減らします。

## Workspaceの作成

DaytonaでWorkspaceを作成することは、プロジェクト用の専用開発環境を確保する簡単なプロセスです。
...
Gitリポジトリからワークスペースを作成すると、[接続されたGit Provider](/docs/en/configuration/git-providers)またはカスタムGitリポジトリURLを使用して新しい開発環境がセットアップされます。
...
<Aside type="tip">
`--no-ide`フラグを使用して、[IDE](/docs/en/usage/ide)でWorkspaceを自動的に開くことをスキップします。
...
- **リポジトリのリストから選択**
   ...
   [Git Providerを接続](/docs/en/configuration/git-providers)している場合、Workspaceと接続するために選択できるリポジトリのリストが表示されます。
...
<Aside type="tip">
   ...
   [Builder](/docs/en/usage/builders)の設定も含まれます。
</Aside>

<br />

4. WorkspaceをデプロイするTargetを選択します。Targetを選択すると、[Target設定](/docs/en/configuration/target-config)の入力が求められます。設定は選択されたTargetによって異なります。
...
2. WorkspaceをデプロイするTargetを選択します。Targetを選択すると、[Target設定](/docs/en/configuration/target-config)の入力が求められます。設定は選択されたTargetによって異なります。
...
<Aside type="tip">
コマンドラインフラグを使用して、ワークスペース作成時にProject Builderを設定します。詳細については、**[Builders](/docs/en/usage/builders)**を参照してください。
</Aside>
...
詳細については、**[デフォルトIDEの設定](/docs/en/usage/ide#set-the-default-ide)**を参照してください。
:::

## ワークスペースの削除

Daytonaは1つまたは複数のワークスペースを削除するオプションを提供し、不要になった開発環境を削除することで開発環境の管理を支援します。

1. 次のコマンドを実行してワークスペースを削除します：
...
2. ワークスペースをハイライトして`x`を押すことで、削除対象としてマークします。

3. 選択したワークスペースで`Enter`を押して削除します。
...
Daytonaは既存の1つまたは複数のワークスペースを非対話的に削除するオプションを提供し、手動選択なしで開発環境の迅速で自動化されたクリーンアップを可能にします。

1. 削除するワークスペース名/UUIDを指定して次のコマンドを実行します：

```shell
daytona delete WORKSPACE_1 WORKSPACE_2
```

このコマンドを実行すると、Daytonaは指定されたワークスペースの削除を確認するよう求めます。削除しようとしているワークスペース名を確認でき、アクションを確認またはキャンセルするオプションがあります。
...
1. 実行中のWorkspaceからローカルマシンにポートを転送するには、次のコマンドを実行します：
...
1. 実行中のWorkspaceのポートに対してパブリックURLを生成するには、次のコマンドを実行します：
...
このURLは他の人と共有して、WorkspaceのSpecifiedポートで実行されているサービスへのリアルタイムアクセスを提供できます。

<Aside type="tip">
または、WorkspaceのシェルでNext のコマンドを実行してパブリックURLを生成することもできます：
