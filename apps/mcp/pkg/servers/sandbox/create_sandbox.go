...
func (s *DaytonaSandboxMCPServer) getCreateSandboxTool() *mcp.Tool {
	return &mcp.Tool{
		Name:        "create_sandbox",
...
func (s *DaytonaSandboxMCPServer) handleCreateSandbox(ctx context.Context, request *mcp.CallToolRequest, input *CreateSandboxInput) (*mcp.CallToolResult, *CreateSandboxOutput, error) {
	...
	for retry := range maxRetries {
		sandbox, _, err := s.apiClient.SandboxAPI.CreateSandbox(ctx).CreateSandbox(*createSandboxReq).Execute()
		if err != nil {
