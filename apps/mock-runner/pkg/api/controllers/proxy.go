...
// ProxyRequest handles proxying requests to the shared toolbox container
//
//	@Tags			toolbox
//	@Summary		Proxy requests to the sandbox toolbox
//	@Description	Forwards the request to the shared toolbox container
...
func getProxyTarget(ctx *gin.Context) (*url.URL, map[string]string, error) {
	...
	// Get toolbox container IP (all sandboxes use the same toolbox container)
	toolboxIP := runner.Mock.GetToolboxContainerIP()
	if toolboxIP == "" {
		message := "toolbox container not running"
		ctx.Error(common_errors.NewBadRequestError(errors.New(message)))
		return nil, nil, errors.New(message)
	}

	// Build the target URL pointing to the shared toolbox container
	targetURL := fmt.Sprintf("http://%s:2280", toolboxIP)
