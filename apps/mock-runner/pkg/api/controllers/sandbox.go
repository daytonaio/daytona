...
import (
	...
	"github.com/daytonaio/mock-runner/pkg/common"
...
func Create(ctx *gin.Context) {
	...
	containerId, err := runner.Mock.Create(ctx.Request.Context(), createSandboxDto)
	if err != nil {
...
func Destroy(ctx *gin.Context) {
	...
	err := runner.Mock.Destroy(ctx.Request.Context(), sandboxId)
	if err != nil {
...
func CreateBackup(ctx *gin.Context) {
	...
	err = runner.Mock.StartBackupCreate(ctx.Request.Context(), sandboxId, createBackupDTO)
	if err != nil {
...
func Resize(ctx *gin.Context) {
	...
	err = runner.Mock.Resize(ctx.Request.Context(), sandboxId, resizeDto)
	if err != nil {
		runner.StatesCache.SetSandboxState(ctx, sandboxId, enums.SandboxStateError)
		ctx.Error(err)
		return
...
func UpdateNetworkSettings(ctx *gin.Context) {
	...
	// Mock: Network settings are not actually applied since there are no real containers
	// Just return success
...
func GetNetworkSettings(ctx *gin.Context) {
	// Return empty settings for mock
...
func Start(ctx *gin.Context) {
	...
	err = runner.Mock.Start(ctx.Request.Context(), sandboxId, metadata)
...
func Stop(ctx *gin.Context) {
	...
	err := runner.Mock.Stop(ctx.Request.Context(), sandboxId)
	if err != nil {
		runner.StatesCache.SetSandboxState(ctx, sandboxId, enums.SandboxStateError)
		ctx.Error(err)
		return
