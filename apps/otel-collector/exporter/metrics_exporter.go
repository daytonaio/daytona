...
type metricsExporter struct {
	...
	grpcClients map[string]pmetricotlp.GRPCClient
...
func (e *metricsExporter) pushMetrics(ctx context.Context, md pmetric.Metrics) error {
	...
	e.logger.Debug("Exporting metrics",
	...
	case "grpc":
		return e.exportViaGRPC(ctx, md, endpointConfig)
	case "http", "https":
		return e.exportViaHTTP(ctx, md, endpointConfig)
	default:
		return consumererror.NewPermanent(fmt.Errorf("unsupported protocol: %s", endpointConfig.Protocol))
	}
}

func (e *metricsExporter) exportViaGRPC(ctx context.Context, md pmetric.Metrics, cfg *cache.EndpointConfig) error {
...
func (e *metricsExporter) exportViaHTTP(ctx context.Context, md pmetric.Metrics, cfg *cache.EndpointConfig) error {
...
func (e *metricsExporter) getOrCreateGRPCClient(cfg *cache.EndpointConfig) (pmetricotlp.GRPCClient, error) {
	e.mu.RLock()
	if e.grpcClients == nil {
		e.mu.RUnlock()
		e.mu.Lock()
		e.grpcClients = make(map[string]pmetricotlp.GRPCClient)
		e.mu.Unlock()
		e.mu.RLock()
	}
...
func (e *metricsExporter) getOrCreateHTTPClient(cfg *cache.EndpointConfig) *http.Client {
	e.mu.RLock()
...
func (e *metricsExporter) extractSandboxID(ctx context.Context) (string, error) {
...
func (e *metricsExporter) addHeadersToContext(ctx context.Context, headers map[string]string) context.Context {
	if len(headers) == 0 {
		return ctx
	}
