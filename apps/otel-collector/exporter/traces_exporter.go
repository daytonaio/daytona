...
type tracesExporter struct {
	...
	grpcClients map[string]ptraceotlp.GRPCClient
...
func (e *tracesExporter) pushTraces(ctx context.Context, td ptrace.Traces) error {
	...
	endpointConfig, err := e.resolver.GetEndpointConfig(ctx, sandboxID)
	if err != nil {
	...
	case "grpc":
		return e.exportViaGRPC(ctx, td, endpointConfig)
	case "http", "https":
		return e.exportViaHTTP(ctx, td, endpointConfig)
	default:
		return consumererror.NewPermanent(fmt.Errorf("unsupported protocol: %s", endpointConfig.Protocol))
	}
}

func (e *tracesExporter) exportViaGRPC(ctx context.Context, td ptrace.Traces, cfg *cache.EndpointConfig) error {
	client, err := e.getOrCreateGRPCClient(cfg)
	if err != nil {
		return fmt.Errorf("failed to create gRPC client: %w", err)
	}

	// Add headers to context
	ctx = e.addHeadersToContext(ctx, cfg.Headers)
...
func (e *tracesExporter) exportViaHTTP(ctx context.Context, td ptrace.Traces, cfg *cache.EndpointConfig) error {
...
func (e *tracesExporter) getOrCreateGRPCClient(cfg *cache.EndpointConfig) (ptraceotlp.GRPCClient, error) {
	e.mu.RLock()
	if e.grpcClients == nil {
		e.mu.RUnlock()
		e.mu.Lock()
		e.grpcClients = make(map[string]ptraceotlp.GRPCClient)
		e.mu.Unlock()
		e.mu.RLock()
	}
...
func (e *tracesExporter) getOrCreateHTTPClient(cfg *cache.EndpointConfig) *http.Client {
	e.mu.RLock()
...
func (e *tracesExporter) extractSandboxID(ctx context.Context) (string, error) {
...
func (e *tracesExporter) addHeadersToContext(ctx context.Context, headers map[string]string) context.Context {
	if len(headers) == 0 {
		return ctx
	}
...
func (e *tracesExporter) shutdown(ctx context.Context) error {
	...
	// Note: ptraceotlp.GRPCClient doesn't expose Close() directly
	// In production, you'd want to track the underlying connections and close them
	e.grpcClients = nil
	e.httpClients = nil
