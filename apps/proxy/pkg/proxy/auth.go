...
func (p *Proxy) Authenticate(ctx *gin.Context, sandboxID string) (err error, didRedirect bool) {
	...
	if authKey == "" {
		if ctx.Query(DAYTONA_SANDBOX_AUTH_KEY_QUERY_PARAM) != "" {
			...
			// Check for auth key in cookies
			cookie, err := ctx.Cookie(DAYTONA_SANDBOX_AUTH_COOKIE_NAME)
			if err == nil && cookie != "" {
				hasAccess := p.hasSandboxAccess(ctx, sandboxID, cookie)
				if !hasAccess {
