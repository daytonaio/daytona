	...
	if shouldPollUpdate {
		go func() {
			...
			for {
				select {
				case <-ticker.C:
					// Bound each tick so a hung API call can't pile up goroutines.
...
// doActivityUpdate performs one cache-checked activity POST. Cache ops use a
// non-cancellable ctx so a completed POST never loses its cache write.
