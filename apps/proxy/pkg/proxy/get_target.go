...
func (p *Proxy) GetProxyTarget(ctx *gin.Context) (*url.URL, string, map[string]string, error) {
	...
	if err != nil {
		ctx.Error(common_errors.NewBadRequestError(fmt.Errorf("failed to get runner domain: %w", err)))
		return nil, "", nil, fmt.Errorf("failed to get runner domain: %w", err)
	}

	// Build the target URL
	targetURL := fmt.Sprintf("%s/workspaces/%s/main/toolbox/proxy/%s", runnerInfo.ApiUrl, sandboxID, targetPort)
