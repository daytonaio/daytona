...
func (p *Proxy) browserWarningMiddleware() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		if ctx.Request.URL.Path == ACCEPT_PREVIEW_PAGE_WARNING_PATH || targetPort == TERMINAL_PORT || targetPort == TOOLBOX_PORT || targetPort == SESSION_DAEMON_PORT || targetPort == RECORDING_DASHBOARD_PORT {
			ctx.Next()
			return
		}
