...
import (
	...
	"github.com/daytonaio/runner-android/pkg/common"
...
func (e *Executor) createSandbox(ctx context.Context, job *apiclient.Job) (any, error) {
	...
	_, daemonVersion, err := e.cvdClient.Create(ctx, createSandboxDto)
	if err != nil {
...
func (e *Executor) startSandbox(ctx context.Context, job *apiclient.Job) (any, error) {
	...
	daemonVersion, err := e.cvdClient.Start(ctx, job.ResourceId, metadata)
	if err != nil {
		return nil, err
	}
...
func (e *Executor) stopSandbox(ctx context.Context, job *apiclient.Job) (any, error) {
	return nil, e.cvdClient.Stop(ctx, job.ResourceId)
}

func (e *Executor) destroySandbox(ctx context.Context, job *apiclient.Job) (any, error) {
	err := e.cvdClient.Destroy(ctx, job.ResourceId)
	if err != nil {
...
func (e *Executor) updateNetworkSettings(ctx context.Context, job *apiclient.Job) (any, error) {
	...
	return nil, e.cvdClient.UpdateNetworkSettings(ctx, job.ResourceId, updateNetworkSettingsDto)
}
...
func (e *Executor) forkSandbox(ctx context.Context, job *apiclient.Job) (any, error) {
	...
	info, err := e.cvdClient.ForkVM(ctx, cuttlefish.ForkOptions{
	...
	if info == nil {
		return nil, fmt.Errorf("fork not supported for Cuttlefish")
	}
...
func (e *Executor) cloneSandbox(ctx context.Context, job *apiclient.Job) (any, error) {
	...
	info, err := e.cvdClient.CloneVM(ctx, cuttlefish.CloneOptions{
		SourceSandboxId: payload.SourceSandboxId,
		NewSandboxId:    payload.NewSandboxId,
	})
	if err != nil {
	...
	if info == nil {
		return nil, fmt.Errorf("clone not supported for Cuttlefish")
	}
