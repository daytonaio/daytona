...
import (
	...
	"github.com/daytonaio/runner-android/pkg/models"
...
type MetricsServiceConfig struct {
	...
	LocalMode bool // true when running with local CVD, false for remote SSH connections
}
...
func (s *MetricsService) collectAndCacheMetrics(ctx context.Context) error {
	...
	if s.localMode {
		...
		// Remote mode: collect from remote CVD host via SSH
		s.collectRemoteSystemMetrics(ctx, metrics)
	}

	// Cuttlefish doesn't have the same snapshot concept
	metrics.SnapshotCount = 0

	// Get allocated resources from running instances
...
// collectRemoteSystemMetrics collects system metrics from the remote CVD host via SSH
func (s *MetricsService) collectRemoteSystemMetrics(ctx context.Context, metrics *models.SystemMetrics) {
	log.WithField("component", "metrics").Debug("Collecting system metrics from remote Cuttlefish host")

	remoteMetrics, err := s.cvdClient.GetRemoteMetrics(ctx)
	if err != nil {
...
func (s *MetricsService) getAllocatedResources(ctx context.Context, metrics *models.SystemMetrics) {
	...
	sandboxes, err := s.cvdClient.ListWithInfo(listCtx)
	if err != nil {
	...
	for _, info := range sandboxes {
		if info != nil {
			...
			totalAllocatedDiskGB += 20 // Default 20GB per instance
		}
	}
