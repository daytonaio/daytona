...
const (
	...
	// Default SSH user for connecting to Linux sandboxes
	// The daemon SSH server accepts any username (password-based auth)
	DEFAULT_SANDBOX_SSH_USER = "daytona"
	// Default SSH port inside the sandbox (daemon-exposed SSH)
	DEFAULT_SANDBOX_SSH_PORT = 22220
)
...
// GetSandboxSSHPort returns the SSH port inside the sandbox
// This can be overridden via the SANDBOX_SSH_PORT environment variable
func GetSandboxSSHPort() int {
	if port := os.Getenv("SANDBOX_SSH_PORT"); port != "" {
		if parsedPort, err := strconv.Atoi(port); err == nil {
			return parsedPort
		}
	}
	return DEFAULT_SANDBOX_SSH_PORT
}
