...
func (s *RunnerService) RunnerInfo(ctx context.Context, in *pb.RunnerInfoRequest) (*pb.RunnerInfoResponse, error) {
	// Get cached system metrics
	cpuUsage, ramUsage, diskUsage, allocatedCpu, allocatedMemory, allocatedDisk, snapshotCount := s.getCachedSystemMetrics(ctx)

	return &pb.RunnerInfoResponse{
		Metrics: &pb.RunnerMetrics{
...
func (s *RunnerService) getSystemMetrics() (float64, float64, float64, error) {
	cpuUsage, err := s.getCPUUsage()
	if err != nil {
		cpuUsage = -1.0
	}

	ramUsage, err := s.getRAMUsage()
	if err != nil {
		ramUsage = -1.0
	}
...
func (s *RunnerService) getAllocatedResources(ctx context.Context) (int64, int64, int64, error) {
	...
	for _, containerItem := range containers {
		cpu, memory, disk, err := s.getContainerAllocatedResources(ctx, containerItem.ID)
		if err == nil {
...
func (s *RunnerService) getSnapshotCount(ctx context.Context) (int64, error) {
...
func (s *RunnerService) getCPUUsage() (float64, error) {
	// Read initial CPU stats
	stats1, err := s.readCPUStats()
	if err != nil {
		return -1.0, err
	}
...
func (s *RunnerService) readCPUStats() (*cpuStats, error) {
...
func (s *RunnerService) getRAMUsage() (float64, error) {
...
func (s *RunnerService) getDiskUsage() (float64, error) {
	// Get disk usage for the root filesystem
	var stat syscall.Statfs_t
	err := syscall.Statfs("/", &stat)
	if err != nil {
		return s.getDiskUsageCommand()
	}
...
func (s *RunnerService) getDiskUsageCommand() (float64, error) {
...
func (s *RunnerService) getContainerAllocatedResources(ctx context.Context, containerID string) (int64, int64, int64, error) {
...
func (s *RunnerService) parseStorageQuotaGB(sizeStr string) (int64, error) {
...
func (s *RunnerService) getCachedSystemMetrics(ctx context.Context) (float64, float64, float64, int64, int64, int64, int64) {
	metrics := s.cache.GetSystemMetrics(ctx)
	if metrics == nil {
...
func (s *RunnerService) collectAndCacheMetrics(ctx context.Context) error {
	// Get current cached metrics to preserve valid values
	cachedMetrics := s.cache.GetSystemMetrics(ctx)
	...
	var finalSnapshotCount int64 = -1
	...
	cpuUsage, ramUsage, diskUsage, err := s.getSystemMetrics()
	if err == nil {
	...
	allocatedCpu, allocatedMemory, allocatedDisk, err := s.getAllocatedResources(ctx)
	if err == nil {
		// Only update if values are valid (not 0 or -1)
	...
	s.cache.SetSystemMetrics(ctx, metrics)
	return nil
}

// StartMetricsCollection starts a background goroutine that collects metrics every 20 seconds
func (s *RunnerService) StartMetricsCollection(ctx context.Context) {
	go func() {
