...
func (s *RunnerService) RunnerInfo(ctx context.Context, in *pb.RunnerInfoRequest) (*pb.RunnerInfoResponse, error) {
	...
	return &pb.RunnerInfoResponse{
		Metrics: &pb.RunnerMetrics{
			...
			CurrentSnapshotCount:         int64(metrics.SnapshotCount),
		},
	}, nil
}
...
func (s *RunnerService) getSystemMetrics(ctx context.Context) *SystemMetrics {
...
func (s *RunnerService) getAllocatedResources(ctx context.Context, metrics *SystemMetrics) {
	containers, err := s.dockerClient.ContainerList(ctx, container.ListOptions{All: true})
	if err != nil {
		s.log.Error("Error listing containers when getting allocated resources", "allocatedResources", err)
		return
	}
