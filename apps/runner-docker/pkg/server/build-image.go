...
func (s *RunnerServer) BuildImage(ctx context.Context, req *pb.BuildImageRequest) (*pb.BuildImageResponse, error) {
	if !strings.Contains(req.GetImage(), ":") || strings.HasSuffix(req.GetImage(), ":") {
		return nil, common.NewBadRequestError(errors.New("image name must include a valid tag"))
	}

	if !strings.Contains(req.GetImage(), ":") || strings.HasSuffix(req.GetImage(), ":") {
		return nil, fmt.Errorf("invalid image format: must contain exactly one colon (e.g., 'myimage:1.0')")
	}
	...
	resp, err := s.apiClient.ImageBuild(ctx, buildContext, types.ImageBuildOptions{
...
func (s *RunnerServer) tagImage(ctx context.Context, sourceImage string, targetImage string) error {
	...
	err := s.apiClient.ImageTag(ctx, sourceImage, targetImage)
	if err != nil {
		return err
	}
