...
func (s *RunnerServer) commitContainer(ctx context.Context, sandboxId, imageName string) error {
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		commitResp, err := s.apiClient.ContainerCommit(ctx, sandboxId, container.CommitOptions{
			Reference: imageName,
			Pause:     false,
		})
		if err == nil {
...
func (s *RunnerServer) pushImage(ctx context.Context, imageName string, reg *pb.Registry) error {
	...
	responseBody, err := s.apiClient.ImagePush(ctx, imageName, image.PushOptions{
