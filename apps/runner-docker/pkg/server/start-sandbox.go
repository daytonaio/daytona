...
func (s *RunnerServer) StartSandbox(ctx context.Context, req *pb.StartSandboxRequest) (*pb.StartSandboxResponse, error) {
	...
	c, err := s.apiClient.ContainerInspect(ctx, req.SandboxId)
	if err != nil {
		return nil, err
	}
	...
	err = s.apiClient.ContainerStart(ctx, req.SandboxId, container.StartOptions{})
	if err != nil {
		return nil, err
	}
...
func (s *RunnerServer) waitForContainerRunning(ctx context.Context, sandboxId string, timeout time.Duration) error {
	...
	for {
		...
		case <-ticker.C:
			c, err := s.apiClient.ContainerInspect(ctx, sandboxId)
			if err != nil {
				return err
			}
...
func (s *RunnerServer) execSync(ctx context.Context, sandboxId string, execOptions container.ExecOptions, execStartOptions container.ExecStartOptions) (*ExecResult, error) {
	...
	response, err := s.apiClient.ContainerExecCreate(ctx, sandboxId, execOptions)
	if err != nil {
		return nil, err
	}
...
func (s *RunnerServer) inspectExecResp(ctx context.Context, sandboxId string, execStartOptions container.ExecStartOptions) (*ExecResult, error) {
	resp, err := s.apiClient.ContainerExecAttach(ctx, sandboxId, execStartOptions)
	if err != nil {
		return nil, err
	}
	defer resp.Close()
	...
	res, err := s.apiClient.ContainerExecInspect(ctx, sandboxId)
	if err != nil {
		return nil, err
	}
