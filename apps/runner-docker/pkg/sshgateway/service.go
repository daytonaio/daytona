...
func NewService(dockerClient client.APIClient, log *slog.Logger) *Service {
...
func (s *Service) Start(ctx context.Context) error {
	...
	serverConfig := &ssh.ServerConfig{
		PublicKeyCallback: func(conn ssh.ConnMetadata, key ssh.PublicKey) (*ssh.Permissions, error) {
			...
			s.log.Warn("Public key authentication failed for sandbox", "sandboxId", sandboxId)
	...
	s.log.Info("SSH Gateway listening on port", "port", s.port)
	...
	for {
		...
		default:
			...
			if err != nil {
				s.log.Warn("Failed to accept incoming connection", "error", err)
				continue
			}
...
func (s *Service) handleConnection(conn net.Conn, serverConfig *ssh.ServerConfig) {
	...
	go func() {
		for req := range reqs {
			...
			s.log.Debug("Global request", "type", req.Type)
...
func (s *Service) handleChannel(newChannel ssh.NewChannel, sandboxId string) {
	s.log.Debug("New channel", "type", newChannel.ChannelType(), "sandboxId", sandboxId)
	...
	go func() {
		for req := range clientRequests {
			...
			s.log.Debug("Client request", "type", req.Type, "sandboxId", sandboxId)

			ok, err := sandboxChannel.SendRequest(req.Type, req.WantReply, req.Payload)
			if req.WantReply {
				if err != nil {
					s.log.Warn("Failed to send request to sandbox", "error", err)
	...
	go func() {
		for req := range sandboxRequests {
			...
			s.log.Debug("Sandbox request", "type", req.Type, "sandboxId", sandboxId)

			ok, err := clientChannel.SendRequest(req.Type, req.WantReply, req.Payload)
			if req.WantReply {
				if err != nil {
					s.log.Warn("Failed to send request to client", "error", err)
					if replyErr := req.Reply(false, []byte(err.Error())); replyErr != nil {
