...
func Create(ctx *gin.Context) {
	...
	_, daemonVersion, err := runner.LibVirt.Create(ctx.Request.Context(), createSandboxDto)
	if err != nil {
...
func Destroy(ctx *gin.Context) {
	...
	err := runner.LibVirt.Destroy(ctx.Request.Context(), sandboxId)
	if err != nil {
...
func CreateBackup(ctx *gin.Context) {
	...
	err = runner.LibVirt.CreateBackupAsync(ctx.Request.Context(), sandboxId, createBackupDTO)
	if err != nil {
...
func Resize(ctx *gin.Context) {
	...
	err = runner.LibVirt.Resize(ctx.Request.Context(), sandboxId, resizeDto)
	if err != nil {
		runner.StatesCache.SetSandboxState(ctx, sandboxId, enums.SandboxStateError)
		ctx.Error(err)
		return
...
func UpdateNetworkSettings(ctx *gin.Context) {
	...
	err = runner.LibVirt.UpdateNetworkSettings(ctx.Request.Context(), sandboxId, updateNetworkSettingsDto)
	if err != nil {
		ctx.Error(err)
		return
	}
...
func Info(ctx *gin.Context) {
	...
	if info.SandboxState == enums.SandboxStateStarted {
		daemonVersionStr, err := runner.LibVirt.GetDaemonVersion(ctx.Request.Context(), sandboxId)
		if err == nil {
