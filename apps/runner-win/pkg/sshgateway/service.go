...
func NewService(libvirtClient *libvirt.LibVirt) *Service {
...
// getSandboxDetails gets sandbox information via libvirt client
func (s *Service) getSandboxDetails(sandboxId string) (*SandboxDetails, error) {
	// Get container details via libvirt client
	container, err := s.libvirtClient.ContainerInspect(context.Background(), sandboxId)
	if err != nil {
