...
type Config struct {
	...
	// bind-mounted RO into sandboxes using the in-container layered backend.
	// When empty, the layered backend is disabled and sandboxes that select
	// it silently fall back to s3fuse. Typically /usr/local/bin/archil.
