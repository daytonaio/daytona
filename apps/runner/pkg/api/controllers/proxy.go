...
// PortProxyRequest handles proxying requests to a sandbox's container on a specific port
//
//	@Tags			proxy
//	@Summary		Proxy requests to a sandbox port
//	@Description	Forwards the request to the specified sandbox's container on the given port
//	@Param			sandboxId	path		string	true	"Sandbox ID"
//	@Param			port		path		string	true	"Port to forward to"
//	@Param			path		path		string	true	"Path to forward"
//	@Success		200			{object}	any		"Proxied response"
//	@Failure		400			{object}	string	"Bad request"
//	@Failure		401			{object}	string	"Unauthorized"
...
func PortProxyRequest(ctx *gin.Context) {
	proxy.NewProxyRequestHandler(getPortProxyTarget, nil)(ctx)
}

func getPortProxyTarget(ctx *gin.Context) (*url.URL, map[string]string, error) {
	runner := runner.GetInstance(nil)
	...
	// Build the target URL with the specified port
	targetURL := fmt.Sprintf("http://%s:%s", containerIP, port)
