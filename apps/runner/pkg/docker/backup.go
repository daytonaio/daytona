...
func (d *DockerClient) CreateBackup(ctx context.Context, containerId string, backupDto dto.CreateBackupDTO) error {
	...
	d.log.InfoContext(ctx, "Creating backup for container", "containerId", containerId)
...
func (d *DockerClient) CreateBackupAsync(ctx context.Context, containerId string, backupDto dto.CreateBackupDTO) error {
	...
	go func() {
		...
		if err != nil {
			d.log.ErrorContext(ctx, "Error creating backup for container", "containerId", containerId, "error", err)
		}
	}()
...
func (d *DockerClient) createBackup(containerId string, backupDto dto.CreateBackupDTO) error {
	...
	if err != nil {
		if errors.Is(err, context.Canceled) {
			...
			d.log.InfoContext(ctx, "Backup canceled for container", "containerId", containerId)
			return err
		}
		if errors.Is(err, context.DeadlineExceeded) {
			d.statesCache.SetBackupState(ctx, containerId, enums.BackupStateFailed, err)
			d.log.ErrorContext(ctx, "Backup timed out during commit", "containerId", containerId)
			return err
		}
		d.log.ErrorContext(ctx, "Error committing container", "containerId", containerId, "error", err)
		d.statesCache.SetBackupState(ctx, containerId, enums.BackupStateFailed, err)
		return err
	}
	...
	if err != nil {
		...
		if errors.Is(err, context.DeadlineExceeded) {
			d.statesCache.SetBackupState(ctx, containerId, enums.BackupStateFailed, err)
			d.log.ErrorContext(ctx, "Backup timed out during push", "containerId", containerId)
			return err
		}
		d.log.ErrorContext(ctx, "Error pushing image", "image", backupDto.Snapshot, "error", err)
		d.statesCache.SetBackupState(ctx, containerId, enums.BackupStateFailed, err)
		return err
	}
	...
	d.log.InfoContext(ctx, "Backup created successfully", "snapshot", backupDto.Snapshot, "containerId", containerId)

	err = d.RemoveImage(ctx, backupDto.Snapshot, true)
	if err != nil {
		d.log.ErrorContext(ctx, "Error removing image", "image", backupDto.Snapshot, "error", err)
