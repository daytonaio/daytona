...
func (d *DockerClient) ContainerDiskResize(ctx context.Context, sandboxId string, newStorageGB float64, cpu int64, memory int64, operationName string) error {
	d.log.InfoContext(ctx, fmt.Sprintf("Starting %s for sandbox with new storage", operationName), "sandboxId", sandboxId, "newStorageGB", newStorageGB)
	...
	if originalContainer.GraphDriver.Name == "overlay2" {
		if upperDir, ok := originalContainer.GraphDriver.Data["UpperDir"]; ok {
			...
			d.log.DebugContext(ctx, fmt.Sprintf("Overlay2 UpperDir: %s", overlayDiffPath))
		}
	}
	...
	d.log.DebugContext(ctx, "Renaming sandbox", "oldName", oldName)
	...
	d.log.DebugContext(ctx, "Setting storage", "bytes", newStorageBytes, "gigabytes", float64(newStorageBytes)/(1024*1024*1024), "filesystem", filesystem)

	// Apply CPU/memory changes if specified (0 = don't change)
	if cpu > 0 {
		newHostConfig.CPUQuota = cpu * 100000
		newHostConfig.CPUPeriod = 100000
		d.log.DebugContext(ctx, "Setting CPU quota", "cores", cpu)
	}
	if memory > 0 {
		newHostConfig.Memory = common.GBToBytes(float64(memory))
		newHostConfig.MemorySwap = newHostConfig.Memory
		d.log.DebugContext(ctx, "Setting memory", "gigabytes", memory)
	}
	...
	if overlayDiffPath != "" {
		d.log.DebugContext(ctx, "Copying data directly between overlay2 layers using rsync")
		err = d.copyContainerOverlayData(ctx, overlayDiffPath, sandboxId)
		if err != nil {
			d.log.ErrorContext(ctx, "Failed to copy overlay data", "error", err)
			d.log.WarnContext(ctx, "Old sandbox preserved as for manual data recovery", "oldName", oldName)
	...
	d.log.InfoContext(ctx, fmt.Sprintf("%s completed - container ready to be started", operationName), "sandboxId", sandboxId)
	return nil
}
...
func (d *DockerClient) copyContainerOverlayData(ctx context.Context, oldContainerOverlayPath, newContainerId string) error {
	...
	if newContainer.GraphDriver.Name == "overlay2" {
		if upperDir, ok := newContainer.GraphDriver.Data["UpperDir"]; ok {
			...
			d.log.DebugContext(ctx, "New container overlay2 UpperDir", "newUpperDir", newUpperDir)
		}
	}
	...
	d.log.DebugContext(ctx, "Copying overlay data", "from", oldContainerOverlayPath, "to", newUpperDir)
