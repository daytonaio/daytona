...
// CreateSnapshot creates a snapshot from a running container by committing it,
...
func (d *DockerClient) CreateSnapshot(ctx context.Context, snapshotDto dto.CreateSnapshotDTO) (*dto.CreateSnapshotResponseDTO, error) {
	...
	err = d.RemoveImage(ctx, snapshotPath, true)
	if err != nil {
		log.Warnf("Error removing local image %s: %v", snapshotPath, err)
		// Don't fail - the snapshot was already pushed successfully
	}

	err = d.RemoveImage(ctx, localImageName, true)
	if err != nil {
		log.Warnf("Error removing local image %s: %v", localImageName, err)
		// Don't fail - the snapshot was already pushed successfully
	}
