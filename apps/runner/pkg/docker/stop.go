...
func (d *DockerClient) Stop(ctx context.Context, containerId string) error {
	...
	if err == nil && state == enums.SandboxStateStopped {
		d.log.DebugContext(ctx, "Sandbox is already stopped", "containerId", containerId)
	...
	if err != nil {
		d.log.WarnContext(ctx, "Failed to deduce sandbox state", "containerId", containerId, "error", err)
		d.log.WarnContext(ctx, "Continuing with stop operation")
	}
	...
	d.log.DebugContext(ctx, "Sandbox stopped successfully", "containerId", containerId)
	d.statesCache.SetSandboxState(ctx, containerId, enums.SandboxStateStopped)
...
func (d *DockerClient) stopContainerWithRetry(ctx context.Context, containerId string, timeout int) error {
	...
	if err != nil {
		d.log.WarnContext(ctx, "Failed to stop sandbox for multiple attempts", "containerId", containerId, "attempts", utils.DEFAULT_MAX_RETRIES, "error", err)
		d.log.WarnContext(ctx, "Trying to kill sandbox", "containerId", containerId)
		err = d.apiClient.ContainerKill(ctx, containerId, "KILL")
		if err != nil {
			d.log.WarnContext(ctx, "Failed to kill sandbox", "containerId", containerId, "error", err)
		}
		return err
	}
	return nil
