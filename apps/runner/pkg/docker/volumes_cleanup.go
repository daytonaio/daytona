...
func (d *DockerClient) CleanupOrphanedVolumeMounts(ctx context.Context) {
	...
	d.log.InfoContext(ctx, "Volume cleanup", "dry-run", dryRun)
	...
	if err != nil {
		d.log.ErrorContext(ctx, "Volume cleanup aborted", "error", err)
		return
	}
	...
	for _, dir := range mountDirs {
		if !inUse[normalizePath(dir)] {
			if dryRun {
				d.log.InfoContext(ctx, "[DRY-RUN] Would clean orphaned volume mount", "path", dir)
			} else {
