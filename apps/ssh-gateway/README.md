...
```bash
ssh -p 2220 <TOKEN>@<GATEWAY_HOST>
```

**Example:**

```bash
ssh -p 2220 Fg8Jx2nPtWAVY5pVN0TlUcbCDNPF-ePB@localhost
```
...
4. **Connection Proxying**: Establishes a connection to the runner's SSH gateway (port 2220)
...
| ------------------ | ------------------------------------- | ----------------------- | -------- |
| `SSH_GATEWAY_PORT` | Port for the SSH gateway to listen on | `2220`                  | No       |
