...
func (g *SSHGateway) Start() error {
	serverConfig := &ssh.ServerConfig{
		...
		PublicKeyCallback: func(conn ssh.ConnMetadata, key ssh.PublicKey) (*ssh.Permissions, error) {
			...
			validation, err := g.apiClient.ValidateSSHAccess(token)
			if err != nil {
			...
			return &ssh.Permissions{
				Extensions: map[string]string{
					...
					"runner-domain": validation.RunnerDomain,
					"token":         token,
...
func (g *SSHGateway) handleConnection(conn net.Conn, serverConfig *ssh.ServerConfig) {
	...
	if sandboxId != "" {
		...
		sandboxState, err := g.apiClient.GetSandboxState(sandboxId)
		if err != nil {
		...
		status := strings.ToLower(strings.TrimSpace(sandboxState.State))
		log.Printf("Sandbox %s state: %s", sandboxId, status)
		if status != "started" {
			log.Printf("Sandbox %s is not started (state: %s), closing connection", sandboxId, status)
			g.sendErrorAndClose(conn, fmt.Sprintf("Sandbox is not started (state: %s). Please start the sandbox before attempting to connect.", status))
			return
		}
...
func (g *SSHGateway) connectToRunner(sandboxId string, runnerDomain string, signer ssh.Signer) (*ssh.Client, error) {
	...
	if strings.Contains(host, "localtest.me") && strings.Index(host, ":") != -1 {
