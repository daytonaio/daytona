...
func EnsureSshConfigEntryAdded(profileId, resourceId string, gpgKey *string) error {
	...
	newContent, err := appendSshConfigEntry(configPath, profileId, resourceId, knownHostsFile, false, string(existingContent))
	if err != nil {
		return err
	}
	...
	if gpgKey != nil {
		...
		_, err := appendSshConfigEntry(configPath, profileId, resourceId, knownHostsFile, true, newContent)
		if err != nil {
			return err
		}
		resourceHostname := GetHostname(profileId, resourceId)
		err = ExportGPGKey(*gpgKey, resourceHostname)
		if err != nil {
			return err
		}
	}
