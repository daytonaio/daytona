...
from daytona import CreateSandboxFromSnapshotParams, Daytona, ScreenshotOptions, ScreenshotRegion
...
def main():
    ...
    # --- Display Information ---
    print("\n=== Display Information ===")
    display_info = sandbox.computer_use.display.get_info()
    ...
    screenshot_cursor = sandbox.computer_use.screenshot.take_full_screen(show_cursor=True)
    ...
    try:
        ...
        screenshot_region = sandbox.computer_use.screenshot.take_region(region)
    ...
    try:
        ...
        screenshot_compressed = sandbox.computer_use.screenshot.take_compressed(options)
    ...
    new_position = sandbox.computer_use.mouse.move(400, 300)
    print(f"Moved mouse to: ({new_position.x}, {new_position.y})")

    # Perform a click
    click_result = sandbox.computer_use.mouse.click(500, 400, button="left")
    print(f"Clicked at: ({click_result.x}, {click_result.y})")

    # Double click
    double_click = sandbox.computer_use.mouse.click(500, 400, button="left", double=True)
    print(f"Double-clicked at: ({double_click.x}, {double_click.y})")

    # Right click
    right_click = sandbox.computer_use.mouse.click(600, 400, button="right")
    print(f"Right-clicked at: ({right_click.x}, {right_click.y})")

    # Scroll the mouse wheel
    scroll_result = sandbox.computer_use.mouse.scroll(500, 400, direction="down", amount=3)
    print(f"Scrolled: {scroll_result}")

    # Drag operation
    drag_result = sandbox.computer_use.mouse.drag(100, 100, 300, 200, button="left")
    ...
    # Type some text
    print("Typing text...")
    sandbox.computer_use.keyboard.type("Hello from Daytona SDK!")
    ...
    sandbox.computer_use.keyboard.hotkey("ctrl+a")
    ...
    print("\n=== Final Screenshot ===")
    final_screenshot = sandbox.computer_use.screenshot.take_full_screen(show_cursor=True)
    save_screenshot(final_screenshot.screenshot, "screenshot_final.png")

    # --- Stop Computer Use ---
    print("\n=== Stopping Computer Use ===")
    stop_result = sandbox.computer_use.stop()
