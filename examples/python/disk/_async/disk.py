...
async def main():
    async with AsyncDaytona() as daytona:
        print("🚀 Starting Async Disk Management Example")
        print("==========================================")

        try:
            # List all existing disks
            print("\n📋 Listing all disks...")
            existing_disks = await daytona.disk.list()
            ...
            disk_size = 20  # 20GB
            disk = await daytona.disk.create(disk_name, disk_size)
            print(f"✅ Created disk: {disk.name} ({disk.id}) - {disk.size}GB - State: {disk.state}")

            # Get the disk by ID
            print("\n🔍 Getting disk details...")
            retrieved_disk = await daytona.disk.get(disk.id)
            ...
            updated_disks = await daytona.disk.list()
            ...
            print("\n📋 Final disk list...")
            final_disks = await daytona.disk.list()
            print(f"Found {len(final_disks)} disks after cleanup")

            print("\n🎉 Async disk management example completed successfully!")
