...
func ForwardPort(targetId, workspaceName string, targetPort uint16, profile config.Profile) (*uint16, chan error) {
	hostPort := targetPort
	...
	go func() {
		for {
			...
			targetUrl := fmt.Sprintf("%s:%d", workspace.GetWorkspaceHostname(targetId, workspaceName), targetPort)
