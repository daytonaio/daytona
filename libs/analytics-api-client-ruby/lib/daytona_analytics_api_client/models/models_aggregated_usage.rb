...
module DaytonaAnalyticsApiClient
  class ModelsAggregatedUsage < ApiModelBase
    attr_accessor :first_start

    attr_accessor :last_end

    attr_accessor :sandbox_count
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DaytonaAnalyticsApiClient::ModelsAggregatedUsage` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DaytonaAnalyticsApiClient::ModelsAggregatedUsage`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'sandbox_count')
        self.sandbox_count = attributes[:'sandbox_count']
      end
    ...
    def hash
      [first_start, last_end, sandbox_count, total_cpu_seconds, total_disk_gb_seconds, total_gpu_seconds, total_price, total_ramgb_seconds].hash
    end
