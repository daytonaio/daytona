...
type VolumesAPI interface {
	...
	/*
	ChangeVolumeBackend Change a volume's backend
	...
	@return VolumesAPIChangeVolumeBackendRequest
	*/
	ChangeVolumeBackend(ctx context.Context, volumeId string) VolumesAPIChangeVolumeBackendRequest

	// ChangeVolumeBackendExecute executes the request
	//  @return VolumeDto
	ChangeVolumeBackendExecute(r VolumesAPIChangeVolumeBackendRequest) (*VolumeDto, *http.Response, error)
...
type VolumesAPIChangeVolumeBackendRequest struct {
	ctx context.Context
	ApiService VolumesAPI
	volumeId string
	changeVolumeBackend *ChangeVolumeBackend
	xDaytonaOrganizationID *string
}

func (r VolumesAPIChangeVolumeBackendRequest) ChangeVolumeBackend(changeVolumeBackend ChangeVolumeBackend) VolumesAPIChangeVolumeBackendRequest {
	r.changeVolumeBackend = &changeVolumeBackend
	return r
}

// Use with JWT to specify the organization ID
func (r VolumesAPIChangeVolumeBackendRequest) XDaytonaOrganizationID(xDaytonaOrganizationID string) VolumesAPIChangeVolumeBackendRequest {
	r.xDaytonaOrganizationID = &xDaytonaOrganizationID
	return r
}

func (r VolumesAPIChangeVolumeBackendRequest) Execute() (*VolumeDto, *http.Response, error) {
	return r.ApiService.ChangeVolumeBackendExecute(r)
}
...
Switches an existing volume between the s3fuse and experimental backends in place. The volume's S3 bucket and data are preserved; only the mount strategy changes. Refuses to switch while any sandbox referencing the volume is running.
 ...
 @return VolumesAPIChangeVolumeBackendRequest
*/
func (a *VolumesAPIService) ChangeVolumeBackend(ctx context.Context, volumeId string) VolumesAPIChangeVolumeBackendRequest {
	return VolumesAPIChangeVolumeBackendRequest{
		ApiService: a,
		ctx: ctx,
		volumeId: volumeId,
...
//  @return VolumeDto
func (a *VolumesAPIService) ChangeVolumeBackendExecute(r VolumesAPIChangeVolumeBackendRequest) (*VolumeDto, *http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "VolumesAPIService.ChangeVolumeBackend")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/volumes/{volumeId}/backend"
	...
	if r.changeVolumeBackend == nil {
		return localVarReturnValue, nil, reportError("changeVolumeBackend is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.changeVolumeBackend
