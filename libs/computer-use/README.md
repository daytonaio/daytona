...
if err := computerUse.RestartProcess("xfce4"); err != nil {
    log.Printf("Failed to restart xfce4: %v", err)
}

// Get process logs
if logs, err := computerUse.GetProcessLogs("xfce4"); err == nil {
    log.Printf("Xfce4 logs: %s", logs)
}

// Get process errors
if errors, err := computerUse.GetProcessErrors("xfce4"); err == nil {
    log.Printf("Xfce4 errors: %s", errors)
}
```
