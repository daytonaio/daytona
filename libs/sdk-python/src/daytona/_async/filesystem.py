...
class AsyncFileSystem:
    ...
    async def search(self, request: SearchRequest) -> SearchResults:
        ...
        Example:
            ...
            results = await sandbox.fs.search(SearchRequest(
                query="TODO",
                path="src/",
        ...
        if request.path:
            request.path = prefix_relative_path(await self._get_root_dir(), request.path)
        else:
            request.path = "."

        # Use the standard toolbox API method
        return await self._toolbox_api.search_content(self._sandbox_id, search_request=request)
