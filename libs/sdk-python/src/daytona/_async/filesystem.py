...
class AsyncFileSystem:
    ...
    @overload
    async def download_file(self, *, options: DownloadFileOptions) -> bytes | None:
    ...
    @with_instrumentation()
    async def download_file(self, *args: object, **kwargs: object) -> bytes | None:  # type: ignore[override]
        if "options" in kwargs and isinstance(kwargs["options"], DownloadFileOptions):
            return await self._download_file_with_options(kwargs["options"])
    ...
    async def _download_file_with_options(self, options: DownloadFileOptions) -> bytes | None:
        ...
        try:
            if self._is_io_destination(options.destination):
                await self._download_file_to_stream_direct(options)
                return None
            ...
            raise DownloadAbortedError() from None

    async def _download_file_to_stream_direct(self, options: DownloadFileOptions) -> None:
    ...
    @overload
    async def upload_file(self, *, options: UploadFileOptions) -> None:
        """Upload via options bag with streaming, abort, and progress. See UploadFileOptions.

        Note:
            In the async variant, ``on_progress`` is invoked from an httpx worker thread,
            not the asyncio event loop. Callbacks must be thread-safe. Use
            ``loop.call_soon_threadsafe()`` to dispatch asyncio operations from the callback.
        """
    ...
    @with_instrumentation()
    async def upload_file(self, *args: object, **kwargs: object) -> None:  # type: ignore[override]
        if "options" in kwargs and isinstance(kwargs["options"], UploadFileOptions):
            await self._upload_file_with_options(kwargs["options"])
            return
    ...
    async def _upload_file_with_options(self, options: UploadFileOptions) -> None:
        ...
        try:
            ...
            if options.cleanup_on_abort:
                try:
                    await self.delete_file(options.destination)
                except Exception as e:
            ...
            if options.cleanup_on_abort:
                try:
                    ...
                    warnings.warn(f"Upload abort cleanup failed: {e}", stacklevel=2)
            raise UploadAbortedError() from None
