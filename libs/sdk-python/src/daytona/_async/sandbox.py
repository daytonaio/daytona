...
class AsyncSandbox(SandboxDto):
    ...
    async def create_signed_download_url(
        self, path: str, expires_in_seconds: int | None = None
    ) -> SignedFileDownloadUrl:
        ...
        Example:
            ```python
            download = await sandbox.create_signed_download_url("/home/daytona/report.pdf")
            print(f"Download URL: {download.url}")
            # Share download.url - no authentication needed
            ```
        """
        return await self._sandbox_api.get_signed_file_download_url(
            self.id, path=path, expires_in_seconds=expires_in_seconds
        )

    @intercept_errors(message_prefix="Failed to expire signed file download url: ")
    async def expire_signed_download_url(self, token: str) -> None:
        """Expires a signed file download URL, making it immediately invalid.

        Args:
            token (str): The token of the signed download URL to expire.
        """
        await self._sandbox_api.expire_signed_file_download_url(self.id, token)
