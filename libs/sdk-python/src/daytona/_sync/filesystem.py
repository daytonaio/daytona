...
from ..common.file_watcher import FilesystemEvent, FilesystemEventType, FileWatchCallback, SyncWatchHandle, WatchOptions
...
class FileSystem:
    ...
    def delete_file(self, path: str) -> None:
        ...
        self._toolbox_api.delete_file(
            self._sandbox_id, path=prefix_relative_path(self._get_root_dir(), path)
        )
    ...
    def get_file_info(self, path: str) -> FileInfo:
        ...
        Example:
            ...
            ```
        """
        return self._toolbox_api.get_file_info(
            self._sandbox_id, path=prefix_relative_path(self._get_root_dir(), path)
        )
    ...
    def list_files(self, path: str) -> List[FileInfo]:
        ...
        Example:
            ...
            ```
        """
        return self._toolbox_api.list_files(
            self._sandbox_id, path=prefix_relative_path(self._get_root_dir(), path)
        )
    ...
    def watch_dir(
        ...
        # Create a thread-safe event for stopping
        ...
        def handle_websocket():
            try:
                ...
                # Run the async handler in the current thread's event loop
                asyncio.run(async_handler())
            except Exception as e:
                print(f"WebSocket connection error: {e}")

        # Start the WebSocket handler in a separate thread
        websocket_thread = threading.Thread(target=handle_websocket, daemon=True)
        ...
        return SyncWatchHandle(close_func)
