...
# Proxy.
class DaytonaSandboxNotFoundError(DaytonaNotFoundError):
    """The targeted sandbox does not exist."""


class DaytonaSandboxNotStartedError(DaytonaBadRequestError):
    """The targeted sandbox is not running."""


class DaytonaRunnerUnreachableError(DaytonaBadGatewayError):
    """The runner backend is unreachable."""


# API.
class DaytonaApiKeyExpiredError(DaytonaAuthenticationError):
    """The API key used for the request has expired."""


class DaytonaOrganizationSuspendedError(DaytonaForbiddenError):
    """The organization is suspended; operation is not allowed."""


class DaytonaSandboxDiskExpansionLimitError(DaytonaForbiddenError):
    """The sandbox disk expansion limit was reached."""


class DaytonaSandboxRunnerNotFoundError(DaytonaNotFoundError):
    """The runner backing the sandbox could not be located."""


class DaytonaSandboxStateChangeInProgressError(DaytonaConflictError):
    """The sandbox is in the middle of a state change; retry shortly."""


class DaytonaVolumeInUseError(DaytonaConflictError):
    """The volume is currently attached and cannot be deleted."""


class DaytonaDefaultRegionRequiredError(DaytonaBadRequestError):
    """A default region must be configured for this operation."""


class DaytonaNoAvailableRunnersError(DaytonaBadRequestError):
    """No runner is available to satisfy the request."""


class DaytonaOrganizationQuotaExceededError(DaytonaBadRequestError):
    """The organization's quota was exceeded."""


class DaytonaSandboxBackupStateError(DaytonaBadRequestError):
    """The sandbox's backup is in an invalid state for this operation."""


class DaytonaSandboxOperationNotSupportedError(DaytonaBadRequestError):
    """The requested operation is not supported for this sandbox."""


class DaytonaSandboxStateError(DaytonaBadRequestError):
    """The sandbox state does not allow this operation."""


class DaytonaSnapshotStateChangeInProgressError(DaytonaBadRequestError):
...
CODE_TO_ERROR: dict[tuple[str, str], type[DaytonaError]] = {
    ...
    # Proxy
    (SOURCE_PROXY, "SANDBOX_NOT_FOUND"): DaytonaSandboxNotFoundError,
    (SOURCE_PROXY, "SANDBOX_NOT_STARTED"): DaytonaSandboxNotStartedError,
    (SOURCE_PROXY, "RUNNER_UNREACHABLE"): DaytonaRunnerUnreachableError,
    # API
    (SOURCE_API, "API_KEY_EXPIRED"): DaytonaApiKeyExpiredError,
    (SOURCE_API, "ORGANIZATION_SUSPENDED"): DaytonaOrganizationSuspendedError,
    (SOURCE_API, "SANDBOX_DISK_EXPANSION_LIMIT"): DaytonaSandboxDiskExpansionLimitError,
    (SOURCE_API, "SANDBOX_RUNNER_NOT_FOUND"): DaytonaSandboxRunnerNotFoundError,
    (SOURCE_API, "SANDBOX_STATE_CHANGE_IN_PROGRESS"): DaytonaSandboxStateChangeInProgressError,
    (SOURCE_API, "VOLUME_IN_USE"): DaytonaVolumeInUseError,
    (SOURCE_API, "DEFAULT_REGION_REQUIRED"): DaytonaDefaultRegionRequiredError,
    (SOURCE_API, "NO_AVAILABLE_RUNNERS"): DaytonaNoAvailableRunnersError,
    (SOURCE_API, "ORGANIZATION_QUOTA_EXCEEDED"): DaytonaOrganizationQuotaExceededError,
    (SOURCE_API, "SANDBOX_BACKUP_STATE_ERROR"): DaytonaSandboxBackupStateError,
    (SOURCE_API, "SANDBOX_OPERATION_NOT_SUPPORTED"): DaytonaSandboxOperationNotSupportedError,
    (SOURCE_API, "SANDBOX_STATE_ERROR"): DaytonaSandboxStateError,
    (SOURCE_API, "SNAPSHOT_STATE_CHANGE_IN_PROGRESS"): DaytonaSnapshotStateChangeInProgressError,
}
