...
import { processStreamingResponse, stdDemuxStream } from './utils/Stream'
...
export class Process {
   ...
   * console.log('Command output:', logs.stdout);
   * console.log('Command error:', logs.stderr);
   */
  ...
  public async getSessionCommandLogs(
    ...
    const url = `${previewLink.url.replace('http', 'ws')}/process/session/${sessionId}/command/${commandId}/logs?follow=true`//&DAYTONA_SANDBOX_AUTH_KEY=${previewLink.token}`

    let ws: WebSocket
    if (RUNTIME === Runtime.BROWSER) {
      ws = new WebSocket(url + '&DAYTONA_SANDBOX_AUTH_KEY=' + previewLink.token)
    } else {
      ws = new WebSocket(
        url,
        {
