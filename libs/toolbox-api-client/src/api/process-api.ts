...
export const ProcessApiAxiosParamCreator = function (configuration?: Configuration) {
    return {
         ...
         * @throws {RequiredError}
         */
        terminateSessionCommand: async (sessionId: string, commandId: string, options: RawAxiosRequestConfig = {}): Promise<RequestArgs> => {
            // verify required parameter 'sessionId' is not null or undefined
            assertParamExists('terminateSessionCommand', 'sessionId', sessionId)
            // verify required parameter 'commandId' is not null or undefined
            assertParamExists('terminateSessionCommand', 'commandId', commandId)
            const localVarPath = `/process/session/{sessionId}/command/{commandId}/terminate`
                .replace(`{${"sessionId"}}`, encodeURIComponent(String(sessionId)))
                .replace(`{${"commandId"}}`, encodeURIComponent(String(commandId)));
            // use dummy base URL string because the URL constructor only accepts absolute URLs.
            const localVarUrlObj = new URL(localVarPath, DUMMY_BASE_URL);
...
export const ProcessApiFp = function(configuration?: Configuration) {
    ...
    return {
         ...
         * @throws {RequiredError}
         */
        async terminateSessionCommand(sessionId: string, commandId: string, options?: RawAxiosRequestConfig): Promise<(axios?: AxiosInstance, basePath?: string) => AxiosPromise<void>> {
            const localVarAxiosArgs = await localVarAxiosParamCreator.terminateSessionCommand(sessionId, commandId, options);
            const localVarOperationServerIndex = configuration?.serverIndex ?? 0;
            const localVarOperationServerBasePath = operationServerMap['ProcessApi.terminateSessionCommand']?.[localVarOperationServerIndex]?.url;
            return (axios, basePath) => createRequestFunction(localVarAxiosArgs, globalAxios, BASE_PATH, configuration)(axios, localVarOperationServerBasePath || basePath);
        },
    }
};
...
export const ProcessApiFactory = function (configuration?: Configuration, basePath?: string, axios?: AxiosInstance) {
    ...
    return {
         ...
         * @throws {RequiredError}
         */
        terminateSessionCommand(sessionId: string, commandId: string, options?: RawAxiosRequestConfig): AxiosPromise<void> {
            return localVarFp.terminateSessionCommand(sessionId, commandId, options).then((request) => request(axios, basePath));
        },
    };
};
...
export class ProcessApi extends BaseAPI {
     ...
     * @memberof ProcessApi
     */
    public terminateSessionCommand(sessionId: string, commandId: string, options?: RawAxiosRequestConfig) {
        return ProcessApiFp(this.configuration).terminateSessionCommand(sessionId, commandId, options).then((request) => request(this.axios, this.basePath));
    }
}
