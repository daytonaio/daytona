...
async def click(x: int, y: int, button: str = "left", double: bool = False) -> MouseClickResponse
```
...
async def drag(start_x: int, start_y: int, end_x: int, end_y: int, button: str = "left") -> MouseDragResponse
```
...
async def take_region(region: ScreenshotRegion,                      show_cursor: bool = False) -> RegionScreenshotResponse
```
...
async def take_compressed(options: Optional[ScreenshotOptions] = None) -> CompressedScreenshotResponse
```
...
async def take_compressed_region(region: ScreenshotRegion, options: Optional[ScreenshotOptions] = None) -> CompressedScreenshotResponse
```
