...
<Frame>![](/images/sandbox-states.png)</Frame>
...
The Daytona SDK provides an option to create Sandboxes with default or custom configurations. You can specify the language, [Snapshot](/snapshots), resources, environment variables, and volumes for the Sandbox.
Running Sandboxes utilize CPU, memory, and disk storage. Every resource is charged per second of usage.

<Tip>
  If you want to prolong the auto-stop interval, you can [set the auto-stop interval
  parameter](/sandbox-management#auto-stop-interval) when creating a Sandbox.
</Tip>
...
When Sandboxes are not actively used, it is recommended that they be stopped. This can be done manually [using the stop command](/sandbox-management#stop-and-start-sandbox) or automatically by [setting the auto-stop interval](/sandbox-management#auto-stop-and-auto-archive).
...
<Note>
  All resource parameters are optional. If not specified, Daytona will use default values appropriate for the selected
...
To get the preview URL for a specific port, check out [Preview & Authentication](/preview-and-authentication).
...
<Tip>
  Check out the [Daytona CLI](/getting-started#setting-up-the-daytona-cli) if you prefer managing Sandboxes through the terminal:
