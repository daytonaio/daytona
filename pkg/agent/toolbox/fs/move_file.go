...
func MoveFile(c *gin.Context) {
	...
	if sourcePath == "" || destPath == "" {
		c.JSON(400, gin.H{"error": "source and destination paths are required"})
		return
	}
	...
	if err != nil {
		...
		c.JSON(http.StatusInternalServerError, gin.H{"error": "unable to access source file"})
		return
	}
	...
	if _, err := os.Stat(absDestPath); err == nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "destination already exists"})
		return
	}
