...
func (a *ApiServer) Start() error {
	...
	projectConfigController := protected.Group("/project-config")
	{
		...
		projectConfigPrebuildsGroup := projectConfigController.Group("/prebuild")
		{
			...
			projectConfigPrebuildsGroup.GET("/:prebuildId", prebuild.FindPrebuild)
			projectConfigPrebuildsGroup.POST("/", prebuild.SetPrebuild)
			projectConfigPrebuildsGroup.DELETE("/", prebuild.DeletePrebuild)
		}
