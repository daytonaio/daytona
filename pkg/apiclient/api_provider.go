...
func (a *ProviderAPIService) InstallProviderExecute(r ApiInstallProviderRequest) (*http.Response, error) {
	...
	localVarPath := localBasePath + "/runner/provider/install"
...
type ApiListProvidersForRunnerRequest struct {
	ctx        context.Context
	ApiService *ProviderAPIService
}

func (r ApiListProvidersForRunnerRequest) Execute() ([]ProviderInfo, *http.Response, error) {
	return r.ApiService.ListProvidersForRunnerExecute(r)
}

/*
ListProvidersForRunner List providers for runner

List providers for runner

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return ApiListProvidersForRunnerRequest
*/
func (a *ProviderAPIService) ListProvidersForRunner(ctx context.Context) ApiListProvidersForRunnerRequest {
	return ApiListProvidersForRunnerRequest{
		ApiService: a,
		ctx:        ctx,
	}
...
//
//	@return []ProviderInfo
func (a *ProviderAPIService) ListProvidersForRunnerExecute(r ApiListProvidersForRunnerRequest) ([]ProviderInfo, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodGet
		localVarPostBody    interface{}
		formFiles           []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "ProviderAPIService.ListProvidersForRunner")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}
...
func (r ApiUpdateProviderRequest) ProviderDownloadUrlsDTO(providerDownloadUrlsDTO map[string]string) ApiUpdateProviderRequest {
	r.providerDownloadUrlsDTO = &providerDownloadUrlsDTO
	return r
}
...
func (a *ProviderAPIService) UpdateProviderExecute(r ApiUpdateProviderRequest) (*http.Response, error) {
	...
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.providerDownloadUrlsDTO == nil {
		return nil, reportError("providerDownloadUrlsDTO is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.providerDownloadUrlsDTO
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["Bearer"]; ok {
