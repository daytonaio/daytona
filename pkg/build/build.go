...
func (b *Build) Compare(other *Build) (bool, error) {
	if b.BuildConfig != nil && *b.BuildConfig == (buildconfig.BuildConfig{}) {
		bHash, err := b.getBuildHashWithoutBuildConfig()
		if err != nil {
			return false, err
		}
		buildHash, err := other.getBuildHashWithoutBuildConfig()
		if err != nil {
			return false, err
		}
		return bHash == buildHash, nil
	}

	bHash, err := b.GetBuildHash()
	if err != nil {
		return false, err
	}

	buildHash, err := other.GetBuildHash()
	if err != nil {
		return false, err
	}
