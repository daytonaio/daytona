...
func TestBuildRunner(t *testing.T) {
	...
	s.Runner.GitService = git_mocks.NewMockGitService()
...
func (s *BuildRunnerTestSuite) TestRunBuildProcess() {
	...
	logger := logger_mocks.NewMockLogger()
	logger.On("Write", mock.Anything).Return(0, nil)

	s.Runner.RunBuildProcess(&s.mockBuilder, logger, mocks.MockBuild, "", nil)
