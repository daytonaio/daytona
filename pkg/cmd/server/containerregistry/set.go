	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if serverFlag == "" || usernameFlag == "" || passwordFlag == "" {
			if len(containerRegistries) == 0 {
				...
				selectedUsername = registryView.Username
			} else {
		...
		res, err = apiClient.ContainerRegistryAPI.SetContainerRegistry(context.Background(), url.QueryEscape(selectedServer), selectedUsername).ContainerRegistry(*registryDto).Execute()
		if err != nil {
			log.Fatal(apiclient.HandleErrorResponse(res, err))
		}
