...
var targetSetCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		target_util.TargetCreationFlow(target_util.TargetCreationParams{
			Ctx:               ctx,
			ApiClient:         apiClient,
		...
		if selectedTargetConfig.Name == targetconfig.NewTargetConfigName {
			...
			res, err = apiClient.TargetConfigAPI.SetTargetConfig(ctx).TargetConfig(apiclient.CreateTargetConfigDTO{
		...
		targetName = selectedTargetConfig.Name

		target_view.SetTargetNameView(&targetName, util.ArrayMap(targets, func(t apiclient.TargetDTO) string {
			return t.Name
		}))
