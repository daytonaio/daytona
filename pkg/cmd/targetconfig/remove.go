...
var removeCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if len(args) == 0 {
			...
			selectedTarget, err := targetconfig.GetTargetConfigFromPrompt(targetConfigs, activeProfile.Name, nil, false, "Remove")
			if err != nil {
			...
			selectedConfigName = selectedTarget.Name
		} else {
		...
		views.RenderInfoMessageBold(fmt.Sprintf("Target %s removed successfully", selectedConfigName))
		return nil
	},
}
