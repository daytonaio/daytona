...
func processGitURL(repoUrl string, apiClient *apiclient.APIClient, projects *[]apiclient.CreateProjectDTO, ctx context.Context) (*string, error) {
	...
	newRepoUrl, res, err := apiClient.GitProviderAPI.GetUrlFromRepository(ctx).Repository(*repo).Execute()
	if err != nil {
		return nil, apiclient_util.HandleErrorResponse(res, err)
	}

	repo, res, err = apiClient.GitProviderAPI.GetGitContext(ctx, url.QueryEscape(newRepoUrl.Url)).Execute()
	if err != nil {
		return nil, apiclient_util.HandleErrorResponse(res, err)
	}
