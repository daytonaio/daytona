  ...
  RunE: func(cmd *cobra.Command, args []string) error {
  	if deleteAllFlag {
  		if deleteYesFlag {
  			...
  			err := DeleteAllWorkspaces(deleteForceFlag)
  			if err != nil {
  				return err
  			}
  		} else {
  	...
  	if !deleteYesFlag {
  		...
  		for _, workspace := range workspaceDeleteList {
  			err := RemoveWorkspace(ctx, apiClient, workspace, deleteForceFlag)
  			if err != nil {
...
func init() {
  DeleteCmd.Flags().BoolVarP(&deleteAllFlag, "all", "a", false, "Delete all workspaces")
  DeleteCmd.Flags().BoolVarP(&deleteYesFlag, "yes", "y", false, "Confirm deletion without prompt")
  DeleteCmd.Flags().BoolVarP(&deleteForceFlag, "force", "f", false, "Delete a workspace by force")
}
