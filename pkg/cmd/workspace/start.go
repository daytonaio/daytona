...
var startCodeFlag bool
var startYesFlag bool
  	...
  	if len(selectedWorkspacesNames) == 1 {
  		...
  		if startProjectFlag == "" {
  			...
  			if startCodeFlag {
  				...
  				err = openIDE(ideId, activeProfile, workspaceId, startProjectFlag, projectProviderMetadata, startYesFlag, gpgKey)
  				if err != nil {
  					return err
  				}
  			}
...
func init() {
  ...
  StartCmd.PersistentFlags().BoolVarP(&startAllFlag, "all", "a", false, "Start all workspaces")
  StartCmd.PersistentFlags().BoolVarP(&startCodeFlag, "code", "c", false, "Open the workspace in the IDE after workspace start")
  StartCmd.PersistentFlags().BoolVarP(&startYesFlag, "yes", "y", false, "Automatically confirm any prompts")
