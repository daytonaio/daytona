	...
	for i := 1; config.MultiProject || i == 1; i++ {
		...
		if len(config.ProjectConfigs) > 0 && !config.BlankProject {
			...
			if projectConfig.Name != selection.BlankProjectIdentifier {
				...
				configRepo, res, err := config.ApiClient.GitProviderAPI.GetGitContext(context.Background(), url.QueryEscape(projectConfig.RepositoryUrl)).Execute()
				if err != nil {
				...
				if branch != nil {
					configRepo.Branch = &branch.Name
					configRepo.Sha = branch.Sha
				}
