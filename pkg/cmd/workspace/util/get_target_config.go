...
func GetTargetConfig(params GetTargetConfigParams) (*targetconfig.TargetConfigView, error) {
	...
	targetManifest, res, err := params.ApiClient.ProviderAPI.GetTargetConfigManifest(context.Background(), selectedTargetConfig.ProviderInfo.Name).Execute()
	if err != nil {
		return nil, apiclient_util.HandleErrorResponse(res, err)
	}
	...
	err = targetconfig.SetTargetConfigForm(selectedTargetConfig, *targetManifest)
	if err != nil {
		return nil, err
	}
