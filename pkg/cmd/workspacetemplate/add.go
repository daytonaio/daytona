...
var workspaceTemplateAddCmd = &cobra.Command{
	Use:     "add",
...
func RunWorkspaceTemplateAddFlow(apiClient *apiclient.APIClient, gitProviders []apiclient.GitProvider, ctx context.Context) (*apiclient.WorkspaceTemplate, error) {
	...
	res, err = apiClient.WorkspaceTemplateAPI.SetWorkspaceTemplate(ctx).WorkspaceTemplate(createWorkspaceTemplate).Execute()
	if err != nil {
		return nil, apiclient_util.HandleErrorResponse(res, err)
	}
	...
	if createWorkspaceTemplate.GitProviderConfigId == nil && *createWorkspaceTemplate.GitProviderConfigId == "" {
		gitProviderConfigId, res, err := apiClient.GitProviderAPI.GetGitProviderIdForUrl(ctx, url.QueryEscape(createWorkspaceTemplate.RepositoryUrl)).Execute()
		if err != nil {
			return nil, apiclient_util.HandleErrorResponse(res, err)
		}
...
func init() {
	workspaceTemplateAddCmd.Flags().StringVar(&nameFlag, "name", "", "Specify the workspace template name")
	cmd_common.AddWorkspaceConfigurationFlags(workspaceTemplateAddCmd, workspaceConfigurationFlags, false)
}
