...
func (s *ContainerRegistryStore) Find(server, username string) (*containerregistry.ContainerRegistry, error) {
	containerRegistryDTO := ContainerRegistryDTO{}
	tx := s.db.Where("server = ? AND username = ?", server, username).First(&containerRegistryDTO)
	if tx.Error != nil {
		return nil, tx.Error
	}
