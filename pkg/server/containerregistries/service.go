...
type IContainerRegistryService interface {
	Delete(server, username string) error
	Find(server, username string) (*containerregistry.ContainerRegistry, error)
...
func (s *ContainerRegistryService) Find(server, username string) (*containerregistry.ContainerRegistry, error) {
	return s.store.Find(server, username)
}
...
func (s *ContainerRegistryService) Delete(server, username string) error {
	cr, err := s.Find(server, username)
	if err != nil {
		return err
	}
