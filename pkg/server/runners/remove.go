...
func (s *RunnerService) RemoveRunner(ctx context.Context, runnerId string) error {
	...
	if err != nil {
		return s.handleRemoveError(ctx, runner, err)
	}
	...
	err = s.revokeApiKey(ctx, runner.Id)
	if err != nil {
		return s.handleRemoveError(ctx, runner, err)
	}
...
func (s *RunnerService) handleRemoveError(ctx context.Context, r *models.Runner, err error) error {
	if err != nil {
