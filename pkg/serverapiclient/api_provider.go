...
/*
RemoveTarget Set a provider target
...
*/
func (a *ProviderAPIService) RemoveTarget(ctx context.Context, provider string, target string) ApiRemoveTargetRequest {
	return ApiRemoveTargetRequest{
		ApiService: a,
		ctx: ctx,
		provider: provider,
...
// Execute executes the request
func (a *ProviderAPIService) RemoveTargetExecute(r ApiRemoveTargetRequest) (*http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "ProviderAPIService.RemoveTarget")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/provider/{provider}/{target}"
	localVarPath = strings.Replace(localVarPath, "{"+"provider"+"}", url.PathEscape(parameterValueToString(r.provider, "provider")), -1)
...
// Provider target to set
func (r ApiSetTargetRequest) Target(target TargetDTO) ApiSetTargetRequest {
	r.target = &target
	return r
}
...
/*
SetTarget Set a provider target
...
*/
func (a *ProviderAPIService) SetTarget(ctx context.Context, provider string) ApiSetTargetRequest {
	return ApiSetTargetRequest{
		ApiService: a,
		ctx: ctx,
		provider: provider,
...
// Execute executes the request
func (a *ProviderAPIService) SetTargetExecute(r ApiSetTargetRequest) (*http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "ProviderAPIService.SetTarget")
	if err != nil {
		return nil, &GenericOpenAPIError{error: err.Error()}
	}
