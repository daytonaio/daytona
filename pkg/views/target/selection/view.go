...
func (d ItemDelegate[T]) Render(w io.Writer, m list.Model, index int, listItem list.Item) {
	...
	providerLabel := i.target.ProviderInfo.Name
	if i.target.ProviderInfo.Label != nil && *i.target.ProviderInfo.Label != "" {
		providerLabel = *i.target.ProviderInfo.Label
	}
	idWithTargetConfigString := fmt.Sprintf("%s (%s)", i.Id(), providerLabel)
