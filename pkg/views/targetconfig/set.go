...
func NewTargetConfigNameInput(targetName *string, existingTargetNames []string) error {
	input := huh.NewInput().
		Title("Name").
		Value(targetName).
...
func SetTargetConfigForm(target *TargetConfigView, targetManifest map[string]apiclient.TargetConfigProperty) error {
