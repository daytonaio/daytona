...
func getLongDescriptionText() string {
	...
	response += "\n" + fmt.Sprintf("  \x1b[1m%s\x1b[0m%s\n\n", "Daytona", " - your Dev Environment Manager") +
		...
		fmt.Sprintf("  \x1b[1m%-*s\x1b[0m%s", helpDescriptionLabelWidth, "3) daytona server target set", "Set a target to spin up your Dev Environments on\n") +
...
func getLongDescriptionFull() string {
	...
	response +=
		"\n" +
			...
			fmt.Sprintf("%s\x1b[1m%-*s\x1b[0m%s", "    @@@@@       @@@@@@@@    ", helpDescriptionLabelWidthWithSigil, "3) daytona server target set", "Set a target to spin up your Dev Environments\n") +
